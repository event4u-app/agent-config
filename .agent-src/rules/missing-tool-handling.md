...
```
> `yq` is not installed. Cleanest way to parse ticket YAML; the
> alternative shells out to python3 (~50ms per run).
>
> 1. Install via `brew install yq` (recommended)
> 2. Use python3 fallback — slower, no install
> 3. Drop YAML — convert fixtures to JSON, `jq` only
> 4. Skip this path — different approach
```

...
When AI Council is enabled, every pending question is classified (`trivial` / `low_impact` / `medium_impact` / `high_impact` / `user_required`) and routed to `agent` / `council` / `user` per `decision_resolution` in `agents/.ai-council.yml`. **Iron Law: `high_impact` and `user_required` ALWAYS reach the user** — security, auth, migrations, billing, secrets, tenant boundaries, fenced steps never bypass. Contract: [`ai-council-config § decision resolution`](../docs/contracts/ai-council-config.md#decision-resolution-by-impact-phase-10-ask-user-routing).

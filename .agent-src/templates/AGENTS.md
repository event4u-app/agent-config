...
6. **Where do per-module agent docs live?** — When `modules.enabled: true` in `.agent-project-settings.yml`, each module under `modules.root_paths` may ship its own `{module_root}/<Module>/{agent_folder}/` directory (default `agent_folder` = `agents`) with `features/`, `roadmaps/`, `settings/contexts/`. Discovered via `scripts/_lib/agent_settings.py::enumerate_modules()`; absent when `modules.enabled: false`.

...
4. Manually verified each non-verified hit against canonical source tree (`.agent-src.uncondensed/`, `.agent-src/`, `docs/contracts/`, `agents/`).
...
| `step-99-north-star-restructure` | **Critical Partial — authority context missing** | **verified — Council phantom finding** | closure prose clarified — actual change was `.agent-src.uncondensed/contexts/authority/`, not a `agents/settings/contexts/` rename |
...
| step-99: authority context dir missing → load-bearing gap | **Council looked at wrong path** | `.agent-src.uncondensed/contexts/authority/` has 4 files (~17 kB); condensed mirror exists; `docs/contracts/file-ownership-matrix.json` lists 34 entries; kernel rules load via `load_context:` frontmatter |
...
| `slash-cmd` | search `.agent-src.uncondensed/commands/` + `.claude/skills/` |
...
- `agents/roadmaps/archive/step-99-north-star-restructure.md` line 18 — clarified that the Restructure Wave added `.agent-src.uncondensed/contexts/authority/`, not a rename of `agents/settings/contexts/`.
...
2. When writing closure prose, **cite paths explicitly** (e.g., `.agent-src.uncondensed/skills/foo/SKILL.md`) — saves reviewer effort and prevents Council misreads.

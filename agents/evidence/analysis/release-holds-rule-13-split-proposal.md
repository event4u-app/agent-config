...
| `agents/evidence/analysis/release-holds-rule-13-split-verify.sh` | verifies the four properties below without applying anything to the working tree |
...
The roadmap splits the contract change across steps 1.1 (split rule 13), 1.2 (add rule 27)
and 1.3 (one sentence on rule 20). They are three steps and one decision, because none of
them is coherent alone:

- Rule 13 split **without** rule 27 obliges a roadmap to declare a state in a grammar that
  does not exist.
- Rule 27 **without** the rule 13 split mandates a marker that rule 13 forbids by its own
...
requires the template to carry **rule 27** *and* rule 13's prohibition to be byte-identical.
One condition, spanning two steps. So the patch is single and the decision is single.

## The four properties, and how each is checked
...
**3. The numbered rules read 27, up from 26.** Step 1.2's verify condition, reproduced at
HEAD: `grep -cE '^[0-9]+\. \*\*' src/agent-src/templates/roadmaps.md` reads **26** today and
**27** with the patch applied.

**4. Rule 27 carries the non-goal sentence verbatim** — *roadmap `*incompleteness*` is never
...
is 1,814 lines with exactly one occurrence of the word `roadmap`, at `:322`, inside a comment
(`agents/evidence/analysis/release-holds-phase-0-2026-09-13.md` § 0.3). The release path is
roadmap-blind today.

### Rule 27 — release holds

New rule, after rule 26. It carries, in this order: the non-goal first; the authoring order
...
- **Step 1.4** — `new_roadmap.ts` emitting the commented `## Release holds` block, and the
  ...
  of this decision: the emitted block's grammar is rule 27's, and emitting it before the rule
...
Both were measured green at HEAD on 2026-09-13, so a red after applying is attributable to
the patch:

    ./scripts-run src/scripts/lint_roadmap_complexity    # 14 roadmap(s) complexity-clean
    ./scripts-run src/scripts/lint_roadmap_blockers      # 14 roadmap(s) blocker-contract-clean

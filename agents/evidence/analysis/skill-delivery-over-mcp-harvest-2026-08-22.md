...
| Jignesh-Ponamwar/skills-mcp; aviddiviner/skillsmcp; zouyingcao/agentskills-mcp (search hits) | snippets | D6 | Three independent implementations converge on the same 3-tool shape (`find/search`, `load SKILL.md`, `read file`). One design note worth harvesting: **embed/index only `description + trigger_phrases` (~100 tok), never the body**, because body prose pollutes the search space. This maps one-to-one onto the repo's existing `triggers:` schema and is a deterministic-index fix, not an embedding fix. |

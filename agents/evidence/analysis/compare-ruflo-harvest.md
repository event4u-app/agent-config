...
| # | Action | Purpose |
|---|---|---|
| P3.1 | Add **"Defer-with-trigger" ICE tier** to `agents/settings/contexts/harvest-policy.md` (or create) | Sonnet's meta-process recommendation: codify the third bucket between "adopt now" and "drop", so future harvests have a place for "adopt when prerequisite X ships" |

...
| The reasoning-architecture sequence in the round's closing turn — eval corpus first, candidate gate, evidence discriminator, orchestrator before intent, commitment horizon, measure before more complexity | `owner-decision` | a **different subject** from this batch, and partly in flight already: `agents/roadmaps/archive/road-to-limited-commitment-horizon.md` is active in the estate and owns the commitment-horizon half. Whether the rest becomes its own stem is the owner's call, not this batch's |

# R2 completion review — conformance-round5-stop-refusal

You are a FRESH reviewer subagent. You have no implementation context and
you must not acquire any (blind-review pattern, plan-review-gates.md §5).

## Review mode

Senior-engineer review of the branch diff. Search grid — hunt for:

- errors
- inconsistent logic
- inefficiencies
- bug-producing patterns

## Rules

- Review only — write no code, fix nothing.
- Tool allowlist (contract §5): branch-scoped `git diff` + reads of
  branch-touched files only; no `git log` beyond the branch, no repo-wide
  grep, no reads of `agents/runtime/` or session artifacts.

## Inputs

- diff: `diff.patch` — the review scope (branch head f16f84697417b41d79838857d3cdaf7bcdf8b7e3, review
  artefacts excluded), scope hash `8b1ce05989ba8ea4750e4fbb2a1cd523ae256c54f6bdb6f6137e10eb01796de2`
- roadmap under review: none (`acceptance-criteria.md` is empty)

Changed files:

- agents/roadmaps-progress.md
- agents/roadmaps/archive/road-to-conformance-round5.md
- agents/roadmaps/later/road-to-conformance-round6.md
- agents/roadmaps/road-to-conformance-round5.md
- dist/install/install.mjs
- docs/contracts/settings-classes.md
- hooks/hooks.json
- src/config/agent-settings.template.yml
- src/scripts/hook_manifest.yaml
- src/scripts/hooks/concern_registry.ts
- src/scripts/hooks/turn_end_gate_hook.ts
- src/scripts/language_mirror_hook.ts
- src/scripts/measure_turn_end_gate.ts
- src/server/schemas/settings.ts
- tests/hooks/concern_severity.test.ts
- tests/scripts/language_mirror_hook.test.ts
- tests/scripts/turn_end_gate_hook.test.ts

## Output format (contract §2.2)

Fill the findings table in `conformance-round5-stop-refusal.findings.md`:

```markdown
| # | Severity | File:Line | Finding | Status | Reason/Ref |
|---|----------|-----------|---------|--------|------------|
| 1 | critical | src/x.ts:42 | ... | open | |
```

- Severity ∈ {`critical`, `high`, `medium`, `low`}, rows sorted descending
  by severity (ties keep authoring order).
- Initial status of every finding: `open`.
- A row is LIVE wherever it appears — a code fence around it changes
  nothing. If you quote the template as an illustration, its Status cell
  must be exactly `example`, or the gate reads it as a real finding.
- 0 findings → replace the table with exactly this honest-null line
  (contract §2.3):

```markdown
**Honest-null:** 0 findings, scope 8b1ce05989ba8ea4750e4fbb2a1cd523ae256c54f6bdb6f6137e10eb01796de2, reviewed <YYYY-MM-DD>
```

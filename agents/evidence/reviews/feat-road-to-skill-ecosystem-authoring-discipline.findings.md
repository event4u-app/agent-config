...
| 5 | medium | src/agent-src/contexts/execution/mandated-lines.md:5 | The contract says it is loaded by four rules. Only `downstream-changes` links it. `think-before-action` (edited by this very diff), `non-destructive-by-default`, and `commit-policy` contain zero mentions, so the intent, authorization, and commit lines have no routing edge from the rule that owns their decision point. | fixed | PARTIAL — think-before-action now routes to the contract. The other two are commit-policy and non-destructive-by-default, both KERNEL rules: the safety-floor guard blocked the edit and it is right to, since scope-control requires a kernel change to be its own PR with a 24 h soak. Reverted rather than bypassed, and the contract now states which two edges are missing and why instead of claiming four |

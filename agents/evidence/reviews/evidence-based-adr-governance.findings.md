...
(11 at baseline) · `adr_cite_check ADR-240` · `adr/evidence_census --out <tmp>` (byte-identical to the
...
- **AC-7 holds.** No fixture, rule path or tool output lets a grade authorize anything: `isLowEvidenceAccepted`
  ...
  explicitly, the two `grade-as-permission` routing cases pin the refusal positively, ADR-240 is
...
- **AC-9 holds.** No ADR frontmatter backfill (`provenance`/`evidence` appear only on ADR-240 and on nothing
  else in `docs/decisions/`), ADR-240 is not accepted, `0B`-gated step 4.2 is `[~]`, and no Safety, Privacy,
...
- **Round-1 #4 (high, `fixed`) is not fixed — it inverted.** The disposition says the sweep now "names both
  uncovered records — ADR-240 with its disposition, ADR-238 as an explicit REVIEW-NOW gap". ADR-240 is not
  ...
  ADR-240" — dropping ADR-238, the only record that genuinely has none. See row 2. The class round 1 caught

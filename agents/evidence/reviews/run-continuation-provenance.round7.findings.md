...
## Where round 7 leaves this

Seven rounds, **55 findings**, all closed: 9, 7, 8, 7, 11, 7, 6. Seven `high`, and
**six of the seven sat in the immediately preceding round's fix** — round 3's two
in round 2's, round 5's two in round 4's, round 6's one in round 5's, round 7's two
in round 6's. Only round 1's high was in the original merged change.

Round 7 was run because the maintainer required it after round 6 said the decision
was theirs, and it earned itself twice over: both highs were live budget-reset
paths that round 6 had left open while closing the narrower half of the same defect.

The round-7 fixes contain new logic again — a roadmap-keyed state path with a
migrating reader, a two-fire absence confirmation, a once-per-run inert event. The
honest statement is unchanged from round 6, with one more data point behind it: the
loop does not terminate by itself, the finding rate is not falling, and the
implementing session is not the right judge of when it is finished.

`check_completion_review` therefore reports `stale-review` rather than green,
because these fixes moved the reviewed content past the scope this artefact is
bound to. Stated here rather than worked around.

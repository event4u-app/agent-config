...
# Autonomous roadmap drain — run 24, 2026-09-11

> **This run destroyed run 21's record on its first attempt at writing this file, exactly
> as run 23 did, and the repair is in the history rather than amended out of it.** The brief
> said to write the summary at `agents/evidence/drain-run-summary.md`. That path is not
> free: it holds **run 21**, 1,285 lines, and its own header says in as many words that it
> was deliberately left in place rather than overwritten. The Write landed as an *update*,
> the commit reported `85 insertions(+), 1269 deletions(-)`, and only that deletion count
> surfaced it. Restored from `HEAD~1` in the next commit; this file is the summary.
>
> **It is a recurrence, and that is the part worth recording.** Run 23's header carries the
> identical confession about the identical file. The disposition that followed it was a
> warning in prose, inside the very file a fresh run does not read before writing — so it
> could not have worked, and did not. Per `recurring-criticism`, a repeat indicts the
> system rather than the run: what would actually stop it is the brief naming the
> per-run path (`drain-run-summary-run-N.md`), or a check refusing a write that deletes
> more lines than it adds from an evidence artifact. Neither exists; naming that here is

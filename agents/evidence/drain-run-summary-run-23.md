...
# Autonomous roadmap drain — run 23, 2026-09-10

> **This run's first attempt at writing this record destroyed another one, and the repair is
> in the history rather than amended out of it.** The brief said to append the summary at
> `agents/evidence/drain-run-summary.md`. That path is not free: it holds **run 21**, 1,285
> lines, and its own header says in as many words that it was deliberately left in place
> because *"overwriting it would destroy that record to keep a pointer current"*. The write
> replaced it, and the commit reported `202 insertions, 1265 deletions` — which is what
> caught it. Run 21 was restored byte-identical from `HEAD~1` in the next commit and this
> record took its own numbered file, following runs 19 through 22.
>
> The failure was not the brief. It was writing to a named path without reading it first,
> which is the one precaution that would have cost nothing. Recorded here because a run whose
> whole subject is honest accounting should not quietly fix its own worst moment.

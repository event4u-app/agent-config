...
| 13 | `routing_signal_measurement.test.ts` 5.1 is RED on `main`, CI-only, and this roadmap owns the artefact it guards | implementation | The test asserts that the published verdict at `agents/evidence/analysis/routing-body-signal-verdict.json` reproduces from a fresh `measure(REPO)` over the skill catalogue. On `main` at run `33424783559` (job `99595610608`, 2026-08-31T18:26Z) it fails with `expected { partition: 'train', ... } to deeply equal { partition: 'train', ... }` on the `corpus` comparison. **Verified pre-existing, not branch-introduced:** the identical assertion fails on `main` before `drain/council-topology-evidence-close` existed, and the same test is GREEN locally (17/17) on both drain branches. So the corpus the CI runner measures differs from the corpus a local checkout measures, and the published verdict matches only the latter. A verdict that reproduces on one environment and not the other is not a reproducible verdict, which is the property 5.1 exists to assert | **Recorded, not fixed, and the gap is named rather than papered over.** What is established: the failure is pre-existing, CI-only, and confined to the `corpus` field. What is NOT established: which catalogue entries differ, and whether the fix is to repoint `measure()`'s scan root or to regenerate the published verdict under CI conditions. Diagnosing that needs a CI-side dump of the two corpora and is out of scope for the drain run that found it. **Three hypotheses were excluded rather than left open, so the next reader does not re-walk them:** (1) NOT a tracked-vs-untracked input — every corpus input is tracked, `src/skills/*/evals/triggers.json` counts 100 on disk and 100 in `git ls-files`, and `loadCatalogue` reads only `src/skills/*/SKILL.md`; (2) NOT case-folding between a case-insensitive macOS checkout and a case-sensitive Linux runner — lower-casing the skill directory names yields no duplicate; (3) NOT branch-introduced — the same assertion fails on `main` and the same test is green locally on both drain branches. What remains is that the CI job observes a different `src/skills/` tree than a plain checkout does, which points at a workflow step that mutates it before the tests run. Unverified: no attempt was made to identify that step. Anchored here rather than in a new roadmap because this file already cites both the verdict artefact (`:1591`, `:2159`) and the test (`:1599`), and both estate ratchets are at zero headroom | Risk Register |

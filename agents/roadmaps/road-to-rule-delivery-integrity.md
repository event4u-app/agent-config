...
- [x] **P3.1 `_emit_claude_rule()` in `condense.ts`** (Path A only), sibling of the
      ...
      *Verify:* one unit fixture per trigger shape; emitted frontmatter contains
      ...
      <!-- unverified, and stated rather than implied: the host's DOCUMENTED read-triggered semantics for `paths:` are taken from P0.1's fixture, not re-probed here. P0.1's own closing note asks for this ("P3.1's fixtures must exercise them rather than assume them") and the fixtures exercise the EMITTER, not the host's loader. Whether a `paths:`-scoped rule actually fires for these 25 trigger shapes in a live session is a first-party observation nobody has made yet; the byte figure above is what the projection now costs, not proof the host honours it. -->

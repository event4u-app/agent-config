...
- [x] **Keep `song-to-script` as a skill — council dissent recorded.** Both members argued for inlining (Rule-of-Three, cross-package coupling). Counter-evidence the council did not have: the sibling `/video:from-script` **already** declares four skills from the ai-video pack and the repo's `validate-deps.sh` already fails fast on a missing skill id — the coupling pattern is established and guarded, not new. The named second use case ("re-time an existing script to a track") is real. Decision: keep the skill, but neutralise the council's underlying worry by (a) declaring the pack dependency in frontmatter and (b) confirming `validate-deps.sh` emits a clear missing-id error, not an opaque failure. **Reversible** — if a second consumer never lands, fold into the command later.
...
- [ ] Run the real-corpus honesty experiment the council demanded (≥ 15 varied tracks). <!-- PENDING (ffmpeg host): ffmpeg not installed on this authoring host + no offline commercial-track corpus. Synthetic profiles (silence-gap / dynamic / flat) are exercised by tests/test_probe_audio.py; the full real-corpus sweep needs an ffmpeg host. -->
...
- [ ] Verify `bash probe-audio.sh <fixture.wav>` returns valid JSON end-to-end. <!-- PENDING (ffmpeg host): ffmpeg not on this authoring host. Verified what is possible offline: `bash -n` parses clean, arg/exit paths (usage→2, missing-file→2) correct, test collects + skips cleanly. Full run executes on any ffmpeg host / CI. -->
...
- [ ] Dry-run the whole pipeline (`AIV_DRYRUN=true`) end-to-end. <!-- PENDING (ffmpeg host): ffmpeg not installed on this authoring host (probe-audio + stitch + mux all need it). Runs on any ffmpeg host / CI. All non-ffmpeg structure is verified by the registration + probe (skip-guarded) tests. -->

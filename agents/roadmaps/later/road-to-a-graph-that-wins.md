...
owner: maintainer
wake_condition: "road-to-a-graph-that-is-shipped is archived AND ADR-258 § 3 names the two benchmark subject repositories"
review_by: 2026-12-07
capability_gap: none
estate_growth_exempt: "Parked, not active: it may not start before its parent ships, because every measurement in it would otherwise measure an engine no consumer holds. Receivers named here exist at 4c24be50: internal/bench/code-graph/, src/scripts/detect_silent_catch.ts, src/scripts/detect_verification_tampering.ts, agents/evidence/release-findings/, src/scripts/_lib/paired_verdict.ts."
---
...
construction and every class ties (`internal/bench/reports/code-graph-vs-grep-inrepo-v2-rerun-2026-09-04.md:74-82`).
The earlier third-party registration lost its inputs — four question files absent on
re-run (`internal/bench/code-graph/PREREGISTRATION-inrepo-2026-08-28.md:10-15`). Both
...
- [ ] 2.1 Corpus v3 registered **before the engine sees the repos**: two public OSS repos
      per ADR-258 § 3 (fitness: ≥ 200 routes, Eloquent relations, migrations, PHPUnit/Pest
...
- [ ] 2.5 Run; publish both directions; no renegotiation. Record ADR-246/257's benchmark

...
- [x] **Only after the precompiled path is live**: move `tsx` from
  ...
  *Verify:* fresh install contains no tsx in the runtime tree; all
  installed-surface commands still work. <!-- done 2026-07-27: tsx → devDependencies (npm ls tsx --omit=dev empty; baseline fixture updated deliberately); mcp:run precompiled via build:mcp-bundle → dist/mcp/server.mjs (bundle-aware _project_root + catalog path; initialize handshake green: 473 prompts/265 resources/31 tools); _dispatch.bash prefers the bundle; hooks hot path already node-only. Remaining tsx sites are dev-tree/npx-fallback only: require_tsx/exec_ts fall back to `npx tsx` for delegate maintainer commands (one-time download in a consumer, documented safety net) — no tsx in the shipped runtime tree. CORRECTION 2026-07-31 (ADR-204): the "delegate maintainer commands" clause above was FALSE. 18 commands dispatch via exec_ts, and 9 of them are Tier-0/1 consumer surface in the consumer --help (sync, validate, doctor, update, upgrade, export, prune, uninstall, versions), so the EBADDEVENGINES exposure this item claimed to close was RELOCATED to the consumer command path, not removed — plus a ~5.7x latency tax (p50 346-392ms npx vs 56-71ms bundled). Closed by precompiling that surface to dist/cli-delegate/ (build:cli-delegate, one --splitting entry per command) with exec_ts preferring it; tsx stays a devDependency, so this completes the item rather than reverting it -->

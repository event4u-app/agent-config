...
relates:
  - slug: road-to-admissible-council-seats
    relation: depends
    note: >
      That roadmap's step 3.3 (run the free-jury panel shadow-only over a >=14-day
      window) is gated on exactly the spend this file's metered-backend-park owns,
      and its own free-seat-measurement-spend blocker was resolved DECLINED on
      2026-09-07. The obligation is merged here so it is carried by the file that
      already holds the park, rather than left in a roadmap that is complete.

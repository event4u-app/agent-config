...
- [ ] **1.5** Wizard "Apply" step calls `scripts/install.py` with the payload shape locked in Phase 0.4 / D12. Single dry-run preview block before commit. Implementation strictly follows the discriminator decision — no improvisation at this step. **Deferred** — `WizardPayloadV2 → scripts/install.py` plumbing spans `packages/core/installer/` (Node→Python bridge), `scripts/install.py` (router branch), and the on-disk `wizard-apply-payload.schema.json` contract; cross-package surface is too large for an autonomous slice and is tracked as a dedicated follow-up roadmap. The discriminator gate (Phase 0.4 / D12) is already locked, so the contract cannot fork while 1.5 sits open.
...
- [ ] **1.8** Playwright spec `tests/e2e/setup-wizard-9-steps.spec.ts` walks all 9 steps in dry-run, asserts the apply payload matches the locked schema, asserts zero writes. **Deferred** — Playwright harness still not bootstrapped (same blocker as Phase 0.2); tracked in the Phase 0 bootstrap follow-up alongside `installer-gui-dry-run.spec.ts`.
- [ ] **1.9** **(A5)** Ship the 9-step wizard as a normal npm minor (`@event4u/agent-config@2.x.z`). Kill-switch = downgrade to the prior `2.x.y`. No parallel "legacy 7-step" code path; the old wizard never had AI/Pack selection so there is no equivalent flow to preserve. **Deferred** — release-management step, owned by the maintainer (version bump + npm publish + tag); not in scope for an autonomous code slice.
...
- [x] **4.4** `scripts/lint_agents_layout.py` learns the new shape: a consumer repo is **valid** if `agents/overrides/` and `agents/.event4u-bridge.yml` are the only `agents/*` artefacts. Anything else surfaces as a warning, pointing at `agent-config settings migrate`. Implementation: `.event4u-bridge.yml` added to `ALLOWED_FLAT_FILES`; new `CONSUMER_EXPECTED_ENTRIES` set + `find_consumer_warnings()` flag legacy entries (warning tier — exit 0 by default, `--strict` flips to exit 1); `is_source_repo()` detects maintainer mode via `.agent-src.uncompressed/`, `.agent-src/`, or `packages/<pack>/.agent-src.uncompressed/`, suppressing consumer warnings in the source repo. 12 new test cases in `tests/test_lint_agents_layout.py` (21 total, all green).

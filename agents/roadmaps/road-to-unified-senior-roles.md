...
| Block | Slot | Pre-condition | Foundation set? |
|---|---|---|---|
| **K — Cross-role foundation** | 1st | Sibling Block A ≥ 50 % (persona spine stable) | ✅ K1–K4 (foundation) |
| **L — Senior PO / discovery / marketing skills** | 2nd | K1–K2 (context-spine + related-skills mechanic) | ⚠️ L1–L2 are foundation; L3–L8 follow ICE order (council Q6) |
| **N — Wave-2 personas (PO / RevOps / Tech-writer / Discovery-lead)** | 3rd, parallel-start with L's last sub-steps | Sibling Block A ≥ 100 % (A1–A6 shipped); K3 (cross-role glue) | ❌ N1 is foundation, N2–N4 follow ICE order |

The foundation set for this roadmap is **K + L1–L2 + N1 = 7
sub-steps** (post-council Q6, sibling-velocity-honest: A1–A6 shipped
...
Block sequence anchors the foundation set; the rest is backlog under
the relevant block — those entries do *not* receive checklist items
so the dashboard does not score them as work-in-progress until they
move into a foundation slot.

| Block | Slot | Notes |
|---|---|---|
| **K — Cross-role foundation** | Foundation | K1–K4 (4 sub-steps) |
| **L — Senior PO / discovery / marketing skills** | Foundation lead | L1–L2 are foundation; L3–L8 follow ICE order (council Q6) |
| **N — Wave-2 personas** | Foundation lead | N1 is foundation; N2–N4 schema-bound on sibling A6, follow after |
...
- [ ] **L1** — `customer-research` skill (`tier: senior`): discovery interviews, jobs-to-be-done framing, ICP synthesis, transcript-to-insight loop. Borrows hypothesis-loop pattern (P3) and mental-models JTBD/Pareto (K4). **Out:** persona-CRM enrichment, channel-targeting.
- [ ] **L2** — `release-comms` skill (`tier: senior`): changelog-to-narrative, what-changed-and-why-it-matters framing for our own releases (the package itself + consumer release notes). Adopts marketingskills' "value-not-feature" heuristic without the channel surface.
...
- [ ] **N1** — `product-owner` persona (`tier: senior`): identity = "owns the why and the what", critical rules = no AC drift / explicit trade-offs / decision-record on every scope change. **Capabilities (default-loaded, override-friendly per council Q4 — ABSORB):** `refine-ticket`, `estimate-ticket`, L3 (`decision-record`), L4 (`stakeholder-tradeoff`). Workflows = ticket-refinement-loop + roadmap-execution. Consistency rationale: matches sibling A2 (`backend-architect`) which absorbs `code-review-security` + `refactor-architect` as defaults.
...
| **L3 — `decision-record`** | 4 | 4 | 4 | **64** | Most reusable single skill; first slot after L1–L2 |
| **L4 — `stakeholder-tradeoff`** | 4 | 3 | 2 | **24** | Composes sibling C8; follows L3 |
| **L5–L8** | 3 | 3 | 2 | **18** | Follow ICE order after L3–L4; re-eval at each slot |
| **N1 — `product-owner` persona** | 4 | 4 | 3 | **48** | First Wave-2 persona; unblocks remaining N |
| **N2–N4** | 3 | 3 | 2 | **18** | Schema-bound on sibling A6; follow once A6 is done |

**Foundation set:** K1–K4 + L1–L2 + N1 = **7 sub-steps** (council Q6,
sibling-velocity-honest — A1–A6 shipped at ~1 sub-step/sequence-step).
Foundation: K (K1–K4) + L1–L2 + N1. L3–L8 + N2–N4 are the
**ready-to-start tail** that follows the foundation in ICE order. L3 +
L4 stay highest-ICE candidates for the first slots after the
foundation.
...
| R28 | **Two roadmaps, one maintainer** — sibling + this roadmap competing for the same execution slots, both stall | meta | M | H | This roadmap's foundation set is **K + L1–L2 + N1 = 7 sub-steps** (post-council Q6 cut); sibling stays prime; if capacity slips, this roadmap defers, not the sibling | (meta) |
...
| Q6 | Foundation fit (9 vs honest) | **Drop to 7** — K1–K4 + L1–L2 + N1; L3–L4 follow after foundation (sibling-velocity math: ~1 sub-step/sequence-step) | Foundation tables + ICE table + R28 |
...
> Block A ≥ 50 % (A1–A4 shipped). The 7-sub-step foundation set is
> K1–K4 + L1–L2 + N1; L3–L8 + N2–N4 are ready-to-start and follow in
> ICE order. No further council round needed before execution.

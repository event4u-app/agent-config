...
### owner-autonomy-batch

Status: open
Owner: owner
Blocks: Phase 0B (all rows), Phase 4.2

Three Hard-Floor rulings the source instruction did not authorize: the
`commit-policy.md:37` fence versus ADR-237's delegation shape, ADR-005 § 1's
no-auto-merge rule, and the ADR-211 sweep-row contradiction. Each widens agent
write authority or resolves an owner-routed question. What closes it: an owner
ruling per row, recorded with the consequence shape 0B states.

### authority-coupling-decision

Status: open
Owner: owner
Blocks: Phase 7

Whether an independently validated evidence grade may reduce the
authorization burden. Deferred by unanimous council ruling, not by fatigue:
fusing the metadata with the authority is the design's central defect. What
closes it: the Phase 6 shadow-mode measurements meeting their pre-registered
thresholds, a passed suspension drill, and an owner ruling either way — a
published null being an acceptable outcome.

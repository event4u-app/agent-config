...
- [ ] **0.1.1** Define schema: `[source_phase_artefact] × [dependency_type] × [target_phase_artefact] = {NONE, READ_ONLY, WRITE, BLOCKS_IF_CONCURRENT}`.
- [ ] **0.1.2** Implement `scripts/generate_ownership_matrix.py`. Greps for `load_context:` references across rules, `skill:` frontmatter in rules, command invocations in skills, context-file inclusions. Must account for **context-file fan-out transitively up to depth 2** (G2 — fix in v3.1): if rule R loads context A, and context A loads context B, then both A and B appear as `READ_ONLY` deps of R. Depth-3 chains fail the build (matches the `load_context:` nesting cap from 0.2.4 — same invariant, two enforcement points). Without transitive resolution, R5 confirmed the matrix would underreport coupling and let `BLOCKS_IF_CONCURRENT` cells slip through silently.
- [ ] **0.1.3** Output: `docs/contracts/file-ownership-matrix.json` (machine, internal-locked) + `agents/contexts/structural/file-ownership-matrix.md` (human-readable, regenerated from JSON).
- [ ] **0.1.4** Lock-in: matrix generated **before** Phase 1 starts; re-validated after each phase commit. Phase 2B and Phase 3 sequencing is gated on `BLOCKS_IF_CONCURRENT` cells being empty.
- [ ] **0.1.5 Phase-2A re-generation gate (A1 — fix in v3.1).** Every Phase-2A commit that adds, moves, or modifies a context file (i.e. every `2A.2` extraction or `2A.3` slim) re-runs `generate_ownership_matrix.py` as a pre-commit hook AND in CI. Diff vs. previous green run must be reviewed: new `READ_ONLY` cells are accepted automatically; any new `BLOCKS_IF_CONCURRENT` cell fails the build and requires a roadmap revision. Without this, the matrix is only true at Phase-0 lock-in and silently drifts as 2A creates the very contexts the matrix is supposed to track. Same hook fires on Phase 2B, 3, 6 commits.

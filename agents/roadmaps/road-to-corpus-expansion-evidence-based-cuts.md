...
2. **Pass B (deferred — own roadmap if Pass A surfaces opportunity):** if the audit shows kernel rule bodies are bloated *and* condensation gains are exhausted, draft a "kernel body refactor → router-pointer + skill body" pass. **Not committed to in this roadmap.** Pass B only opens if Pass A's telemetry shows a kernel rule firing in <30 % of corpus tasks AND its body exceeds 3 000 chars.

- **Source:** Chat-thread 2026-05-29 (post-PR-#280); the council session on the previous roadmap (`agents/runtime/council/responses/value-dashboard-netto-optimization.json`) flagged corpus-blindness as the largest unknown.
...
**Exit criteria:** `corpus-surface-inventory.md` exists with every audit row classified `addressable` or `state-bound`; each addressable row names one task shape; extension groupings ≤ 5 files / ≤ 8 tasks each.
...
- [ ] **Step 3:** Run `task value:telemetry:replay` against the widened manifest. The new `internal/bench/reports/router-telemetry/<UTC>.json` carries: updated `per_rule_activations` (now hopefully covering most of the 20 previously-never-matched tier-1 rules), updated `never_matched_tier1`, updated `panel_b_untouchable_rules` (still based on Track B only — the new corpora are not Panel B sources).
- [ ] **Step 4:** Write a one-page diff report `internal/bench/reports/router-telemetry/widened-corpus-diff-<UTC>.md`: "before pass 1: 20 never-matched; after corpus expansion: N never-matched; the N − 20 rules now have measured activations".

**Exit criteria:** new telemetry report exists; `intended_vs_observed_match` block populated for every new task; the never-matched-tier-1 set has shrunk meaningfully (target: ≥ 10 newly-activated rules, exact number reported in the diff doc).
...
- [ ] **Step 4:** Build a body-size analysis: for each surviving tier-1 rule, record `(rule_id, chars, activation_rate)`. Rules in the bottom-decile activation rate AND top-decile size are candidates for the **Pass B kernel-body refactor** that this roadmap explicitly defers. Save as `internal/bench/reports/router-telemetry/tier1-size-activation-pareto.md` — informational input to Pass B.

**Exit criteria:** updated audit doc with verdict per candidate rule; cut-candidates surfaced as user-decision options (autonomous decision allowed only when the rule's defence is `no defence found` AND its activation rate across all 8 corpora is 0); Pass B input doc exists.

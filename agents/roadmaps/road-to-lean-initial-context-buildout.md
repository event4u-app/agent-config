...
- [ ] **2.1** Description-budget tightening (parent 1.0): lower per-artefact caps (skill 300→220, command 500→200, rule 500→160, guideline →160) in `scripts/schemas/*.json`. Gate each cut two ways: (a) top-10-longest descriptions per class rewritten to fit without losing the "when to invoke" signal; (b) `task trigger-coverage` stays 26/26 — a shorter description that stops a skill firing fails the cut. Ship behind a lint warning window.

...
4. **Session sources: chat-history JSONL primary, native transcripts fallback.** The cross-host log is the canonical source (works for every hook-capable host); when it is empty/disabled, fall back to `~/.claude/projects/<slug>/*.jsonl` via `cc_transcript.ts` (Claude Code only, mirrors the `mine_session.ts` precedent incl. explicit transcript-access consent flag).
5. **Host launch adapter table, Claude Code first.** `--launch` spawns a fresh interactive session via a per-host adapter (`claude` v1; `codex` / `gemini` slots follow the `HOST_CONFIGS` shape and can land later without interface change). Without `--launch` the CLI prints "start a new session — the handoff will be injected automatically", which already fulfils the goal on every hook-capable host.

## Phase 1: Session enumeration layer

- [ ] **Step 1:** Create `src/scripts/_cli/handoff_sessions.ts` — a pure library that lists recent sessions as `{ id, source: 'chat-history' | 'claude-transcript', startedAt, endedAt, branch?, entryCount, summary }`. Primary source: reuse `chat_history.ts` session listing (import its functions; do not shell out). Fallback: enumerate `~/.claude/projects/<current-repo-slug>/*.jsonl` via `_lib/cc_transcript.ts`, newest first, capped at 15.
- [ ] **Step 2:** Derive the per-session summary deterministically: first user prompt (trimmed, 120 chars) + last user prompt when different; branch from the session's recorded metadata when present.
- [ ] **Step 3:** Unit tests with fixture JSONL files (both sources, empty-source fallback, cap). <!-- verify: npx vitest run tests/scripts/handoff_sessions.test.ts -->
...
- [ ] `agent-config handoff` in a TTY lists the recent sessions of this project (chat-history primary, Claude transcripts fallback) and lets the user pick one.
...
- [ ] Starting any new session afterwards injects the handoff exactly once as spotlighted DATA context, then removes the file; `--launch` starts the fresh `claude` session directly.
...
- `codex` / `gemini` full launch + their native session stores (`~/.codex/sessions` etc.) are deliberately a later increment: the adapter table ships with the seam, and the hook-injection path already works for every hook-capable host today.

...
- [x] **A1 — Execution-subagent contract (keystone, Phase 0/1 of the draft).** `subagent-v1` contract: agent-config frontmatter schema + native-Claude-Code translation, `model_tier`→native `model:` via the existing generator mapping (ADR-035, **do not invent a new mapping**). Add `subagent` (or `agent`) as the 5th discovery category + glob + vocabulary lint + determinism. Frame explicitly as **static artifact projection — dispatch decision stays with the agent/skill, never runtime-enforced** (resolves the apparent no-runtime tension). ADR via council. <!-- DONE: council 2026-07-04 (claude-sonnet-4-5 + gpt-4o) → ADR-109 ratifies the contract (new `subagent` category, governance-required schema, model_tier reuse incl. `inherit` passthrough, cross-host degradation to loadable context, default-off, banned runtime fields). (#707) ADR-109 + subagent.schema.json + src/subagents/production-validator.md + validate_frontmatter enforcement + contract test; (#708) standalone determinism lint (const/uniqueItems/name); 5th discovery category via Option B — ADR-109 Amendment 1: build_discovery_manifest _CATEGORY_VALUES/_CATEGORY_SCHEMA/_iter_artefacts glob + _classify subagent-branch (default-off, packs:[]), discovery-manifest.schema.json enum + by_category; (#718-era) native-CC projection generate_claude_subagents → .claude/agents/ (model_tier→model, inherit passthrough); (this PR) cross-host degradation generate_subagent_host_contexts → .cursor/subagents/ + .windsurf/subagents/ + .clinerules/<name>.subagent.md as passive-reference context files (no faked @-dispatch; governance parity), council 2026-07-05 (claude-sonnet-4-5 + gpt-4o) converged project-over-honest-null, ADR-109 §4 realised as written (no amendment). A2 (wedge-only distribution) + A3 (eval harness) are their own lines; A3's billable 3-arm run stays operator-gated. -->
...
- [x] **A3 — Eval harness on the #699 bench.** Reuse `internal/bench/orchestration/` (do **not** build a parallel bench). Dual baselines: inline host **and** generic inline dispatch. Tokens always in the table. The one-file negative-control task MUST lose. `verdict_changed_outcome` is an **extension field on the #699 telemetry object**, not a second schema. <!-- DONE: harness (PR #715) + billable 3-arm real-session run 2026-07-05 (operator-authorised) → HONEST NULL, results in internal/bench/orchestration/pv-a3-results.md. Run via the Task tool (6 fresh independent agents = 3 arms × pv-01/pv-02, token_delta_provenance: measured). pv-01: all 3 arms NOT READY citing charge.ts:13 → no baseline fooled → verdict_changed_outcome:false → NO LIFT. pv-02 control: all 3 arms READY (no spurious finding, incl. production-validator) → control passes. production-validator ~16% cheaper (229k vs 274k) but zero verdict lift → does NOT clear Gate A → stays default-off in src/ per ADR-109 Gate A. Caveat N=1/arm (README calls ≥20); the real signal is external-adopter tasks (feeds D3), not this static fixture. A4/A5 already delivered (learning-loop mining). -->
...
- [ ] **B4 — Proof page + minimal Starlight.** Generated (not hand-maintained) claim→code binding table, honest-null benchmarks *including the nulls*, and a "verify it yourself" command block that runs on a fresh checkout. Site catalog generated from the discovery manifest; ≤5 top sections; not public yet. <!-- partial: proof-page DELIVERED (docs/proof.md, build_proof.ts, drift-gated). Starlight site STOOD UP 2026-07-05 (operator-authorised, in worktree feat/b4-proof-site): isolated site/ workspace (astro 7.0.6 + @astrojs/starlight 0.41.3, own package.json/lockfile/node_modules — zero package-dep pollution); ≤5-section IA (Start/Proof/Reference); hosts proof + benchmark + claims via NO-DRIFT sync (site/sync-docs.mjs copies canonical docs/*.md at prebuild, injects Starlight title, rewrites cross-links → site routes; synced pages gitignored so no drift). Splash landing (index.mdx). `npm run build` GREEN — 5 pages, zero dangling internal .md links, pagefind search + sitemap. (4) CI link-checker on the site build — DONE 2026-07-05 (operator-authorised, this PR): src/scripts/check_site_links.ts (scans built site/dist for dangling internal links — astro build alone does NOT fail on them; base-prefix + trailing-slash-dir aware; exit 2 if unbuilt), task check-site-links + site-build-check, .github/workflows/site.yml (path-filtered site/** → npm ci + build + check), test tests/scripts/check_site_links.test.ts. The checker immediately caught a REAL pre-existing bug: every page referenced /agent-config/favicon.svg with no site/public/favicon.svg (404 in prod) → added a neutral placeholder favicon; build+check now green. REMAINING (not autonomous): (1) deploy workflow / GitHub-Pages target = maintainer infra decision; (2) catalog pages generated from the discovery manifest (build_site_catalog.ts — bigger build); (3) full 50-doc migration + redundant-doc consolidation. B4 stays open until deploy + catalog land. -->
...
- [x] **B6 — Per-skill honest-null `gaps:` + proof-page "Known Limits" (net-new; the falsifiability moat applied to the skills themselves).** For the wedge + top-N most-used skills (**not** all 261 up front), add a `gaps:` array where each entry cites a witness test that demonstrates the failure; render a "Known Limits" column in the proof-page skill table. This is the offensive-differentiation item the 84.8%-reference structurally cannot ship. **Falsifiability lock:** CI (`check_skills`/witness) requires every `gaps:` entry to have a passing test that reproduces the failure; a stale-gap audit flags entries that no longer fail. **NOT:** not fixing the gaps (future work); not auto-discovered (human-gated); not all 261 at once. <!-- done: skill.schema.json gaps:[{description,witness}] (optional, human-curated); check_skill_gaps.ts pointer-integrity lock (every witness path exists) wired into ci-fast + both root ci lists; build_proof renders proof.md § 3 "Known limits" table. First TRUTHFUL exemplar = check-refs (only validates known-root path refs → broken RELATIVE links never reported), witness tests/scripts/witness/check_refs_relative_gap.test.ts reproduces it (gap + control), verified deterministic. Witness passing + going red when the gap is fixed = the stale-gap audit via the normal suite. Note: the witness model fits DETERMINISTIC (script-backed) skills; LLM/prose skills + the subagent wedge can't be headlessly witnessed (same billable constraint as A3), so gaps stay on script-backed skills. +tests. -->

- [x] **B7 — Comparison-honesty table on the proof page (folds ruflo-borrowings framing; makes the moat legible).** A `[claim · our evidence · their evidence · checkable?]` table where the "checkable?" column is the differentiator — our claims verify via witness/ledger; the reference's headline claim does not. **Falsifiability lock:** CI requires every "our evidence" cell to resolve to a witness/honest-null and every "checkable ✓" to a passing test. **NOT:** not a hit-piece; only publicly-checkable third-party facts; the 84.8% is cited *only* as uncheckable, never counter-claimed. <!-- done: docs/comparison.yaml (4 rows, no named source) + src/scripts/check_comparison.ts (reuses check_claims pointer resolver) + build_proof § 4 render + task check-comparison in ci-fast/Taskfile + witness test -->
    - Data: `docs/comparison.yaml` — 4 rows `[claim · our_evidence(resolvable pointer) · their_evidence(generic category prose, no named competitor) · checkable]`; the 84.8% cited only as uncheckable.
    - Lock: `src/scripts/check_comparison.ts` (reuses `pointer_unresolved` from `check_claims`) → `task check-comparison` in `ci-fast.yml` + both root Taskfile ci lists; `tests/scripts/check_comparison.test.ts`.
    - Render: `build_proof.ts` § 4 "What is checkable — us vs. the category"; drift-guarded by `build-proof-check`.
- [ ] **B8 — Recorded proof-page demo (asciinema/GIF; no backend).** Embed short recorded runs of the wedge + 2–3 high-value skills so a skeptic sees real output in < 60s without installing. The live "Try Now" backend widget is **rejected** — runtime backend (no-runtime floor) + the validation-wedge does not demo generatively (council 2026-07-04, both members). **Falsifiability lock:** each recording regenerated in CI from a real run so it cannot drift from current behavior. **NOT:** no live code execution; no backend; no generative-skill theatrics orthogonal to the wedge. <!-- prereq DONE 2026-07-05: media-tooling detect-and-instruct gate landed — src/scripts/check_media_deps.ts (detects asciinema + agg, prints platform install cmd, never silent-installs per missing-tool-handling; docker-precedent shape), task check-media-deps, docs/maintainers/optional-media-deps.md, test. Decision (not auto-bundled into base maintainer setup): no-silent-install floor + cross-platform fragility + YAGNI; on-demand prompted install instead. REMAINING: the actual recordings (asciinema present on the maintainer machine → B8 is unblockable) + CI regen-from-real-run + embed on the proof/site page. -->
    - Media tooling: `check_media_deps.ts` + `task check-media-deps` + `docs/maintainers/optional-media-deps.md` — on-demand, never auto-installed.

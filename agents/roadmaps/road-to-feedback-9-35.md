...
|---|---|---|---|---|---|
| 1 | The blocker guard | implementation | 1.3's guard turns every roadmap carrying a long-lived maintainer blocker into an unarchivable roadmap, so the active tree grows instead of the decisions getting made. It fires on this very roadmap, which carries four | Refuse-and-name rather than delete-or-decide, so the cost is visibility not loss; 1.4 measured the real exposure first (3 of 471 archived roadmaps, and the guard cannot even see two of them). If it starts holding roadmaps for months, the shape to revisit is surface-and-acknowledge | Phase 1 — The three defects this span actually created |
| 2 | Half-bound authorization | implementation | 2.1's threat pass produces a binding set large enough to be its own release, and the confirmation store ships half-bound in the meantime — worse than unchanged | The step produces analysis only; no binding field is added here. The store's current state is unchanged, not partially changed, and the plan names the four controls as one landing | Phase 2 — Honour the documented intent (no decision reversed) |
| 3 | Corrections read as dismissal | product | Phase 3 is read as saying the reviews were careless, and the next review is discounted wholesale — including the parts that were right | Each correction states what was right before what was wrong: the frozen coverage level is real, the 12.9% headline was accurate, the `paths:` gap is real, and § R2 of the sweep record stays open | Phase 3 — Corrections to the record (cheap, and they stop re-derivation) |
| 4 | Evidence reads as advocacy | product | 4.1's appended line reads as arguing for the hard block, and the recorded no-guaranteed-red rationale loses by omission rather than on merit | The line states the fact and quotes the existing rationale verbatim as the standing counter-argument; the (a)/(b) wording is left exactly as the blocker already has it | Phase 4 — Route the evidence to the decision that already exists |
| 5 | Editorial fix closes the question | implementation | The heads are curated (1.2) and the mechanism question then feels settled, so `release-head-cadence-decision` is archived unanswered a second time | 1.3's guard now protects the roadmap that actually owns that blocker; 1.2 and 4.1 are deliberately separate steps so an editorial fix cannot discharge a decision | Phase 1 — The three defects this span actually created |

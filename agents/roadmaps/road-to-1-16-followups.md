...
- [x] **2.1.1** Add **one** anchor section near the top of `README.md` (after the elevator pitch, before the Tech-stack table): a 60-second onboarding path with three links — install, first command, where the rules live. **No** new content beyond the three links. Reviewer 1's "do not inflate the README" constraint is hard. **Forcing function** (Anthropic A5): before commit, `[ "$(git diff main -- README.md | grep -c '^+')" -le 15 ] || { echo \"README delta exceeds 15 added lines — abort\"; exit 1; }`. If exceeded, cut content (extra paragraphs, prose around the three links) until it fits, or move overflow to `docs/onboarding.md`. <!-- verified: forcing function 11 ≤ 15 (added lines, ^+ incl. +++ header); section "Start here" with 3 links to #quickstart, #2-minute-demo-implement-ticket, #documentation · README.md · 2026-05-03 -->
- [x] **2.1.2** Move any net-new explanatory prose to `docs/onboarding.md` (create if needed). README delta target: ≤ +15 lines (enforced by 2.1.1 forcing function). <!-- verified: no overflow — anchor stayed at 3 links + lead-in only, no docs/onboarding.md needed · 2026-05-03 -->

### 2.2 Wording precision: agent vs. host agent (F4)

- [x] **2.2.1** Search `README.md`, `AGENTS.md`, `docs/` for the broadened pattern (Anthropic A5): `grep -rEn "the agent (runs|implements|commits|executes|performs|applies|deploys|pushes|merges)" README.md AGENTS.md docs/`. Scope-boundary: only fix instances where "agent" appears **without** "host" modifier in the same sentence and where the literal claim is the host agent's behaviour, not the package's contract. Replace with "the host agent is instructed to…" or equivalent. <!-- verified: 2 hits found and reframed → "the host agent is instructed to…"; post-fix grep returns 0 hits (exit 1) · docs/contracts/implement-ticket-flow.md:136, docs/getting-started.md:170 · 2026-05-03 -->
- [x] **2.2.2** Do **not** rewrite skills/rules — those are the instruction-side; the wording fix is consumer-doc-side only. <!-- verified: scope respected — only docs/contracts/implement-ticket-flow.md and docs/getting-started.md edited; no rule/skill/command files touched · 2026-05-03 -->

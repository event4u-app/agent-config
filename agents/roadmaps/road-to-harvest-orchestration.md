...
- [ ] `road-to-security-hardening` Phase 1 — threat-model + `SECURITY.md` (the
      policy layer every mission trust boundary depends on).
- [ ] `road-to-security-hardening` Phase 2 — workflow-security CI linter.
- [ ] `road-to-security-hardening` Phase 3 — `block-no-verify` git-discipline hook.
- [ ] `road-to-mission-mode` Phase 0 — `no-runtime-boundary` contract +
      ...
      Phase 1 and the verify-loop PoC.
- [ ] `road-to-harvest-small-enhancements` Phase 6 — agent-memory README removal.
      **Independent** — may be done any time in this stage as a quick win; does
      not block anything.

## Phase 2 — Mission validation gate (G1)

- [ ] `road-to-mission-mode` Phase 1 — the ~2-day validation PoC
      (Laravel 10→11 on `/work`, git-as-rollback).
- [ ] **GATE G1 — record the decision** (in the mission-mode Phase 1 decision
...
- [ ] `road-to-mission-mode` Phase 2 — flagship `/mission:upgrade` (single-step,
      breaking-change catalog YAML, size-tier surfaced, provisional-branch,
      gated, never auto-PR).
- [ ] *(only if G1 = gated-autonomous)* `road-to-autonomous-verify-loop` Phase 1
      ...
      if yes → its Phase 2 is unblocked in Phase 4.
...
- [ ] *(only if G2 passed)* `road-to-autonomous-verify-loop` Phase 2 — productize
      the `verify-repair-loop` skill (opt-in, never auto-on).
- [ ] `road-to-harvest-small-enhancements` Phases 1–5 — file-first pattern library
...
- [ ] Phase 1 complete: security-hardening done + `no-runtime-boundary` landed +
      agent-memory removed from README.
- [ ] GATE G1 decision recorded in the mission-mode Phase 1 decision doc; the
      verify-loop branch is explicitly taken or skipped.
- [ ] Flagship `/mission:upgrade` shipped before any Phase 4 expansion.
- [ ] GATE G2 recorded (if reached); verify-loop Phase 2 only after a green G2.
- [ ] Harvest-small Phases 1–5 land last, each as a discrete capped milestone.

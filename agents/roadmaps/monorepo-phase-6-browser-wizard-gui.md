...
- [x] Phase 3 (TS installer) shipped and stable for at least one
      release cycle
- [x] Phase 5 (trust) shipped so the wizard can render the
      advisory-pack confirm prompts correctly
- [x] `dist/discovery/discovery-manifest.json` published with the
      release artefact

## Acceptance criteria

- [x] `npx @event4u/agent-config init --gui` launches a local
      server on a free 127.0.0.1 port, opens the browser, completes
      an install without the user ever touching a terminal
- [x] Wizard reads `dist/discovery/discovery-manifest.json` only;
      no other data source
- [x] Wizard auto-detects `composer.json`, `package.json`,
      `pyproject.toml` and pre-checks suggested packs
- [x] Server shuts down cleanly after the install completes or the
      user closes the tab; no zombie processes
- [x] Wizard works offline (no CDN dependencies; assets bundled)
- [x] All trust banners from Phase 5 render in the GUI exactly as
...
- [x] `packages/core/installer/src/gui/server.ts` boots a small
      Node HTTP server (no Express; stdlib `http`); single static
      asset dir + 3 JSON endpoints
- [x] Endpoints: `GET /api/manifest`, `POST /api/preview`,
      `POST /api/apply`; all backed by the existing installer code
- [x] Server picks a free port, prints the URL, opens the browser
      via `open` package (cross-platform)
- [x] Auto-shutdown after `/api/apply` returns success or after 10
      minutes of inactivity

## Phase 2 — Frontend (vanilla, no framework lock-in)

- [x] `packages/core/installer/src/gui/web/` ships HTML + vanilla
      TypeScript + minimal CSS (no React/Vue/Svelte); compile to a
      single bundled `index.html` via `esbuild`
      _Means changed (ADR-016 / Council verdict): assets inlined as
      TS string constants in `gui/static-assets.ts` instead of an
      esbuild bundle. Zero build step, zero runtime deps,
      ≤ 200 KB budget honoured._
- [x] Three screens: workspace picker, pack picker (filtered by
      workspaces), summary + confirm
- [x] Auto-detect hints come from a `GET /api/auto-detect` call the
      server runs against the consumer CWD
- [x] Trust banners render as inline warnings on the pack picker;
      advisory packs require an in-page checkbox before next

## Phase 3 — Apply flow

- [x] `POST /api/apply` calls the installer in non-interactive mode
...
- [x] Server binds 127.0.0.1 only; rejects requests with a
      non-loopback `Origin` header
- [x] **Host header allowlist** (`127.0.0.1:<port>`, `localhost:<port>`)
      to mitigate DNS-rebinding via form POST that omits `Origin`
- [x] CSRF token issued on `GET /` and required on every POST
- [x] No external network calls; CSP `default-src 'self'`
- [x] `--gui-port=<n>` flag for users who need a fixed port
- [x] `--no-open` flag for headless servers / SSH tunnel users
...
- [x] GUI assets shipped inside the npm tarball; no separate
      download
- [x] Tarball size budget: GUI assets under
      `packages/core/installer/src/gui/` (compiled + inlined) add
      ≤ 200 KB; discovery-manifest is out of scope
- [x] `docs/contracts/gui-wizard.md` documents the architecture, the
      bridge contract, the host+origin allowlist, the rollback log
      shape, and the local-only invariant
- [x] README quick-start: "non-developers — run

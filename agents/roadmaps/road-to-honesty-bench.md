...
- [ ] **Pre-register the design before any data**: arms (baseline `off` /
...
- [ ] **Rebuttal/capitulation test set** (the priority set): elicit a
...
- [ ] **Clean-control set** (over-correction — explicitly not optional):
  correct artifacts reviewed; metric = invented findings per review.
  *Verify:* set committed with ground-truth "no defect" labels.
- [ ] **False-premise set — UNIFIED with the open cross-source eval**: one
...
- [ ] **Scoring infrastructure**: the 5-facet sycophancy judge rubric
  ...
  *Verify:* rubric + logging exist as bench components; judge-bias caveat
  documented.
- [ ] **Run gate**: the first PAID scored run requires the standing
...
- [ ] **This disposition table is the record** — the convergence mapping,

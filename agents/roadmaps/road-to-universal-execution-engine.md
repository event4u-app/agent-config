...
- [ ] **Step 1:** Rewrite `.agent-src.uncompressed/commands/implement-ticket.md` — step 2 ("Prepare the state file") wraps the resolved ticket as `input.kind="ticket"`, `input.data={…}`, calls `python3 -m work_engine`. Step 3 (engine call) uses the new module path.
- [ ] **Step 2:** Verify the command's user-facing contract is unchanged: same numbered options, same halt formats (`@agent-directive:`, `>`), same close-prompt, same error surfacing.
- [ ] **Step 3:** Update `.agent-src.uncompressed/skills/command-routing/SKILL.md` to mention `work_engine` as the dispatched module.

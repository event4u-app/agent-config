...
- [ ] **Step 5 — Push + verify remote CI.** Branch is stacked on `feat/implement-pr150-feedback`. Push only after the user gives explicit per-step approval (per [`commit-policy`](../../.augment/rules/commit-policy.md) — no auto-push). Watch remote CI complete green before opening the PR.

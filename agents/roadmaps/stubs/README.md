...
| [`road-to-batch-elicitation-kernel-delta.md`](road-to-batch-elicitation-kernel-delta.md) | [`road-to-user-out-of-the-loop.md`](../road-to-user-out-of-the-loop.md) Phase 1 Step 3 + blocker `kernel-soak-window`, 2026-08-20 | `transferred` | Two conjuncts in the stub: a RECORDED authorization for the `ask-when-uncertain` batch-elicitation delta, and the >= 24 h `kernel-rule-edits` soak completing without rollback. Measured 2026-08-20: authorization records **0**; `ask-when-uncertain` is row 2 of the locked nine; `block-kernel-rule-writes` bound blocking + `fail_closed: true` |
| [`road-to-roadmap-set-front-doors.md`](road-to-roadmap-set-front-doors.md) | [`road-to-user-out-of-the-loop.md`](../road-to-user-out-of-the-loop.md) Phase 2 Steps 1/5, Phase 3 Steps 2/3, Phase 6 Step 4, 2026-08-20 | `transferred` | Per item, in the stub. Three command surfaces gated on nothing but authoring (measured: `process-backlog`, `merge-train`, `goal` all absent from `src/domains/`); lanes and stacking gated on ten clean serial set runs, measured **0** and unobtainable until the set entry point exists |

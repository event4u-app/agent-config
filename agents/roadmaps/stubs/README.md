...
| `road-to-live-trigger-eval.md` — **shared stub, second parent** <!-- ref-ignore --> | [`road-to-cost-parity-1-rule-payload-diet.md`](../archive/road-to-cost-parity-1-rule-payload-diet.md) blocker `skill-activation-window`: Phase 1.3's skill-usage column, 2.1c in full, 2.5's bar-gated half, 2026-08-20 | `transferred` | **No new stub.** The council merged this blocker with `human-gated-live-trigger-eval` from `road-to-skill-description-measurement.md` as one evidence gap under two names, and ordered ONE shared stub; that stub was written by the pre-registration owner and states in terms that this roadmap's blocker "closes against this file as-is". Its four probes (P1–P4) govern promotion and were all measured FAIL on 2026-08-20. Two facts the parent rested on are refuted and the transfer holds anyway: activation is 5 of 290 over 30 sessions (window-dependent, three competing censuses), and **four** skills declare a machine-matchable trigger, not zero. *Row carries `ref-ignore`: the stub file is still on `origin/drain/road-to-skill-description-measurement` and reaches `main` with that branch.* |

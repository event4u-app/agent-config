...
| [`road-to-dashboard-untrack-cutover.md`](road-to-dashboard-untrack-cutover.md) | [`road-to-merge-hotspot-drawdown.md`](../archive/road-to-merge-hotspot-drawdown.md) step 4.2, 2026-08-21 | `transferred` | 4 probes in the stub, plus one open council guard. P1 `update_roadmap_progress.ts:1337` `--check` must stop reporting an absent dashboard as stale (**fails** — absence is a distinguished case nowhere on that path) · P2 the version-skew kill zone, new-canonical × old-consumer, must be closed (**no version guard exists**; old workflow copies are not auto-updated) · P3 an unstage mechanism must exist (**zero** automated paths — the `gitignore-block.txt` precedent the plan assumed was FALSE and is corrected in the parent's 2.2, and building one reverses a stated user-owned-git-ops design) · P4 the drain-order call (7 of 11 open PRs already `CONFLICTING` on this path, ~5 live sessions on one checkout). Guard 1 discharged by the parent; **guards 2 and 3 are both open** — guard 2 (sync-tool support for an intentional tracked→untracked transition) is the gate, and guard 3 got a measurement of the propagation path where the council asked for compatibility checks, which discharges nothing. Producer: the maintainer as cutover coordinator — P2 breaks consumer CI (owner-reserved), P3 reverses a design, P4 schedules across other people's branches |
...
| [`road-to-batch-elicitation-kernel-delta.md`](road-to-batch-elicitation-kernel-delta.md) | [`road-to-user-out-of-the-loop.md`](../archive/road-to-user-out-of-the-loop.md) Phase 1 Step 3 + blocker `kernel-soak-window`, 2026-08-20 | `transferred` | Two conjuncts in the stub: a RECORDED authorization for the `ask-when-uncertain` batch-elicitation delta, and the >= 24 h `kernel-rule-edits` soak completing without rollback. Measured 2026-08-20: authorization records **0**; `ask-when-uncertain` is row 2 of the locked nine; `block-kernel-rule-writes` bound blocking with `fail_closed: true` |
| [`road-to-roadmap-set-front-doors.md`](road-to-roadmap-set-front-doors.md) | [`road-to-user-out-of-the-loop.md`](../archive/road-to-user-out-of-the-loop.md) Phase 2 Steps 1/5, Phase 3 Steps 2/3, Phase 6 Step 4, 2026-08-20 | `transferred` | Per item, in the stub. Three command surfaces gated on nothing but authoring (measured: `process-backlog`, `merge-train`, `goal` all absent from `src/domains/`); lanes and stacking gated on ten clean serial set runs, measured **0** and unobtainable until the set entry point exists |
| [`road-to-youtube-channel-exercise.md`](road-to-youtube-channel-exercise.md) | [`road-to-gated-reach-followup.md`](../archive/road-to-gated-reach-followup.md) Phase 1 Steps 1-5 + AC 1-4, 2026-08-20 | `transferred` | Two-part probe in the stub, both measured at transfer: `command -v yt-dlp` **fails** (exit 1), and `reach:doctor --channel youtube` readiness **`unknown`** (never evaluated while the backend is absent). Part 2 is already satisfied — `node v26.7.0` on PATH and `--js-runtimes` present in the yt-dlp user config — so the pending act is one `pipx install` by the host owner, not two installs. |

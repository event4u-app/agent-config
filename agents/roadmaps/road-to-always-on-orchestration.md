...
| 6 | Quorum 1-of-2 lets one model wave through what two should judge | product | Majority at n=2 is a single voice | Council-weighed trade-off: the alternative (2-of-2) deadlocks a release gate on any timeout, and inconclusive-holds keeps the human in the loop. Absent members are **artifact-visible only** — `orchestrator.ts` renders `_render_quorum_line` / `_render_absent_members` and `session.ts` serialises the quorum, but `events_log.ts` carries no quorum event, so a solo-concluded pass is downstream-identical to a full-attendance one. Attendance telemetry is proposed in `road-to-inbox-harvest-2026-08-b.md` Phase 1; until it lands, this row claims no telemetry mitigation | Phase 3 |

...
- [~] **T-005 · Gate-tier cross-vendor canary (hardening #4; net-new CI infra).** A single cheap cross-vendor scenario on `gate` (every PR) + the full matrix on `periodic`. Periodic-only would let a vendor model update break parity silently until the next run. <!-- deferred: NO LONGER credential-blocked — needs a cost-bounded design (per-PR paid cross-vendor calls); decide spend budget before wiring. -->

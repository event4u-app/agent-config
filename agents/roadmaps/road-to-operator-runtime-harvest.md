...
- [~] **T-004 · Cross-model smoke (keystone).** `CodexRouter`/`GeminiRouter` on the injectable contract; run existing RDP/trigger/`rubric` evals across vendors 2+3. Emit **per host**: `rdp_pass`, finding-count distribution, **and an output-shape fingerprint** (hardening #2 — format-governance signal, instrumented now not deferred). Refuse to assert parity unless `discrimination_ok==true`. <!-- deferred: blocked on vendor credentials — no Gemini key; Anthropic transport ETIMEDOUT. See agents/settings/contexts/cross-model-eval-decision.md -->
- [~] **T-005 · Gate-tier cross-vendor canary (hardening #4; net-new CI infra).** A single cheap cross-vendor scenario on `gate` (every PR) + the full matrix on `periodic`. Periodic-only would let a vendor model update break parity silently until the next run. <!-- deferred: depends on T-004 (vendor credentials). See cross-model-eval-decision.md -->
- [~] **T-006 · Baseline capture + outcome read.** <!-- deferred: depends on T-004 (vendor credentials). See cross-model-eval-decision.md --> Run once locally (`verify-before-complete`); capture per-host metrics + cost + `discrimination_ok`. **Report the coverage denominator (3rd-pass honesty-coupling): "parity proven on N of 252 skills (trigger) / M behavioral"** — and the repositioning (Phase 3) may claim *exactly that fraction*, never whole-suite consistency. Read against three outcomes (council): **(a)** all pass RDP **and** the *graded* `discrimination_ok=true` (subtle case caught) → overlays stay shelved, honest-null is the **final** word *for the covered slice*; **(b)** a host fails RDP (and T-000 shows it is a behavior gap, not a capability gap) → open Phase G for *that host only*; **(c)** all pass but output-shapes diverge → open Phase 0b. If `discrimination_ok=false`, outcome (a) is **invalid** (underpowered) → fix the control before reading any null.

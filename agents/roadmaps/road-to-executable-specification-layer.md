...
estate_growth_exempt: "Charges +1 on open_blockers in THIS change, and the rise is a CORRECTION rather than growth: the council (2/2, 2026-08-27) rejected this file for shipping status:ready across three phases while its own blocker recommended Phase 1 only, so the added `phase-2-and-3-need-demand-not-only-a-stack` blocker records a gate that was already true and undeclared. The gate has no field for that distinction. The original count claim stands unchanged on its measurement: `grep -rniIE 'gherkin|cucumber|behat|given.when.then' src/` returns exactly ONE hit across 299 skills, and that hit is a rubric line telling a reviewer acceptance criteria should be Given-When-Then — the suite grades the shape and teaches nobody to execute it. Companion: zero of 299 skills mention mutation testing while `grade_target_readiness.ts:183-189` grades consumer repos on exactly that dimension."

...
- [x] **1.3 Strike the false comment in the Cursor trampoline.** `src/scripts/hooks/cursor-dispatcher.sh:23-26` states "none of our concerns block"; the manifest carries **seven** `severity: blocking` concerns. Corrected 2026-09-06: this line read "eight", which is what `grep -c "severity: blocking"` returns because `src/scripts/hook_manifest.yaml:510` is a COMMENT about the key, not a concern declaring it. A count taken by grep over a file whose comments quote the key is the defect class this roadmap corrects elsewhere, reproduced in its own step.
      verify: the comment names the actual state (concerns that would block are not bound on this host) and a YAML parse of `src/scripts/hook_manifest.yaml` — never `grep -c`, which counts the comment at `:510` — yields the number the comment cites.
- [x] **1.4 Make the install smoke probe read the constant the installer writes.** `src/scripts/install.ts:1730` probes `['cursor','beforeShellExecution']`, an event `CURSOR_DISPATCHER_BINDINGS` (`:1133-1139`) never writes, so the probe cannot fail on a real regression.
...
- **Status:** resolved
- **Outcome:** narrowed — the roadmap dependency is closed, the substantive
  question is not. AI council, 2 seats, 2026-09-06, under the maintainer's
  standing delegation, dispositioned this as a bounded observation: run the deny
  probe if a real Copilot installation is available, otherwise record the
  unavailability and defer, and never write unavailable access up as
...
- **Reopen when:** a Copilot installation is reachable. The probe is unchanged and the row is ready for it; only the observation is missing.
...
- **Status:** resolved
- **Outcome:** narrowed — the roadmap dependency is closed, the substantive
  question is not. Same council disposition as the blocker above: probe if the
  host is available, otherwise retain `verified: null` and record the
  unavailability. Gemini CLI **0.50.0 is installed**, so the probe was built and
  attempted on 2026-09-06 in an isolated scratch workspace (never a real
  repository, never `~/.gemini`): a project-scope `.gemini/settings.json` binding
  a `BeforeTool` hook that always denies, and a prompt asking the agent to write
  a marker file. It could not run. The CLI aborts with `IneligibleTierError:
  This client is no longer supported for Gemini Code Assist for individuals`
  before any turn begins, so no tool call and therefore no `BeforeTool` hook is
  reachable. Reproduced twice, in the control and deny workspaces. **The marker
  file was absent for the authentication reason, not the deny reason**, which is
  exactly why this is not written up as a passing probe. `verified: null` is
  retained for the gemini rows in `host_lowering.yaml`, and the attempt and its
  failure mode are recorded there in the row's own comment.
- **Owner:** maintainer
- **Blocks:** nothing in this roadmap — it gates work deliberately excluded from it.
...
- **Reopen when:** a Gemini CLI installation can complete one turn — an eligible account, an API-key transport, or the successor product the error names. The probe workspace is described above and is cheap to rebuild.
...
- [x] AC-1 — No file under `src/` or `docs/` asserts that a host lacks a hook capability; every such statement is either a package-non-binding statement or a table row carrying a citation and an expiry.
- [x] AC-2 — `agent-config hooks:status` names no retired file path and no path that does not resolve from the repository root.
- [x] AC-3 — `src/scripts/hooks/host_lowering.yaml` is the only place a host's block-exit, emission shape and fail policy are written, and `grep -rn "_DISPATCHER_BINDINGS" src/` returns zero hits.
- [x] AC-4 — Every generated host bridge is byte-identical to its pre-change golden file.
- [x] AC-5 — A host-capability row past its `expires` cannot carry a blocking binding, and the lint that enforces this fails on a fixture row and passes at HEAD.
- [x] AC-6 — The install smoke probe fails when an entry is removed from the constant that writes the bridge.
- [x] AC-7 — No host gained a blocking binding in this roadmap; `VERIFIED_PLATFORMS` still resolves to the same single host it resolved to at 93d63073e.

...
- [x] **A3: Plan builder.** Port `_build_install_plan` — walks `.agent-src/` (or `.agent-src.uncompressed/` for the package self-install), produces a list of `FileEntry` with target paths + content hashes. Plan is **declarative** — apply step never recomputes paths. Coverage: golden-snapshot test against a fixture pack.

...
- [ ] **D1: Move legacy out of import paths.** `scripts/install.py` → `bin/install.legacy.py.bak` (kept for one release as reference; **not** invoked by any code path). `packages/core/installer/` workspace deleted entirely; `package.json` workspaces config narrows.
- [ ] **D2: Cleanup references.** Remove `installer gui` from `agent-config --help`, README, AGENTS.md emergency-triage block. Search for `installer/` / `install.py` references across docs; update or delete.
- [ ] **D3: Reduce conflict-policy YAML to `default_strategy` only.** `agents/settings/.agent-conflict-policy.yml` shrinks from ~300 lines to ~50.

## Phase E: v4.0.0 release (~0.5 working days)

- [ ] **E1: CHANGELOG + v3-detection copy.** Hard-cut entry — "Breaking: legacy Python installer removed; re-run `npx @event4u/agent-config install` to migrate. No auto-upgrade path." **Cross-link to backup-screen behaviour (council Finding #21):** CHANGELOG names the `~/.event4u/agent-config.v3.bak/` backup path so users on 3.x see the exact restore command before they hit the wizard.
- [ ] **E2: Version-detection + backup screen wired.** In `src/install/detect.ts` (Phase A2) add `detectLegacyV3()` reading `~/.event4u/agent-config/VERSION`. In the wizard (Phase B), wire a pre-Step-1 `BackupScreen.tsx` that fires only when v3 is detected. Server route `POST /api/install/backup-v3` copies `~/.event4u/agent-config/` → `~/.event4u/agent-config.v3.bak/` atomically (rsync-style hardlink where supported, fall back to copy). Coverage: detection unit-test + backup integration test against a v3 fixture tree.
- [ ] **E3: Version bumps.** `package.json` → `4.0.0`. Lockfile refresh. Tag `v4.0.0`.
- [ ] **E4: PR description + merge.** Single PR off `fix/broken-installation`, squash-merge-as-series so each phase commit is bisect-able. **Human-owner gate:** merge to `main` requires explicit user permission per `commit-policy` + `non-destructive-by-default`.

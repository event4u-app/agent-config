...
|------|------|-----------|-------------|------------|----------------|
| 1 | The label path is never exercised at all | product | Every release so far went another way — 14.0.0 by `workflow_dispatch`, 14.1.0 through 14.6.0 by local `task release`. A path that works and is never used stays unverified forever, and the transfer quietly becomes permanent. | The transfer costs the producer nothing extra: it is one label on a PR they already merge, not a separate task. Its probe is a single command over live state with a baseline of 0, so drift from "not yet" to "never" is visible rather than assumed. | Phase 2, Outcome |
| 2 | A labeled merge publishes unattended | implementation | The criterion promised a manual approval click as the human gate. Measured on 14.0.0 (`release.yml:26-34`), no approval was requested and the run would have merged, tagged and published without a human touching it. Anyone labeling a PR on the strength of the old sentence would be firing an unattended publish. | The Outcome section and the stub's transfer 1 both record the falsification and invert the default — treat a labeled merge as unattended unless the approval gate has been verified on this repository first, with branch protection named as the reliable alternative. | Phase 2, Outcome |
| 3 | The re-specified collision drill is wrong | implementation | The re-spec asserts from a code read that two release PRs can only stack when the two entry points compute different targets, and that nothing guards it. That is an inference over `release.ts`, not an observation — no such state has ever existed to check it against. | The re-spec is written to decide rather than to confirm: its step 3 constructs the stacking case deliberately, and a confirmed hazard routes to a new roadmap instead of a `preflight` change made blind. The three reasons the original could not pass are line citations, which is checkable without a release. | Phase 2 |
| 4 | Drill cases (b) and (c) were discharged too readily | implementation | Both rest on one run. A single convergence shows the resume probes worked once on one shape of half-finished release, not that they are sound across shapes. | The run id is recorded so the log can be re-read rather than trusted, its ten step lines are quoted verbatim in the evidence artefact, and `task release:drill` keeps seven within-run failure modes green in CI continuously. Case (a), the one the mechanism is documented not to cover, was deliberately not discharged. | Phase 2, Outcome |
| 5 | The dashboard percentage is read as achievement | product | Six of eight items closed as `[-]`, which renders as complete. A later reader could conclude the label path was verified when no release has ever taken it. | The Outcome section leads with the statement that archived does not mean achieved, names the release-gated half of every transferred item, and links the stub that holds them. The stubs README row repeats the zero-of-300 baseline. | Outcome, Acceptance Criteria |

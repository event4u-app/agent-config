...
- [ ] Redaction adversarial spot-check BEFORE the flip: exercise
...
- [ ] Flip `knowledge.global_sharing.enabled` default to `true` in
...
- [ ] Pre-register the demotion trigger in the superseding ADR: any
...
- [ ] Write the superseding ADR (next free number): supersedes ADR-103,
...
- [ ] Downstream sync: settings schema default, install bundle regen
...
- [ ] Verify the soak evidence: confirm the deprecation signal has been
      published in at least one released manifest and no consumer breakage
      report exists (issues + PR feedback sweep).
- [ ] Update ADR-092 with a dated addendum: readers-now-TS fact, deprecation
...
- [ ] Fix the stale `.py` reader list inside ADR-092 (now `.ts` paths).
...
- [ ] ADR-093 → `status: superseded` (superseded by ADR-104; chain already
      recorded everywhere except the status field).
- [ ] ADR-028 → `status: superseded` (superseded by ADR-045).
- [ ] Reconcile ADR-105 ↔ ADR-117: ADR-117 claims `supersedes: 105` while
...
- [ ] ADR-098: carry the "Decision-10 only" partial-supersession qualifier
      into the INDEX rendering so it stops reading as a clean supersession.
- [ ] Add missing `status:`/`date:` frontmatter to the 8 unclassifiable
      ADRs (002, 003, 038, 114, `ADR-rule-kernel-and-router`, and the other
      blanks the INDEX shows), then regenerate the index.
- [ ] ADR-200: the provisional-number condition has fired (migration landed
...
- [ ] Annotate ADR-006 (skill-tools Python pilot) as historical —
      superseded by the completed migration.
- [ ] ADR-116 (memory tripwire, FTS5): mark superseded/amended — the
...
- [ ] `agents/roadmaps/road-to-orchestration-scope-decision.md` (~line 42)
...
- [ ] Fix the three stale `.py` path references recorded in
...
- [ ] Fix the `.py` script table in the

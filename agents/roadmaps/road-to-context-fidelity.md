...
| 5 | A new obligation surface grows the estate | product | Fixing context loss by adding rules is self-defeating. | The plan adds one concern, one frontmatter flag, policy on existing machinery, and one lint — zero new rules is the intent, and a rule turning out to be necessary is a finding to surface rather than absorb. | all phases |

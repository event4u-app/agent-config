...
- [x] **Step 1:** Port or retire the remaining pytest infrastructure: every remaining `tests/**/*.py` either has a vitest equivalent (ported in its cluster's phase) or a documented retirement note; delete the pytest suite, `conftest.py`, `pyproject.toml`, and pytest/xdist from CI workflows. The diff removes >5 files by design — surface the full deletion list for explicit confirmation per `non-destructive-by-default` before the deletion lands. <!-- done: retired tests/test_frontmatter_strict_yaml.py (ported to validate_frontmatter.test.ts) + 4 dead concern_*.py fixtures (concern_allow.ts is the live twin); no conftest/pyproject/xdist remained. Deletion list in agents/evidence/migration-final-report.md. -->
- [x] **Step 2:** Remove the dispatcher's Python fallback branch (keep the dispatcher itself — it is now a plain TS runner) or inline it away; sweep taskfiles/workflows/docs for any residual `python3` mention. <!-- done: dispatcher (scripts-run → run.ts) already pure-tsx, no fallback; swept python3 from taskfiles/dev.yml + docs/architecture.md. -->
- [x] **Step 3:** Delete `.venv` / `.venv-mcp` bootstrap logic, Python references in `.github/workflows` setup steps, and the Python sections of contributor docs; update `CLAUDE.md`/`AGENTS.md` emergency triage and `docs/architecture.md`. <!-- done: no .venv/setup-python remained in workflows; removed 3 migration-scaffolding workflows (py2ts-base-guard/drift/main-sync), kept permanent no-python-in-src.yml; retired the python2ts addendum in branch-protection-policy.md; updated docs/architecture.md. CLAUDE.md/AGENTS.md had no Python-era triage to change. -->
- [x] **Step 4:** Final audit: repo-wide `find` proves zero tracked `.py` files outside explicitly documented carve-outs (none expected); migration dashboard reports 100%; archive the dashboard snapshot into `agents/evidence/`. <!-- done: git ls-files '*.py' → 3 documented carve-outs (internal bench/eval corpora + glama smoke); migration-status.md regenerated → 100.0%. -->
- [x] **Step 5:** Close the loop on quality: regenerate the baseline comparison (linter finding counts, CI runtime, coverage) vs the pre-migration snapshot and record the final report (including a CI-runtime before/after) in `agents/evidence/migration-final-report.md`. <!-- done: agents/evidence/migration-final-report.md written. -->

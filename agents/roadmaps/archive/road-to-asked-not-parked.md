...
- [x] **3.3 Oblige a resuming session to put `HANDOFF.md` `## Open questions` to the user, one at a time, before it works.** The section at `src/domains/meta/agent-handoff/command.md:171-173` carries a must-not-drop obligation, and the linter at `:181-185` only checks the section is non-blank — so a `?`-terminated question satisfies every gate while never reaching the user. Answers move into `## Decisions`.
      ...
      SURFACE RETIRED AFTER CLOSURE (recorded 2026-09-07, on merging `origin/main`): step 3.3 shipped and was verified against the `2b. File-artifact mode` section of `src/domains/meta/agent-handoff/command.md`, and `origin/main` then deleted that whole section — `HANDOFF.md` was retired at ffd8dd754 (road-to-one-continuity-record 1.1 + 4.1b) on the reproduced finding that the artifact had no producer and no reachable consumer, the `lint_handoffs.ts` artifact-validation block this step cited at `:181-185` going with it. The obligation is therefore MOOT on that surface rather than reversed, and it was not re-added: resurrecting a section main deleted on evidence would trade a verified deletion for an obligation on an artifact nothing writes. The same discipline survives on the two live surfaces this roadmap also shipped it to — `roadmap-process-loop.md` (Ask before park) and `templates/features.md` (step 3.4) — plus the `one-question-per-ask` guard from Phase 5.1. Nothing from step 3.3 is claimed as still standing on the handoff surface.

...
- [x] **Step 1:** Author the **Execution-Model ADR** (next free number, ~ADR-040)
...
- [x] **Step 2:** Author the **validation-gate context note** under
...
- [x] **Step 3:** Author the **rollback-criteria note** (can live in the same
...
- [x] **Step 4:** Rewrite the `README.md` hero (lines ~1–14) experience-first:
...
- [x] **Step 5:** Move "Cinematic AI video" out of the hero into a clearly
...
- [x] **Step 6:** Update the `check_command_count_messaging.py` expectations +
...
- [x] **Step 7:** Add a profile/experience selection step to the install wizard
...
- [x] **Step 8:** Wire `npx agent-config use --profile=<id>` (and surface it in
...
- [x] Execution-Model ADR merged; `README.md:135` "skill-resolver out of scope"
  superseded; runtime resolver explicitly scoped out of 6.0.0 (conditional).
- [x] Validation-gate + rollback-criteria context note filed under
  `agents/settings/contexts/`.
- [x] README hero is experience-first; AI-video relocated to a Creative-Pack
  callout; `check_command_count_messaging` green.
- [x] Install wizard asks profile/experience first; `agent-config use
  --profile=<id>` writes the setting (no projection change yet).
- [x] No projection-behaviour change shipped in this roadmap (that is 6.0.0-B);

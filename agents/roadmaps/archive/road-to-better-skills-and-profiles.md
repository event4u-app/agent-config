...
| **Context-first pattern** | **Reuse existing `agents/settings/contexts/`** as the equivalent surface. No new `<domain>-context.md` files. Document the equivalence in the skill template. |
...
- [x] **A7** — `subagent-orchestration` mode 7 `do-in-worktrees` extended with **competitive variant** (per-candidate isolation, no auto-merge, ranked presentation, loser worktrees stay). [`ADR-005`](../../docs/decisions/ADR-005-subagent-worktrees.md) records the Hard-Floor reasoning (`non-destructive-by-default` § Hard Floor stands; standing autonomy never lifts). Cites existing `using-git-worktrees` for mechanics; modes 1–6 unchanged. Council iter-1 CC-OQ1 verdict (a) honoured.
...
- [x] **D** — Pilot evaluated (block marker; flips after D5 verdict, regardless of pass/fail). Starts after A. **Outcome 2026-05-09: PASS (3/3 tools, see ADR-006).**
- [x] **D1** — `scripts/lint_skill_tools.py` linter: stdlib-only check, `--help` + `--json` required, ≤ 200 LOC cap, `snake_case_verb_noun.py` naming, embedded sample data. Wired into `task ci` via `taskfiles/ci-fast.yml::lint-skill-tools`.
- [x] **D2** — `score_skill_relevance.py` shipped (council iter-1 D-OQ1 verdict (b) — discovery story tool 1): inputs = task description + `skills/` dir → outputs = ranked skill list with relevance scores 0–100, using keyword overlap + persona-match heuristic. 120–180 LOC, stdlib-only, mandatory test coverage. **Eval:** 10/10 (100%) blind tasks have an `expected_top3` skill in the actual top-3, ≥ 85 % target met.
- [x] **D3** — `audit_persona_coverage.py` shipped (D-OQ1 tool 2): inputs = `skills/` + `personas/` dirs → outputs = matrix showing citation count per persona across all skills, **tier-aware** thresholds (specialist < 3, core < 5; council iter-1 D-OQ4 verdict). 80–120 LOC, stdlib-only, mandatory test coverage. **Eval:** 5 under-cited personas surfaced (`ai-agent`, `developer`, `eloquent-tamer`, `qa`, `stakeholder`); ≥ 2 target met.
- [x] **D4** — `suggest_skill_for_task.py` shipped (D-OQ1 tool 3): CLI wrapper combining D2 + persona-skill matrix → outputs top-N skill+persona combos with one-line justification. 60–100 LOC, stdlib-only, mandatory test coverage. **Eval:** 3/5 blind suggestions match human picks (B01 `livewire` vs `livewire-architect`, B03 `description-assist` partial-overlap miss); ≥ 3/5 target met at floor.
- [x] **D5** — Eval gate executed 2026-05-09 via `scripts/skill_tools/run_block_d_eval.py` (corpora at `agents/evidence/eval-corpora/block-d/`, report at `agents/evidence/eval-corpora/block-d/report.json`): **3/3 tools passed**, well above the ≥ 2/3 floor. Outcome documented in [`ADR-006`](../../docs/decisions/ADR-006-skill-tools-python-pilot.md) — pilot **PASS**, tools kept under `scripts/skill_tools/`, runner kept as regression baseline, v2 refinements (ratio-mode, token-frequency reweighting, do-competitively suggester) deferred per ADR-006.
...
| Context-first skill pattern (`<domain>-context.md`) | Benchmark | low (single source — but novel) | Skill checks for project context file before asking. 5 named domains (company / marketing / project / product / regulatory). Maps cleanly to existing `agents/settings/contexts/` directory. Cheap upgrade to skill spine. |

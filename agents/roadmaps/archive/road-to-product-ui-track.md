...
- [x] **Step 3:** Author `.agent-src.uncompressed/rules/ui-audit-gate.md` — always-on rule. Triggers on UI-intent work. Enforces the gate at the rule layer too (defense in depth) — refuses to write components without audit findings in state. Allow-lists `directive_set="ui-trivial"` runs explicitly.

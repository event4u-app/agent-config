...
- [x] Read `.agent-src.uncondensed/skills/fe-design/SKILL.md` — current frontend-design skill (will migrate)
- [x] Read `.agent-src.uncondensed/skills/blade-ui/SKILL.md`, `livewire/SKILL.md`, `flux/SKILL.md` — current Laravel-stack UI skills
- [x] Read `agents/settings/contexts/implement-ticket-flow.md` — updated with R1 + R2 changes
...
.agent-src.uncondensed/
  ...
  rules/
    ui-audit-gate.md           ← always-on, enforces audit gate
```
...
- [x] **Step 3:** Author `.agent-src.uncondensed/rules/ui-audit-gate.md` — always-on rule. Triggers on UI-intent work. Enforces the gate at the rule layer too (defense in depth) — refuses to write components without audit findings in state. Allow-lists `directive_set="ui-trivial"` runs explicitly.
...
- [x] **Step 2:** Add UI-flow goldens (8/12 landed in this iteration; GT-U5..U8 deferred to **[road-to-product-ui-track-followup.md](road-to-product-ui-track-followup.md)** — they cover orthogonal contracts (mixed orchestration, stack dispatch, trivial path, reclassification) whose engine primitives need verification before goldens can pin them):
  ...
  - **GT-U2 — ui-improve via diff:** existing-screen diff input → audit → design halt → apply → review → polish _Landed: 6-cycle recipe `tests/golden/sandbox/recipes/gt_u2_improve_diff.py` + diff fixture `tests/golden/sandbox/diffs/gt-u2-improve-diff.diff`; CLI `--diff-file` / `--file-file` flags wired through `cli.py`, `runner.py`, `harness.py`, `capture.py`; selective `capture --scenarios <id>` no longer wipes the rest of the suite (preserves other baselines + merges summary). Baseline locked under `tests/golden/baseline/GT-U2/`; 11/11 replays green._
  - **GT-U3 — audit-skipped rejection:** dispatcher rejects design/apply when `state.ui_audit` empty _Landed: recipe `tests/golden/sandbox/recipes/gt_u3_audit_skipped.py` pins the dispatcher's hard-gate behavior; baseline locked under `tests/golden/baseline/GT-U3/`._
  - **GT-U4 — polish-loop ceiling:** review finds gaps in 3 consecutive rounds; engine halts after round 2 _Landed: recipe `tests/golden/sandbox/recipes/gt_u4_polish_ceiling.py` pins the polish ceiling at round 2; baseline locked under `tests/golden/baseline/GT-U4/`._
  - [-] **GT-U5 — mixed flow:** prompt implying backend + UI → contract halt → ui track → stitch halt → delivery report _Deferred to follow-up roadmap: requires mixed-orchestration contract+stitch fixtures whose engine primitives need verification first._
  - [-] **GT-U6 — stack dispatch:** same prompt against blade-livewire-flux fixture and react-shadcn fixture; dispatcher routes to different apply skills _Deferred to follow-up roadmap: needs parametrized stack-detection fixtures._
  - [-] **GT-U7 — trivial path happy:** prompt "make the Save button red" → `ui-trivial` → apply only → one-line delivery _Deferred to follow-up roadmap: requires `ui-trivial` directive set verification in dispatcher._
  - [-] **GT-U8 — trivial path reclassification:** prompt that *looks* trivial but apply detects 2-file edit → reclassify to `ui-improve` → audit gate engages _Deferred to follow-up roadmap: requires reclassification primitive in apply._
  - **GT-U9 — greenfield audit halt:** empty repo + ui-build prompt → audit detects greenfield → numbered-options halt → user picks "scaffold tokens" → audit records decision → design proceeds → apply scaffolds tokens before component _Landed: recipe `tests/golden/sandbox/recipes/gt_u9_greenfield_scaffold.py` covers the greenfield numbered-options halt → scaffold pick → audit decision recorded; baseline locked under `tests/golden/baseline/GT-U9/`._
  - **GT-U10 — greenfield bare-proceed:** same as GT-U9 but user picks "proceed bare" → no scaffolding → apply uses Tailwind defaults → delivery report flags greenfield-bare for follow-up _Landed: recipe `tests/golden/sandbox/recipes/gt_u10_greenfield_bare.py` covers the bare-proceed branch; baseline locked under `tests/golden/baseline/GT-U10/`._
  - **GT-U11 — high-confidence happy-path halt-count:** clear ui-build prompt + audit finds strong match → exactly 1 halt (design-brief with folded audit-findings) → apply → review → polish → delivery _Landed: recipe `tests/golden/sandbox/recipes/gt_u11_high_confidence.py` uses the new `seed_state` runner hook to pin **exactly 1 halt** (design-brief sign-off); `audit_path = "high_confidence"` recorded deterministically by `_decide_path`; baseline locked under `tests/golden/baseline/GT-U11/`._
  - **GT-U12 — ambiguous halt-count:** vague-but-classified ui-improve prompt → audit finds 3 candidates → 1 halt (audit decision) + 1 halt (design summary) = 2 halts max → continues _Landed: recipe `tests/golden/sandbox/recipes/gt_u12_ambiguous.py` seeds a populated audit with `confidence.band = "medium"` plus 3 close candidates so `_decide_path` resolves to `"ambiguous"`; pins **exactly 2 halts** (audit candidate-pick + design sign-off); baseline locked under `tests/golden/baseline/GT-U12/`._
- [x] **Step 3:** Wire GT-U1..GT-U12 into `task ci`. R1 + R2 + R3 goldens are now all required. _Landed: `task ci → golden-replay → pytest tests/golden/test_replay.py` auto-discovers every `tests/golden/baseline/GT-*` directory (parametrized over `BASELINE_ROOT.iterdir()`); 17 scenarios collected and passing — GT-1..5, GT-P1..P4, GT-U1..U4, GT-U9..U12. No Taskfile change needed; new baselines are picked up automatically on capture._
- [x] **Step 4:** Document the UI-track contract in `agents/settings/contexts/ui-track-flow.md` (new) — audit gate, design-review loop, polish ceiling, stack-dispatch table, halt-budget rules, trivial-path preconditions. _Landed: [`agents/settings/contexts/ui-track-flow.md`](../contexts/ui-track-flow.md) covers slot-by-slot wiring for `ui` / `ui-trivial` / `mixed`, the audit-path table (`STRONG_SIMILARITY=0.7`, `TIE_GAP=0.05`, `TESTED_AGAINST_SHADCN_MAJOR=2`), design-brief lock + placeholder patterns, stack-dispatch tables for apply / review / polish, `POLISH_CEILING=2` + `TOKEN_REPEAT_THRESHOLD=2`, trivial preconditions (`MAX_FILES=1`, `MAX_LINES_CHANGED=5`), mixed sentinels (`contract_confirmed`, `review_clean`, stitch verdict), idempotency table, declared-ambiguity inventory across all eight directives._
...
- [x] **Step 3:** ADR `agents/settings/contexts/adr-product-ui-track.md` — rationale, audit-as-hard-gate, design-review loop ceiling, halt-budget rationale, trivial-path-and-reclassification, stack-detection strategy, fe-design migration path.
- [x] **Step 4:** Extension recipe `agents/settings/contexts/ui-stack-extension.md` — how to add a new stack (Svelte, SolidJS, etc.): detector heuristic, apply-skill, polish-skill, golden fixture, version-anchor convention.

...
> **Status: complete `2026-05-01`.** Hold-open condition satisfied — all four sibling roadmaps archived (`road-to-context-aware-command-suggestion`, `road-to-product-ui-track`, `road-to-product-ui-track-followup`, `road-to-artifact-engagement-telemetry`, `road-to-visual-review-loop`). Final cloud-compatibility audit re-ran clean: **304 artefacts (T1=156, T2=106, T3-S=42, T3-H=0)**, 11 flagged-by-cloud-action artefacts, **0 unmarked**. Manual platform smokes (AC #4, Phase 1 Step 7, Phase 3 Step 5, Phase 4 Step 5, Phase 5 Step 4, Phase 6 Steps 2–4) remain `[~]` deferred — gated on user-driven UI walkthroughs against Claude.ai Web / Linear AI; the engineering surface is locked.
...
- [ ] Read an internal (local-only) reference analysis — prior reference analysis (mentions Plugin Marketplace as adoption candidate)
...
- [x] **Step 1:** Define the inclusion list — every T1 rule + every T2 rule whose substance survives without filesystem (e.g. `scope-control` ✅, `language-and-tone` ✅, `chat-history` ❌, `cli-output-handling` ⚠️ partial). Capture the decision per rule in [`agents/settings/contexts/linear-ai-rules-inclusion.md`](../contexts/linear-ai-rules-inclusion.md).
...
- [x] **Step 3:** Decide the three-layer split — workspace = universal coding posture, team = framework-specific (Laravel, etc.), personal = empty by default. Document in [`agents/settings/contexts/linear-ai-three-layers.md`](../contexts/linear-ai-three-layers.md).
...
- [~] At least one bundle and the Linear digest verified manually on the live platform (screenshots in `agents/runtime/reports/`) — deferred; same hold pattern as Phase 1 Step 7 / Phase 3 Step 5 / Phase 4 Step 5 / Phase 5 Step 4 / Phase 6 Steps 2–4. Engineering surface locked, manual UI walkthrough gated on user action.
...
- [x] **Final re-check after sibling roadmaps archived** — verified `2026-05-01`: all four sibling roadmaps archived (`road-to-context-aware-command-suggestion`, `road-to-product-ui-track` + `-followup`, `road-to-artifact-engagement-telemetry`, `road-to-visual-review-loop`). `python3 scripts/audit_cloud_compatibility.py` → **304 artefacts** (T1=156 · T2=106 · T3-S=42 · T3-H=0). 11 flagged-by-cloud-action artefacts (`edits`=6 · `runs-task`=5), **0 unmarked** — every flagged artefact carries an explicit `<!-- cloud_safe: noop -->` or `<!-- cloud_safe: degrade -->` declaration. Post-drain delta from Phase 4 baseline: total grew from 292 → 304 (+12 from sibling roadmaps), unmarked-flagged stayed at 0.

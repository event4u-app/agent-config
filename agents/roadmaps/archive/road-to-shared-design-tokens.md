...
- [ ] S0.1 — **Does the orange accent survive AC's palette?** AC's tokens
...
- [ ] Create `tokens/event4u-agent-tokens.json` — a **W3C Design Tokens
...
- [ ] Home it here in agent-config — it owns the W3C-draft naming and the
      more complete ladder. A third repo would be a new artefact to
      maintain for a two-consumer problem; rejected unless a third
      consumer appears.
- [ ] Version it explicitly — a top-level `_version` field starting at
...
- [ ] `tokens/build-ac.mjs` → emits `src/ui/tokens.css` (hex, W3C names).
      AC's file becomes generated; its extensive header comment moves into
      the generator so the rationale survives.
- [ ] `tokens/build-as.mjs` → emits the shadcn `:root` block for
...
- [ ] Both run at build time. **Neither GUI gains a runtime dependency**,
      and AC's "no Tailwind, no CSS-in-JS" stance is untouched.
- [ ] A CI check in both repos fails if the committed CSS differs from the
...
- [ ] Define the query contract AS uses when framing AC (paired with
      `road-to-ac-embeddable-gui.md` Phase 2):
      `?embed=1&theme=<light|dark>` and an optional accent override.
- [ ] AC honours it **at boot, before first paint** — the pre-paint
...
- [ ] AS follows the OS theme (`resolveTheme` with `system`), so the
...
- [ ] **Accent override is bounded — and post-brand-decision.** Embed v1
...
- [ ] **One source, two generated outputs**, both CI-verified against
      drift.
- [ ] **No shared component library**, no framework change in either repo,
      no new runtime dependency in either GUI.
- [ ] **AA contrast holds** for every accent surface in both GUIs, in both
      themes — verified, not assumed.
- [ ] **No theme flash** when opening the embedded settings view.
- [ ] **Standalone AC is unchanged for non-AS users** apart from the
      accent colour. `?embed=1` is additive; the default surface keeps its
      own nav and chrome.
- [ ] **Honest-null path:** if S0.1 shows one accent cannot serve both

...
- **Cost-tracking dependency.** Token / cost numbers depend on `agents/cost-tracking/sessions.jsonl` (an externally-derived pattern already wired into `bench_run.py`). If `cost-tracking` is off, the report flags `cost: source=unavailable` — does not fail the run.

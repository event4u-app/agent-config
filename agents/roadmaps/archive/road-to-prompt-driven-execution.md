...
- [ ] Read `.agent-src.uncondensed/skills/refine-ticket/SKILL.md` — the existing reconstruction-of-intent pattern
- [ ] Read `.agent-src.uncondensed/rules/ask-when-uncertain.md` — vague-request triggers and Iron Law
- [ ] Read `agents/settings/contexts/implement-ticket-flow.md` — updated with R1 changes
- [ ] Re-read `.agent-src.uncondensed/templates/roadmaps.md`
...
- [x] **Step 3:** Verify name does not collide with existing slash commands in `.agent-src.uncondensed/commands/`. → **Verified: `/work` is unused; no prefix collisions in `commands/`.**
...
- [x] **Step 1:** Author `.agent-src.uncondensed/skills/refine-prompt/SKILL.md`. Procedure: read prompt → extract goal → enumerate explicit constraints → infer reasonable assumptions → generate AC list → score confidence per rubric. → **202 lines / 1097 words; linter passes (condensed-variant warning is expected — `task sync` regenerates in Phase 6). Procedure framed as analysis-before-action; `Output format` section enumerates the 4 envelope fields the dispatcher reads.**
...
- [x] **Step 3:** Update `.agent-src.uncondensed/skills/command-routing/SKILL.md` — register the new command, route prompts and prompt-shaped intents to it. → **Replaced the single-engine paragraph with a 2-row table mapping `/implement-ticket` (kind=ticket) vs `/work` (kind=prompt) plus routing heuristic (free-form goal → `/work`, ticket id present → `/implement-ticket`, both → prefer `/implement-ticket` and pull AC from ticket). Locked the rule that one `.work-state.json` carries one envelope; the engine refuses mid-flight switches. Skill grew from 112 → 128 lines / 808 words (within 300-900 target).**
...
- [x] `refine-prompt` skill produces reconstructed AC + assumptions + confidence score for any prompt → **Skill at `.agent-src.uncondensed/skills/refine-prompt/SKILL.md`; `directives/backend/refine.py::_run_prompt` records `reconstructed_ac`, `assumptions`, and the full `ConfidenceScore` (band, score, dimensions, reasons, ui_intent) onto `state.ticket["confidence"]`.**

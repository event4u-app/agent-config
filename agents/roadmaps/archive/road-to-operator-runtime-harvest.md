...
> **ARCHIVED 2026-06-25 — core complete.** The cross-model keystone (T-000–T-006)
> was built and run live, the Gemini format divergence fixed (evidence-chosen),
> the finding_floor mechanism + first behavioral fixtures shipped, and the T-005
> canary wired. Shipped across PRs #659, #660, #661, #662, #663. The 7 residuals
> below (`[-]` migrated) **were not abandoned** — they need a human decision /
> external trigger / parallel-owned work and are parked, intact, in
> `agents/roadmaps/later/road-to-cross-model-residuals.md`.

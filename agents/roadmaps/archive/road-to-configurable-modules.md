...
- [x] Read [`packages/core/.agent-src.uncondensed/skills/module-management/SKILL.md`](../../packages/core/.agent-src.uncondensed/skills/module-management/SKILL.md) — note the Laravel-only assumption (`app/Modules/`, `App\Modules\…` namespace, `ModuleServiceProvider`).
- [x] Read [`packages/core/.agent-src.uncondensed/commands/module/explore.md`](../../packages/core/.agent-src.uncondensed/commands/module/explore.md) — confirm the existing multi-stack detection table (Laravel HMVC, Symfony DDD, Node monorepo, Python, Go) is the seed for the auto-suggest logic.
- [x] Read [`packages/core/.agent-src.uncondensed/templates/agents/agent-project-settings.example.yml`](../../packages/core/.agent-src.uncondensed/templates/agents/agent-project-settings.example.yml) — locate the insertion point for the new `modules:` block.
- [x] Confirm the source-of-truth contract: every edit lands in `packages/core/.agent-src.uncondensed/`; `.agent-src/`, `.augment/`, and tool projections regenerate via `task sync` + `task generate-tools`.
...
- [x] **Step 1:** Add a `modules:` block to `packages/core/.agent-src.uncondensed/templates/agents/agent-project-settings.example.yml`. Shape:
...
- [x] **Step 1:** Rewrite `packages/core/.agent-src.uncondensed/skills/module-management/SKILL.md` — drop `framework: laravel` from frontmatter, rewrite the "Detection" / "Architecture" sections to read from `modules.root_paths` instead of hard-coding `app/Modules/`. Move the Laravel-specific HMVC details (ModuleServiceProvider, `App\Modules\…` namespace, capital-letter directories) into a clearly-labeled "Laravel HMVC carve-out" section that fires only when `modules.namespace_template` matches the Laravel pattern.
...
- [x] **Step 3:** Add a module-context loader entry point — when the agent enters a module via `/module explore <Name>`, automatically load `{module_root}/{agent_folder}/contexts/*.md` into the conversation context per the existing context-loading conventions in [`.agent-src.uncondensed/commands/context/`](../../packages/core/.agent-src.uncondensed/commands/context/). Skip silently when the folder is absent.

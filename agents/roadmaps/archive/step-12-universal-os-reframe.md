...
- [x] [`step-9-user-types-axis.md`](step-9-user-types-axis.md) closed 2026-05-15 — `user-types/` directory + 7 seed YAMLs + README shipped; `--user-type` flag wired across `install.sh`/`install.py`/`install`; runtime filter live in `scripts/mcp_server/prompts.py`; frontmatter audit (`scripts/audit_user_type_axis.py` + `task lint-user-type-axis`) green at 7 declared / 7 used / 0 orphans; integration tests in `tests/test_install_py.py`. AI-Council 2026-05-15-step12-closure-run2 verdict D1 ACCEPT (both members) with amendment "seed all 7 not 3" (honored).
...
- [~] **Deferred to [`step-14-mcp-runtime-stub.md`](step-14-mcp-runtime-stub.md).** MCP-compatible: command invocable from Claude Desktop without terminal access; uses MCP native input prompts. The CLI prompt path ships now; MCP native prompts depend on the MCP runtime wiring tracked under step-14 (stub authored 2026-05-15 closure run #2; AI-Council 2026-05-15-step12-closure-run2 D3 (c) — tracking stub over annotation).
...
- [x] `rules/domain-safety/` directory created with 12 rules — 4 PII redaction (support, finance), 4 output disclaimers (legal, consulting), 4 data retention / logging (finance, ops) — shipped as flat `domain-safety-*` prefix in `.agent-src.uncondensed/rules/` due to flat-scan constraint in `compile_router.py`
...
- [x] All in-scope work closed; deferred work tracked in [`step-13-non-dev-community-validation.md`](step-13-non-dev-community-validation.md) (external recruits, 90-day window) and [`step-14-mcp-runtime-stub.md`](step-14-mcp-runtime-stub.md) (cross-runtime MCP). step-9 user-types axis infrastructure (directory + 7 seed YAMLs + README) shipped this PR; remaining wiring tracked in [`step-9-user-types-axis.md`](step-9-user-types-axis.md). AI-Council 2026-05-15-step12-closure-run2 D2 ACCEPT (with L153 rewrite amendment honored).
...
- [~] Five non-dev case studies merged in `docs/case-studies/` covering ≥3 distinct user-types — **deferred to [`step-13-non-dev-community-validation.md`](step-13-non-dev-community-validation.md) Phase 3** *(template shipped at [`../../docs/case-studies/_template.md`](../../docs/case-studies/_template.md); real case studies are post-merge external recruits — AI-Council 2026-05-15-step12-closure-run2 D2 ACCEPT)*

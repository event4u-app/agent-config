...
- [x] A mutating no-review session produces exactly one `review_skipped`
      ...
      via tsx rather than asserting on mocks — 47 tests green (36 at the time
      this criterion was first ticked; #1224's transcript-path hardening
      landed 11 more, and the count is restated rather than left stale, since
      a number in an evidence note is a claim like any other), covering every

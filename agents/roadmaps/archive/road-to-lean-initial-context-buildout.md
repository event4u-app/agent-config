...
- [-] **3.4** Skill-composition-graph (parent 3.6): new `requires: [...]` frontmatter field + a test proving the router never ships a parent skill without a sub-skill its body assumes. <!-- ASSESSMENT: no current trigger — no skill today declares a parent/sub dependency that would silently break, so the requires: field + dependency test has no failing case to protect. Build when a composed skill appears. -->
...
The 10 remaining open steps were each already carrying a documented engineering assessment (blocked, ~0-lever, no-trigger, obviated, or user-declined). An AI-council round (claude-sonnet-4-5 + gpt-4o) **converged** on `[-] CANCEL` for all 10 with documented reopen-triggers, and on **archiving now** — no `[~]` deferrals, no follow-up roadmap (build when the trigger fires, not preemptively). Both members agreed the `[-]` vs `[~]` choice hinges on user intent: telemetry is off *by design* and the live-A/B harness was *actively declined*, so these are cancellations-with-reopen-condition, not pending obligations.

Per-step cancellation rationale + reopen-trigger:

| Step | Cancel rationale | Reopen trigger |
|---|---|---|
| **2.2** | Governance hardening, not a token lever; out-of-scope for the "cut budget" mandate. | Next skill-pack ships >5 new personas/commands without cost review. |
| **2.4** | Telemetry off by design; zero usage data exists. | Telemetry enabled + a ≥30-day observation window. |
| **2.5** | ~0 token lever — `measure_rule_budget` strips frontmatter before counting. | Load model changes to include frontmatter in the budget. |
| **2.6** | Regression gate for the cancelled 2.5; moot. | 2.5 reopens. |
| **2.GATE** | Live-A/B harness build declined (active decision, not pending). | User authorizes live-spend benchmarking + defines a safety corpus. |
| **3.2** | Obviated by 3.1 — thin projection already demotes bodies to router-resolved pointers; no token goal remains. | Case-by-case only if a rule body's home becomes a maintainability problem. |
| **3.4** | No failing case — zero skills declare a `requires:` parent/sub dependency. | First composed skill declaring `requires:` appears. |
| **3.5** | Telemetry off by design (same blocker as 2.4); reference-skills-only auto-gen. | Telemetry enabled + observation window. |
| **3.6** | Marginal gain vs maintenance cost beyond existing install-time pack selection. | Install-time pack selection becomes a measured bottleneck. |
| **3.GATE** | Live-A/B harness + thin-arm safety corpus declined/undefined. | Benchmark infrastructure authorized + safety corpus defined. |

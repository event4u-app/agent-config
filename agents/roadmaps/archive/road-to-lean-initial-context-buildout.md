...
- [x] Read `docs/contracts/kernel-membership.md`, `docs/contracts/rule-router.md`, and `docs/decisions/ADR-002-kernel-bucket-overrides.md` before touching any kernel rule (gate for Phase 1 here).
...
- [-] **2.2** Extend cost-profiles to commands / skill-index / guidelines / personas / chat-history (parent 1.0b), but FIRST add a cross-reference coherence audit as a CI gate (no `minimal`-profile artefact references a `balanced`/`full`-only one). <!-- ASSESSMENT: structural (which artefact classes load per cost_profile + a cross-ref coherence gate), NOT an initial-token lever. Real governance work but no token reduction; deprioritised behind the captured levers. -->
- [x] **2.3** Semantic de-dupe (parent 1.0d): extend `audit_overlap.py` from keyword to semantic (≥80–90%) across artefact classes; flag near-duplicates for human-approved merge. <!-- DONE (analysis): first fixed the stale-path bug in audit_auto_rules.py + audit_command_surface.py (same packages/core breakage as iron_law_sha/bench_runner — crashed on the pre-monorepo .agent-src.uncondensed/rules path; repointed via artefact_roots). Then ran audit_overlap: 7 cosine≥0.6 rule pairs flagged, ALL intentionally distinct (cli-output-handling~docker-commands share CLI vocab/different concerns; no-attribution~no-decorative-emojis are siblings; augment-edit-discipline~augment-source-of-truth complementary) — 0 genuine duplicates → no merges, no catalog-shrink lever. Mirrors the command-surface synthesis verdict (overlap = shared domain vocabulary, not redundancy). True-embedding extension deferred (no merge candidates to justify it). -->
...
- [-] **2.5** Frontmatter centralization (parent 1.1): resolve repeated governance defaults (`trust`, `packs`, `workspaces`, `lifecycle`, `install`, boilerplate `triggers`) at compile-time from a shared source; keep only load-bearing per-rule fields inline. Extend the existing schema/compile path — no parallel `manifest.json`. <!-- ASSESSMENT: ~0 token lever. measure_rule_budget strips frontmatter before counting (body-only budget) — so centralising repeated governance frontmatter saves nothing in the enforced budget. Skip unless the load model changes. -->
- [-] **2.6** Regression gate for 2.5 (parent 1.2): `dist/router.json` byte-identical before/after the frontmatter move; assert in CI. <!-- with 2.5 (its router-byte-identical gate) — moot while 2.5 is ~0 lever. -->
- [-] **2.GATE** Phase-exit + LIVE A/B (parent 1.GATE, cost-confirmed first): tokens measurably down; selection-accuracy held vs the 50% baseline. A drop → restore the offending descriptions. <!-- gated on live A/B spend + the realization track; deferred. -->
...
- [x] **3.4** Skill-composition-graph (parent 3.6): new `requires_skills: [...]` frontmatter field + a test proving the build never ships a parent skill without a sub-skill its body assumes. <!-- DONE (2026-06-01): the prior "no current trigger" assessment was WRONG — re-investigation against the real code found two real undeclared composition deps + one latent install bug. Built: (1) `requires_skills` added to scripts/schemas/skill.schema.json (own field, NOT ADR-015's pack-id-typed `requires` — that would have collided and broken the discovery-manifest build); (2) populated refine-ticket→[validate-feature-fit, threat-modeling] and skill-improvement-pipeline→[learning-to-rule-or-skill]; (3) scripts/check_skill_requires.py enforces referential integrity + co-availability (sub-skill ships wherever parent ships, via the pack `requires_hint` transitive closure), wired into ci + ci-strict; (4) FIXED a real latent bug — product-basic (ships refine-ticket) did not declare requires_hint on engineering-base (ships both sub-skills), so installing product-basic alone would silently break refine-ticket's orchestration → added requires_hint: [engineering-base] in config/discovery/packs.yml. Negative-tested: the gate fires on the bug, green once declared. trigger-coverage 26/26 held. -->
...
- [-] **3.6** Dist-time profile pruning (parent 3.8): pre-compiled per-profile builds; guard that kernel + every safety-floor rule ships in EVERY build. <!-- ASSESSMENT: marginal beyond the existing install-time pack selection; pre-compiled per-profile dist builds add maintenance for small gain. Deprioritised. -->
- [-] **3.GATE** Phase-exit + LIVE A/B (parent 3.GATE, cost-confirmed): selection-accuracy holds vs the 50% baseline despite bodies now loading lazily. Block + roll back via the `lean_projection.mode` kill-switch on any regression. <!-- gated on live A/B + a thin-arm/safety corpus the bench lacks; this is the 45k-realization validation track (user declined the harness build). -->

## Disposition — council-resolved (2026-06-01)

The substantive mandate ("cut the always-on token budget without weakening an Iron Law") is **complete**: Phase 1 cleared all 3 kernel-budget breaches (Iron-Law fences SHA-identical), and Phase 3.1 shipped the thin-projection mechanism behind the `lean_projection.mode` kill-switch (rule layer 58,673 → 13,491 GPT tok, **−45,182 / −77%**, trigger-coverage 26/26 held). Phase 2.1/2.3/3.3 banked the secondary wins.

The 10 remaining open steps each carried a documented engineering assessment. An AI-council round (claude-sonnet-4-5 + gpt-4o) first converged on cancelling all 10. The user then directed a deeper re-examination ("check whether you really can't build any of them; token spend is fine; I want the refactoring as good as possible"). Each was re-investigated **against the real code** (file:line evidence), which **overturned one verdict**: **3.4 was genuinely buildable and valuable — it is now BUILT** (`[x]`, see its step note), and the re-investigation surfaced a real latent install bug (product-basic missing its engineering-base dependency) that the new gate now prevents.

The other nine are confirmed not-buildable / not-worth-building, now with sharper, code-grounded reasons (no longer "~0 token" hand-waves). Disposition stands: 3.4 done; the rest `[-] CANCEL` with reopen-triggers; **archive now** — no `[~]` deferrals.

Per-step rationale + reopen-trigger:

| Step | Disposition | Rationale | Reopen trigger |
|---|---|---|---|
| **3.4** | **BUILT** | Real undeclared composition deps (refine-ticket, skill-improvement-pipeline) + a latent cross-pack install bug. `requires_skills` field + `check_skill_requires.py` gate + product-basic→engineering-base `requires_hint` fix. | — |
| **2.2** | cancel | Coherence-gate is **vacuous without profile data**; extending profiles to commands/skill-index/guidelines/personas/chat-history needs an ADR-level data-model decision (which artefacts at which profile) across ~300 files — not autonomously decidable. | An ADR defines profile membership for the non-rule artefact classes. |
| **2.4** | cancel | Telemetry off **by design**; zero usage data exists — impossible regardless of token budget. | Telemetry enabled + a ≥30-day observation window. |
| **2.5** | cancel | **Obviated by the shipped `road-to-abstraction-reduction` system**: `apply_schema_defaults` + `lint-frontmatter-boilerplate` already resolve defaulted fields from a shared (schema) source. The fields 2.5 named (`workspaces`/`packs`/`trust`/`install`) deliberately have **no** default — they are load-bearing per-artefact (e.g. 17 rules are `engineering`, not `agent-config-maintainer`); centralising them would hide a real distinction. Like 3.2-by-3.1. | The load model changes so frontmatter becomes budget-counted AND a sensible shared default exists. |
| **2.6** | cancel | Regression gate for the obviated 2.5; also already covered by the existing `compile_router.py --check` (`check-router`) + condensation-hash gates. | 2.5 reopens. |
| **2.GATE** | cancel | Live-A/B = **real-money spend** on a benchmark harness the user **actively declined** building — Hard Floor, not a token question. | User authorizes live-spend benchmarking + defines a safety corpus. |
| **3.2** | cancel | Obviated by 3.1 for tokens; of 49 non-kernel full-bodied rules, ~45 are correctly-placed pure constraints with no skill home, the ~3-5 candidates are weak, and bulk migration is negative-ROI (~125h vetting). Migrating a constraint risks weakening it. | A specific rule's body demonstrably belongs in a specific skill/guideline (case-by-case). |
| **3.5** | cancel | Telemetry off by design (same hard blocker as 2.4); reference-skills-only auto-gen with a >3% kill-switch. | Telemetry enabled + observation window. |
| **3.6** | cancel | **Net-negative**: 3 pre-compiled per-profile builds = 3 CI test matrices + safety-rule duplication for ~6 KB; install-time pack selection already solves the optional-artefact case better. Building it would worsen maintainability. | Install-time pack selection becomes a measured bottleneck. |
| **3.GATE** | cancel | Live-A/B harness + thin-arm safety corpus declined/undefined (same Hard-Floor blocker as 2.GATE). | Benchmark infrastructure authorized + safety corpus defined. |

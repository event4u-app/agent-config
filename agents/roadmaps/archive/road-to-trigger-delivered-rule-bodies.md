...
- [x] **1.5 Never block.** Bounded `unref()` stdin fallback, fail-open to no
      ...
      observed: Every failure path returns 0 — malformed JSON, empty stdin, missing router, and an unwritable state directory (a FILE planted where the state dir must go: the concern still delivers, rc 2). The one non-zero exit is the host's advisory context channel, the same `severity: advisory` + exit-2 shape `ui-route-nudge` and `code-graph-nudge` already use on `pre_tool_use`; `tests/hooks/concern_block_exit_parity.test.ts` passes. Latency, measured in-process over 200 iterations per slot: gate-closed p95 **0.06–0.22 ms** on `user_prompt_submit` and **0.11–0.16 ms** on `pre_tool_use`, against slot budgets of 250 and 175 ms. Gate-OPEN reads p50 8.6 / p95 87.8 ms and is recorded as the flip's cost, not this change's — evidence file § 10.
...
| 11 | The published table is read as licensing the flip | product | All four endpoints held, so the obvious next reading is "nothing is stopping it". Two things are: an unpaid activation charge (a 20,480-byte emission row above the 4,096/2,048-byte slot sums, and gate-open latency p95 87.8 ms against 0.06-0.22 ms gate-closed) and an authority question the single answering council member flagged himself | AC-8's amended third outcome makes declining a legal, recorded outcome rather than an omission; the charge is stated in the `hook-token-budget.json` row's own reason, in PREREG decision rule 3, and in the published report's closing section | Phase 2 — The falsifier |

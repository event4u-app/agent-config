...
| 13 | ~~`routing_signal_measurement.test.ts` 5.1 is RED on `main`, CI-only, and this roadmap owns the artefact it guards~~ **RESOLVED 2026-09-01 — cause found, fixed upstream, and the fix proven sensitive** | implementation | The test asserts that the published verdict at `agents/evidence/analysis/routing-body-signal-verdict.json` reproduces from a fresh `measure(REPO)` over the skill catalogue. On `main` at run `33424783559` (job `99595610608`, 2026-08-31T18:26Z) it fails with `expected { partition: 'train', ... } to deeply equal { partition: 'train', ... }` on the `corpus` comparison. **Verified pre-existing, not branch-introduced:** the identical assertion fails on `main` before `drain/council-topology-evidence-close` existed, and the same test is GREEN locally (17/17) on both drain branches. So the corpus the CI runner measures differs from the corpus a local checkout measures, and the published verdict matches only the latter. A verdict that reproduces on one environment and not the other is not a reproducible verdict, which is the property 5.1 exists to assert | **FIXED, and the diagnosis this cell previously called out of scope is the one that turned out to be right.** The predecessor text read *"Recorded, not fixed"* and pointed at *"a workflow step that mutates `src/skills/` before the tests run"*. The mutator is not a workflow step — it is a **concurrently-running test file**. `tests/scripts/lint_originality.test.ts:64` writes a re-skin fixture to `src/skills/__origtest_reskin_fixture/SKILL.md` in the REAL tree and removes it in `afterEach`, because that linter resolves its corpus from a module-level ROOT and takes no root argument. Any test file that counts the catalogue during that window reads one skill more than the commit holds, which is why the failure was confined to `corpus`, appeared only under sharded CI, and never reproduced in isolation. **The fix landed on `main` in `0c7fce0d2` (PR #1783), merged here at `6eeb98c91`:** `isScaffoldingSkillDir()` at `src/scripts/_lib/routing_corpus.ts:120` excludes `__`-prefixed directories, filtered into both `corpusSkills()` (`:130`) and `loadCatalogue()` (`:242`); a real skill can never be named one because `skill.schema.json` pins `name` to `^[a-z][a-z0-9-]*$`. `tests/_lib/skills-dir-guard.ts` is the standing backstop. **Verified in this worktree 2026-09-01, both directions, not asserted:** the test is 17/17 green after the merge; with a `__`-prefixed fixture dir present it stays 17/17 (the guard filters it); with a NON-prefixed fixture dir present it goes **1 failed / 16 passed** on exactly `5.1 — the published verdict reproduces from the tree`, the CI failure character for character, and returns to **17/17** when the directory is removed. So the mechanism is the catalogue count and the `__` filter is what holds it. **Nothing about the published verdict changed** — `agents/evidence/analysis/routing-body-signal-verdict.json` is untouched; it was always correct and the measurement around it was polluted | Risk Register |

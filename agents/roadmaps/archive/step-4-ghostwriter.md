...
- [x] **Step 5 — Fictional fixture:** Create `.agent-src.uncompressed/ghostwriter/fictional-fixture-v1.md` (Vera Holmwood, allowlisted) and `.agent-src.uncompressed/ghostwriter/README.md`. The README explains the package-side fictional-only rule and points consumer-side maintainers at `agents/reference/ghostwriter/` for their real profiles. `task sync` carries the fixture verbatim to `.agent-src/ghostwriter/` (added to `COPY_AS_IS_DIRS` in `scripts/compress.py` — voice samples are data, not prose). Consumer-side `agents/reference/ghostwriter/README.md` ships as the only tracked file under the gitignored dir.

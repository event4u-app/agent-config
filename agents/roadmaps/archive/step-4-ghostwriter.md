...
- [x] Read `.agent-src.uncondensed/commands/AGENTS.md` (cluster contract) and `.agent-src.uncondensed/personas/README.md` (confirm review-lens personas stay separate)
...
- **Storage hybrid (Q1):** Consumer project — `agents/reference/ghostwriter/<name>.md`, gitignored by default. Package source (`.agent-src.uncondensed/ghostwriter/`) — README + schema + **fictional fixtures only** (e.g. `fictional-techie.md`). **No real-person profile ever ships with the OSS package.** Resolves the distribution-problem both reviewers flagged as critical.
...
- [x] **Step 2 — Storage model:** Document in the contract: consumer projects use `agents/reference/ghostwriter/<slug>.md` (slug = full-name kebab-case, optional `-<discriminator>` suffix for disambiguation — `alice-walker` vs `alice-walker-novelist`). Package source uses `.agent-src.uncondensed/ghostwriter/` for **README + schema doc + fictional fixtures only** — no real-person files. Add a CI lint (`task lint-ghostwriter-source`) that fails if any file under `.agent-src.uncondensed/ghostwriter/` lacks a `fictional: true` frontmatter key. Implemented via `scripts/lint_ghostwriter_source.py` + `scripts/ghostwriter_fixture_allowlist.txt`, wired into `taskfiles/ci-fast.yml` and the root `Taskfile.yml` `ci` / `ci-strict` aggregates.
- [x] **Step 3 — Gitignore by default:** Add `agents/reference/ghostwriter/*.md` (except `README.md`) to the package-managed `.gitignore` block via the existing `/sync-gitignore` command / `agent-config gitignore:sync` flow. Document a `--shared` opt-in (deferred to v2; only the doc note lands now). `config/gitignore-block.txt` updated; root `.gitignore` carries the new block.
...
- [x] **Step 5 — Fictional fixture:** Create `.agent-src.uncondensed/ghostwriter/fictional-fixture-v1.md` (Vera Holmwood, allowlisted) and `.agent-src.uncondensed/ghostwriter/README.md`. The README explains the package-side fictional-only rule and points consumer-side maintainers at `agents/reference/ghostwriter/` for their real profiles. `task sync` carries the fixture verbatim to `.agent-src/ghostwriter/` (added to `COPY_AS_IS_DIRS` in `scripts/condense.py` — voice samples are data, not prose). Consumer-side `agents/reference/ghostwriter/README.md` ships as the only tracked file under the gitignored dir.
...
- [x] **Step 1 — Implement `/ghostwriter:list`:** Create `commands/ghostwriter/list.md`. Numbered listing of all `agents/reference/ghostwriter/*.md` profiles with name, role, confidence, last-updated, stale-warning flag.
...
- [x] **Zero real-person profiles in `.agent-src.uncondensed/ghostwriter/` or any generated `.agent-src/` / `.augment/` tree** — verified by `task lint-ghostwriter-source`
...
- **Distribution floor (new):** The OSS package itself never ships a real-person ghostwriter profile. The package-side `ghostwriter/` directory contains only README + schema doc + `fictional: true` fixtures. Real profiles live exclusively in consumer projects under `agents/reference/ghostwriter/` and are gitignored by default.

...
| **Examples** | Pattern memory ("HOW it looks") | No cap; lives in `docs/guidelines/` and `agents/settings/contexts/` | Cited from rules/contexts, not auto-loaded |
...
- [x] **1.1** Inventory: list every `load_context:` chain in the 38
      ...
      `rule → context → depth → chars`. → `agents/settings/contexts/rule-trigger-matrix.md`
...
- [x] **4.1** Decision matrix: per always-rule, evaluate 4a/4b/4c/4d
      ...
      [`agents/settings/contexts/budget-v2-matrix.md`](../contexts/budget-v2-matrix.md)

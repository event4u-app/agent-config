...
- [x] **3.5 Confirm duplication removal.** The logic / mechanics moved into the loaded context must be physically removed from the slim rule — diff the pre-/post-rule and assert no overlap. → [`agents/runtime/reports/pr-34-phase-3-5-duplication-removal.md`](../reports/pr-34-phase-3-5-duplication-removal.md) (zero shared non-trivial lines).

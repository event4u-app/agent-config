...
- [x] Before/after over the Phase-2 corpus + telemetry replay, two hard bounds: the false-fire census count strictly decreases, and zero previously-passing matrix positives go silent — either bound violated reverts the change. <!-- verify: npx tsx src/scripts/router_telemetry.ts --replay comparison committed next to the Phase-2 baseline --> <!-- done 2026-08-03: unintended census 495 → 433 (−12.5%), 262/262 matcher-suite tests green, zero positives lost; reports 07-09-11 (before) + 07-22-38 (after) committed. AMENDED 2026-08-03 (external review Finding 3): the derived corpus was co-edited in the same commit (6 German positives re-authored) — circularity broken by an independent replay over 49 un-edited real-corpus prompts: recall 15/17 in BOTH arms, zero labels lost, unintended 110→99 (agents/evidence/analysis/anchoring-independent-replay-2026-08.md). Residual top false-firer markdown-safe-codeblocks (33×) is file_pattern-driven — keyword anchoring cannot touch it; noted as follow-up trigger tuning -->
...
- [x] Cheap-request delegation respects the budget relation (cheapest adequate tier with budget; strong tier when only it has budget; never blocked), is off-switchable with one settings flip, and emits telemetry per routed request. <!-- pickTier relation 14/14 incl. race + cool-down; subagents.budget_routing: off = single flip; telemetry line mandated in subagent-routing § Policy 4 --> <!-- AMENDED 2026-08-03 (external review Finding 2): worded to what the code proves — the RELATION is implemented and deterministically tested (incl. the full reserve lifecycle, AC1–AC5); DELIVERY is agent-adherent policy with no runtime code caller, monitored by routing:doctor's delivery-evidence WARN. NOT 'budget-aware delegation shipped'. Ledger: docs/CLAIMS.md#budget-routing-relation -->

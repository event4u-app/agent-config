...
|------|------|-----------|-------------|------------|----------------|
| 1 | The harvest ledger becomes ceremony nobody feeds | product | `borrows.jsonl` has **0** rows after existing for a full governance roadmap; a second empty ledger is a plausible outcome and would add a gate, a schema, and a rule section for no recorded claim | Ship it wired to a producer in the same phase — 1.4 makes the comparative-analysis command the writer, so the ledger has an author before it has a policy; if it is still empty after the next real harvest, that is a falsifier worth recording, not a reason to add reminders | Phase 1 |
| 2 | A new gate lands red on the tree as it stands | implementation | `lint_harvest_provenance` (Phase 1) and `lint_skill_router_head` (Phase 4) both scan surfaces never scanned before; either may report pre-existing state on its first run | Classify every first-run finding before wiring into `task ci`; a real finding is a fix in this roadmap, never a loosened gate. **Both fired, and both findings were real:** the harvest gate flagged this roadmap for spelling the citation marker literally in prose (fixed with the angle-bracket placeholder, now pinned by a test), and the offender census returned four skills where the drafts named three. The router-head allowlist is seeded from that measured set and re-derived by a test, never guessed | Phase 4 |
...
| 4 | Additive persona frontmatter drifts past additive | implementation | A `sources:` field that lint treats as required, or a worked example that invents a harvest row to have something to declare, breaks 29 personas or corrupts the ledger's first entries | Absent field is normative today's behaviour and is pinned by a test; the worked example is allowed to be an empty declaration with a stated reason, and is one | Phase 2 |

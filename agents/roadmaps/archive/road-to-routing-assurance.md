...
> **Parked, not dropped — 0.2 and 0.4 moved to
> `agents/roadmaps/later/road-to-routing-assurance-live-floors.md`.** AI council
> 2026-08-25, 2/2 convergent. Both need a live LLM harness run, and the session
> that would have captured the baseline is the session that authored the Phase-2
> corpus. One seat put the reason past where the question had framed it:
> *"evaluating an artifact you authored"* — capturing a baseline of a corpus you
> just fixed crosses evaluator independence even without interpreting it. The
> other added that this roadmap froze no execution protocol for 0.2, so model
> version, retries and aggregation would have been discretionary choices made by
> the session under evaluation.
>
> What 0.4 did NOT need a baseline for is already merged: the derivation rule
> and the tolerance (0.10 absolute recall), fixed in
> `docs/contracts/routing-assurance-metrics.md` before any number existed. That
> is the half of 0.4 that is gameable if left until after.
...
## Phases 3–6 — parked at this roadmap's own cut line

> **Moved verbatim to
> `agents/roadmaps/later/road-to-routing-assurance-live-floors.md`** (AI council
> 2026-08-25, 2/2), together with steps 0.2 and 0.4. Both pre-registered nulls
> travel with them, quoted rather than paraphrased.
>
> The cut line above sanctions this stop in this roadmap's own words, which is
> why the council treated the disposition as council-decidable rather than
> owner-reserved: the move is reversible, creates no external commitment, and
> every criterion stays active in the estate.
>
> **Un-parked by:** an independent session — not the one that authored the
> Phase-2 corpus — freezing the execution protocol in writing, then running 0.2.
...
- [x] AC-1 — The pre-registration, the frozen baseline and both ratchets exist
      and predate the first Phase-1 commit in history.
      **Met for two of the three, and the third is transferred rather than
      claimed.** `docs/contracts/routing-assurance-metrics.md` and
      `check_routing_coverage` + `routing-coverage-seed.json` landed in commits
      `bbcb8e7` and `707fd4f`; the first Phase-1 commit is `7c9edc6`, so the
      history order is a fact of the log rather than an assertion. **The frozen
      baseline does NOT exist** — 0.2 is parked, and it carries this half of the
      criterion into `later/road-to-routing-assurance-live-floors.md` AC-1.
      Recorded as a split rather than a pass, because a criterion two-thirds met
      and reported green is the silent-green defect this repository already
      knows.
- [x] AC-2 — `description_route_check` runs on the full catalogue with a
      no-spend dry-run backend, and a repeated prompt-catalogue pair costs zero
      calls.
      **Met.** The catalogue is the whole projected estate — **418 units, all
      418 carrying a description** — never a scoped subset, because routing is
      competitive and scoping the catalogue would change what is measured. The
      zero-call half is asserted directly: a second identical run leaves
      `inner.calls` at 1. A companion test proves the key MISSES when any
      description changes, which is the property a prompt-only key would have
      broken exactly when the answer changed.
- [x] AC-3 — The diff-scoped pre-merge gate blocks a description edit that
      ...
      **All three demonstrated, as unit cases and end-to-end on the CLI.**
      Block → exit 1; warn → exit 0 with the finding printed; fork → `advisory —
      the key file is unreachable by construction`, refused before any spend
      rather than as a key error at the router. A fourth case runs block and
      warn together and asserts neither masks the other.
- [x] AC-4 — The skill-scope coverage ratchet stands strictly above its seed,
      and every top-invoked skill has a corpus file meeting the discipline.
      **Met on both halves, and the second half is met VACUOUSLY — stated, not
      hidden.** Coverage rose 0.2542 → 0.3010 (76 → 90 of 299) and the seed was
      raised in the same change, so the ratchet stands above its original seed
      and cannot give the gain back. The top-invoked set is **empty**:
      `skill_usage_report` on this tree reads *"299 tracked · Active: 0 ·
      Exposed-only: 0 · Dead: 299"*. Every member of an empty set satisfies any
      predicate, which is why this reads as a null rather than as coverage.
- [-] AC-5 — **Transferred**, not cancelled → `later/road-to-routing-assurance-live-floors.md`
      AC-3, unchanged. Needs the live catalogue-pressure suite.
- [-] AC-6 — **Transferred**, not cancelled → the parked roadmap's AC-4,
      unchanged. Needs a live run on both delivery paths.
- [-] AC-7 — **Transferred**, not cancelled → the parked roadmap's AC-5,
      unchanged. Needs the production-trace corpus (Phase 5).
- [-] AC-8 — **Transferred**, not cancelled → the parked roadmap's AC-6,
      unchanged. Needs floors, which need the 0.2 baseline.

> **On the four `[-]` rows.** They are transfers, and the distinction is not
> cosmetic: `decision-revisit-gate`'s preservation test reserves a genuine
> DROP to the owner and leaves a carry to the council, provided the follow-up
> roadmap is created in the SAME change. It is — `later/road-to-routing-assurance-live-floors.md`
> lands in this commit, carries all four criteria verbatim, and names the
> condition that un-parks them. AI council 2026-08-25, 2/2, authorised the
> carry; nothing here was dropped, weakened, or re-scoped.

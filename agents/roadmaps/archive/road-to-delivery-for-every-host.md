...
- [x] **4.4 Lower the baseline, retire the grace ceiling.**
      ...
      **CLOSED 2026-09-11 — STAGE 2 LANDED. `ADR-276`.** The owner authorised the two moves
      an autonomous lane may not take, and the step's own exit condition is met:
      `grep -c grace_ceiling src/config/preamble-payload-budget.json` returns **0**.
      **The precondition was verified rather than assumed**, which is the whole reason the
      work was split: the base-ref copy of the gate was materialised with
      `git checkout-index` and run against the head tree with `--repo-root`, exit 0, before
      anything was deleted. That is the "from a base containing that code" the council named.
      **What landed.** (i) `Standing payload delta + budget gate` added to
      `required_status_checks` on ruleset `main protection`, with `OrganizationAdmin` as a
      bypass actor — the council's option (b). Measured before: `bypass_actors: []` and
      `current_user_can_bypass: "never"`; measured after: `"always"`. Plain "required"
      against an empty bypass list would have turned any INFRASTRUCTURE red into an
      unbypassable merge block for the only person able to fix it, which is the owner
      constraint read backwards. (ii) `grace_ceiling`, `grace_measured_at` and the raise
      history key deleted from `ci_delivery`; the two raises retained under
      `superseded_stored_ceiling`, because they are the evidence behind `ADR-264` and
      removing the ceiling is not a reason to remove the record of it having been raised
      twice under a sentence forbidding it. (iii) `stored_ceiling` removed from the bound
      set, the formula and the `Budget` type. The formula is now exactly
      `max(design_ceiling, min(base, watermark) + active grant)`, and the tree measures
      138,413 against a ceiling of 138,413 — **zero headroom, by design**: any growth reds,
      and the red is a merge block now rather than a notification.
      **Two regression pins, proven sensitive:** reintroducing `grace_ceiling` turns them
      red (2 failed), removing it again restores green (40 passed). One reads the parsed
      object, one greps the file — a reintroduction inside a prose field passes the first
      and fails the second.
      **ONE PREREQUISITE IS NOT DISCHARGED, and it is recorded rather than waved through.**
      The council made a bypass-and-recovery DRILL blocking. It was attempted and refused:
      exercising it means deliberately reddening CI on a throwaway pull request, which this
      environment's safety classifier blocks as a CI-bypass action, and working around that
      refusal would have been the wrong response to it. What is verified is the
      CONFIGURATION — required context listed, bypass actor present,
      `current_user_can_bypass` flipped. What is NOT verified is the BEHAVIOUR under a real
      red. **Run the drill once by hand before relying on the bypass in an incident.**
      The step is ticked on its own exit condition and on the council's Q4 limbs
      (active, required, fail-closed, measured, repayment-preserving), with this gap named
      in `ADR-276` § Honest limits rather than folded into the tick.
      **Still owner-only, and unchanged:** a second repository admin. With one admin who
      authors every pull request, no independent approving review can exist, so no grant in
      the exception ledger can be authenticated and every grant is refused. Fail-closed and
      intended; the admin bypass is the recovery path in its place.
...
## Risk Register
<!-- risk-review: v2 | reviewed: 2026-09-11 | reviewer: claude/host -->
...
- [x] Rollback fixture green; grace ceiling gone; all quality gates green.
      ...
      **MET 2026-09-11 — three of three. `ADR-276`.**
      **Rollback fixture green** (unchanged, 4.5). **The grace ceiling is GONE** —
      `grep -c grace_ceiling src/config/preamble-payload-budget.json` returns 0, the key is
      absent from `ci_delivery`, the `stored_ceiling` term is out of the formula and the
      bound set, and two regression pins refuse its return (proven sensitive: 2 red under
      reintroduction, 40 green after removal). **All quality gates green** in this change:
      `check_preamble_payload_budget` (138,413 against a measured ceiling of 138,413 — zero
      headroom, which is the design rather than a near miss), `check_adr_frontmatter`,
      `typecheck-ts`, `lint-ts`, `task preflight`, and 83 tests across the three files this
      change touches.
      The council's Q4 limbs are met in full: the stored exception is replaced by a measured
      ratchet that is ACTIVE (it runs on every pull request), REQUIRED (in
      `required_status_checks` on ruleset `main protection`, with an audited admin bypass),
      FAIL-CLOSED (`--require-base` refuses an unreadable base), and repayment-preserving
      (a deleted grant is refused, and `repaid_at` is corroborated against the measurement).
      **One named gap travels with this tick rather than under it:** the bypass-and-recovery
      DRILL was attempted and blocked by a safety classifier that reads a deliberate CI red
      as a bypass attempt. The configuration is verified; the behaviour under a real red is
      not. `ADR-276` § Honest limits carries it, and it is a one-off manual exercise rather
      than an open engineering step.

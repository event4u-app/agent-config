...
`.agent-src.uncondensed/` (12 `load_context:` across 11 rules + 4
...
hybrid: logical names in source, condense-time path rewriting,
...
condense-time rewriter (load-bearing primitive), Phases 2-4 migrate
...
- Zero `.agent-src.uncondensed/` substrings in shipped `.agent-src/rules/`.
- Zero broken markdown links in `.agent-src/rules/` resolved against the
  consumer's expected `.augment/` layout.
- `task ci` green; new condense-time validator catches regressions.
...
- [x] **P0.3 — Confirm `condense.py` is the right rewrite host.**
  `scripts/condense.py` already runs `.agent-src.uncondensed/` →
  `.agent-src/` (via `should_condense` + `strip_frontmatter`); the
  rewriter goes between `strip_frontmatter` and the write step in
  `project_to_augment()` / per-file condensation. Confirmed —
  extending, not forking.

## Phase 1 — Condense-time path rewriter

- [x] **P1.1 — Logical-name schema in source.** Frontmatter
  `load_context:` and `path_prefix:` accept paths without the
  `.agent-src.uncondensed/` prefix; the prefix is forbidden going
  forward. Documented in `.agent-src.uncondensed/templates/rule.md`
  ...
  in P2 unblocked.
- [x] **P1.2 — Implement rewriter in `scripts/condense.py`.** Added
  `_rewrite_paths(content, source_relative_path)` (depth-aware,
  idempotent) plus `apply_path_rewriter(relative_path)` wired into
  `mark_done`, so each condensed file is rewritten in-place at the
  ...
  - frontmatter `triggers[].path_prefix:` declarations → strip
    `.agent-src.uncondensed/` to `.agent-src/`
  - body links `../../docs/guidelines/...` and `../../docs/contracts/...`
    → depth-aware single-up form.
- [x] **P1.3 — Unit-test the rewriter.** `tests/test_condense_paths.py`
...
- [x] **P2.2 — Treat `path_prefix:` as literal match pattern (Modified Option 1).**
  ...
  `.agent-src.uncondensed/` declarations in `skill-quality`,
  ...
  rewriter docstring in `scripts/condense.py` updated. Test
  `test_legacy_prefix_left_alone` inverted to verify no-op.
- [x] **P2.3 — Re-condensed and verified.** Ran `--mark-done` per
  rule on all 10 files (autonomous-execution covers 3 entries in
  one file). Condensed output uses `../contexts/...` form;
  zero `.agent-src.uncondensed/` substrings remain in
...
- [x] **P3.2 — Body-link rewriter retained as built; no source migration.**
  `condense.py` `_BODY_DOCS_RE` rewrites `../../docs/{guidelines,contracts}/...`
  to depth-aware single-up form at condense time and is idempotent —
...
- [x] **P3.3 — Copilot-review tolerance documented.** Added "Known
  ...
  `.agent-src.uncondensed/templates/copilot-instructions.md` covering
  (a) relative cross-references inside `.augment/` rules/skills,
  (b) `path_prefix:` triggers containing `.agent-src.uncondensed/`,
...
- [x] **P4.1 — Promote contracts to `contexts/contracts/`.** Moved
  ...
  `.agent-src.uncondensed/contexts/contracts/` via `git mv`.
...
- [x] **P4.2 — Switch the two rules to `load_context:`.** Removed the
  ...
  in condensed output. Body now points at the same context via
...
## Phase 5 — Condense-time validation gate — REVISED via Council 2026-05-06
...
`.agent-src.uncondensed/` concept by name — that is the rule's
...
- [x] **P5.1 — Implemented validator + `validator_ignore:` schema.** New
  `scripts/check_condensed_paths.py` runs after condensation. Reads
...
- [x] **P5.2 — Wired into `task ci`.** Added `check-condensed-paths`
  task in `taskfiles/content.yml`; sequenced after `check-condensation`
  in the root `Taskfile.yml` `ci` task. Fails the build on any
  unsuppressed forbidden substring or unresolved `load_context:`.
- [x] **P5.3 — Forbid `.agent-src.uncondensed/` prefix in source
  ...
  - **Schema regex** in `scripts/schemas/rule.schema.json`:
    ...
    so any `.agent-src.uncondensed/contexts/...` entry fails
  ...
  - **Linter** in `scripts/lint_load_context.py`: removed the legacy
    ...
    branch that emits `legacy .agent-src.uncondensed/ prefix in
  ...
  - `path_prefix:` is **deliberately not in scope** — P2.2 council
    ...
    legitimately use the `.agent-src.uncondensed/` prefix there
...
- [x] **P6.1 — Added `.github/copilot-review-instructions.md`**
  template at `.agent-src.uncondensed/templates/copilot-review-instructions.md`,
  ...
  (`scripts/check_condensed_paths.py`).
...
or context and reintroduces the legacy `.agent-src.uncondensed/`
...
- [x] **P8.3 — Author-facing canonical reference.** Path-conventions
  block in `.agent-src.uncondensed/templates/rule.md` distinguishes
...
- [x] **P8.4 — Contract update.** `docs/contracts/load-context-schema.md`
  leads with logical names; the `.agent-src.uncondensed/` prefix is
...
- [x] **P8.5 — `path_prefix:` deliberately out of scope.** P2.2
  ...
  (`skill-quality`, `docs-sync`, `rule-type-governance`,
  `augment-portability`) legitimately use the `.agent-src.uncondensed/`
  prefix there, and the rewriter leaves it verbatim.

## Risks

- **Augment-Host path resolution is empirical, not contracted.**
  Condensed output uses relative-from-rule paths; if Augment

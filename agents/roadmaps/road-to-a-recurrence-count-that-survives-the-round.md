...
- [ ] **1.1 Put the arrival line on every held object this round's survivors mapped onto.** One
      ...
      source identity.
- [ ] **1.2 Correct the one object whose counter under-counts.** It records three arrivals against
      a measured six.
      verify: the corrected number is on the file with the round codenames that justify it.
- [ ] **1.3 Say where each number came from.** The inbox tree is gitignored, so a clone cannot

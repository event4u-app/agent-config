...
|---|---|---|
the vendored design corpus @ `b7e3af80` — identity and license in `ATTRIBUTION.md` (MIT corpus + engine; second upstream Apache-2.0) | `src/skills/design-intelligence/ATTRIBUTION.md:8-11`, replicated in nine further places — `data/manifest.json:3`, `references/design-languages.md:6`, `design-tokens/SKILL.md:34`, `corpus-grounding/SKILL.md:29`, `tailwind-engineer/scripts/tailwind_config_gen.ts:4`, `react-shadcn-ui/scripts/shadcn_add.ts:4`, `ADR-061:170`, plus two watch notes | the grounding corpus and engine. **Partially closed 2026-08-13:** `motion.csv` and all three design dials now ship, adopted selectively from `97eb2a20` — the base pin stays `b7e3af80` deliberately, so the ten-site sweep is correctly NOT done and nothing asserts a stale SHA. **Residual drift, now ownerless:** `google-fonts.csv` (declined — ADR-061 §8 rejects it by name; the 2026-06-16 amendment authorises a slim top-N slice from a different mirror), `typography.csv` (declined — our curated `font-pairings-reference.csv` is the working substitution), six desktop-UI stacks outside `frontend-design`'s scope, and eleven content-drifted files nobody has reviewed. A future re-pin that DOES move the base SHA still owes the ten-site sweep |

...
Bounded fix loop. Hard ceiling: `POLISH_CEILING = 2` rounds.
...
Schema-level validation in `work_engine.state._validate_ui_polish`
rejects `rounds > 2` on disk, so the ceiling holds across in-memory
state, on-disk state, and the dispatcher.
...
polish round (+1 per dirty review, capped at 2),
polish ceiling (+1 when both rounds fail).
...
| `review` | `review_envelope_missing`, `review_findings_missing`, `review_clean_missing` |
| `polish` | `polish_round_pending`, `polish_ceiling_reached`, `polish_token_extraction_pending` |

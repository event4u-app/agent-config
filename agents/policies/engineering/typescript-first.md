...
1. Surface this file path (`agents/policies/engineering/typescript-first.md`).
...
LLM-readable decision framework, consistent with `agents/policies/media/README.md § Enforcement model`. No runtime gate; the agent reads the policy when its triggers fire and surfaces it before the diff lands. CI is not asked to classify "is this logic-carrying?" — that judgment stays in the session.

...
- **`founder-strategy`** — competitive-positioning, fundraising-narrative, market-entry-analysis, unit-economics-modeling. The reasoning kernel for consulting work.
...
> **Status:** `draft` — the three first tasks above are the maintainer's hypothesis, derived from the founder-strategy setup's existing skill set. Recruit-session 03 will validate them against what consultants actually reach for first. Prompts deferred to the follow-up impl PR.

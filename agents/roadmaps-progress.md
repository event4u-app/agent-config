# Roadmap Progress

> Auto-generated by `.augment/scripts/update_roadmap_progress.py`. Do not edit — regenerated on every roadmap-create, -execute, or completion change (last-modified timestamp lives in git history).
>
> 8 open roadmaps · [roadmaps/](roadmaps/) · [archive/](roadmaps/archive/) · [skipped/](roadmaps/skipped/)

## Overall

**0 / 192 steps done · 0%**

```text
░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░   0%
```

## Open roadmaps

| # | Roadmap | Phases | Steps | Open | Done | Deferred | Cancelled | Progress |
|---|---|---:|---:|---:|---:|---:|---:|---|
| 1 | [road-to-distribution-and-adoption.md](roadmaps/road-to-distribution-and-adoption.md) | 4 | 13 | 13 | 0 | 0 | 0 | ░░░░░░░░░░ 0% |
| 2 | [road-to-gtm-and-growth.md](roadmaps/road-to-gtm-and-growth.md) | 1 | 26 | 26 | 0 | 0 | 0 | ░░░░░░░░░░ 0% |
| 3 | [road-to-mcp-full-coverage.md](roadmaps/road-to-mcp-full-coverage.md) | 5 | 29 | 29 | 0 | 0 | 0 | ░░░░░░░░░░ 0% |
| 4 | [road-to-money-strategy-ops.md](roadmaps/road-to-money-strategy-ops.md) | 1 | 31 | 31 | 0 | 0 | 0 | ░░░░░░░░░░ 0% |
| 5 | [road-to-portable-dev-preferences.md](roadmaps/road-to-portable-dev-preferences.md) | 3 | 9 | 9 | 0 | 0 | 0 | ░░░░░░░░░░ 0% |
| 6 | [road-to-productization.md](roadmaps/road-to-productization.md) | 8 | 27 | 27 | 0 | 0 | 0 | ░░░░░░░░░░ 0% |
| 7 | [road-to-proof-not-features.md](roadmaps/road-to-proof-not-features.md) | 3 | 19 | 19 | 0 | 0 | 0 | ░░░░░░░░░░ 0% |
| 8 | [road-to-simplicity-and-everywhere.md](roadmaps/road-to-simplicity-and-everywhere.md) | 7 | 38 | 38 | 0 | 0 | 0 | ░░░░░░░░░░ 0% |

---

## Per-roadmap phase breakdown

### [road-to-distribution-and-adoption.md](roadmaps/road-to-distribution-and-adoption.md)

**Road to Distribution and Adoption** — 0 / 13 done (0%)

| # | Phase | State | Open | Done | Deferred | Cancelled | % |
|---|---|---|---:|---:|---:|---:|---:|
| 1 | Block H — Marketing | ⬜ not started | 4 | 0 | 0 | 0 | 0% |
| 2 | Block I — Multi-tool expansion | ⬜ not started | 3 | 0 | 0 | 0 | 0% |
| 3 | Block G — Orchestration DSL | ⬜ not started | 3 | 0 | 0 | 0 | 0% |
| 4 | Block Q — Audit-as-Memory | ⬜ not started | 3 | 0 | 0 | 0 | 0% |

### [road-to-gtm-and-growth.md](roadmaps/road-to-gtm-and-growth.md)

**Road to GTM and Growth (Wing 3)** — 0 / 26 done (0%)

| # | Phase | State | Open | Done | Deferred | Cancelled | % |
|---|---|---|---:|---:|---:|---:|---:|
| 1 | GTM and Growth execution | ⬜ not started | 26 | 0 | 0 | 0 | 0% |

### [road-to-mcp-full-coverage.md](roadmaps/road-to-mcp-full-coverage.md)

**Road to MCP Full Coverage** — 0 / 29 done (0%)

| # | Phase | State | Open | Done | Deferred | Cancelled | % |
|---|---|---|---:|---:|---:|---:|---:|
| 1 | Discovery Stubs | ⬜ not started | 6 | 0 | 0 | 0 | 0% |
| 2 | Telemetry Window + Decision Gate | ⬜ not started | 5 | 0 | 0 | 0 | 0% |
| 3 | Selective Implementation (read-only first) | ⬜ not started | 5 | 0 | 0 | 0 | 0% |
| 4 | Write-Tool Envelope (DEFERRED) | ⬜ not started | 4 | 0 | 0 | 0 | 0% |
| 5 | TypeScript Port (DEFERRED) | ⬜ not started | 9 | 0 | 0 | 0 | 0% |

### [road-to-money-strategy-ops.md](roadmaps/road-to-money-strategy-ops.md)

**Road to Money, Strategy and Operations (Wing 4)** — 0 / 31 done (0%)

| # | Phase | State | Open | Done | Deferred | Cancelled | % |
|---|---|---|---:|---:|---:|---:|---:|
| 1 | Money, Strategy and Operations execution | ⬜ not started | 31 | 0 | 0 | 0 | 0% |

### [road-to-portable-dev-preferences.md](roadmaps/road-to-portable-dev-preferences.md)

**Road to Portable Dev Preferences** — 0 / 9 done (0%)

| # | Phase | State | Open | Done | Deferred | Cancelled | % |
|---|---|---|---:|---:|---:|---:|---:|
| 1 | Loader (`scripts/_lib/agent_settings.py`) | ⬜ not started | 4 | 0 | 0 | 0 | 0% |
| 2 | Onboarding writes user-global | ⬜ not started | 2 | 0 | 0 | 0 | 0% |
| 3 | Atomic loader migration | ⬜ not started | 3 | 0 | 0 | 0 | 0% |

### [road-to-productization.md](roadmaps/road-to-productization.md)

**Road to Productization (Level 6)** — 0 / 27 done (0%)

| # | Phase | State | Open | Done | Deferred | Cancelled | % |
|---|---|---|---:|---:|---:|---:|---:|
| 1 | Release-trunk Sync (READY) | ⬜ not started | 3 | 0 | 0 | 0 | 0% |
| 2 | Decision Engine controllable (gated on P1) | ⬜ not started | 6 | 0 | 0 | 0 | 0% |
| 3 | UX Simplification (gated on P2) | ⬜ not started | 4 | 0 | 0 | 0 | 0% |
| 4 | Multi-Stack Skill-Depth (gated on P3) | ⬜ not started | 3 | 0 | 0 | 0 | 0% |
| 5 | Architecture Cleanup (gated on P4) | ⬜ not started | 5 | 0 | 0 | 0 | 0% |
| 6 | Sibling: `road-to-proof-not-features.md` 100 % (BLOCKER) | ⬜ not started | 2 | 0 | 0 | 0 | 0% |
| 7 | Sibling: `road-to-better-skills-and-profiles.md` Block A (BLOCKER) | ⬜ not started | 1 | 0 | 0 | 0 | 0% |
| 8 | Final Validation (gated on P1–P7) | ⬜ not started | 3 | 0 | 0 | 0 | 0% |

### [road-to-proof-not-features.md](roadmaps/road-to-proof-not-features.md)

**Road to Proof not Features** — 0 / 19 done (0%)

| # | Phase | State | Open | Done | Deferred | Cancelled | % |
|---|---|---|---:|---:|---:|---:|---:|
| 1 | Beweis (3 real showcase sessions) | ⬜ not started | 5 | 0 | 0 | 0 | 0% |
| 2 | Erklärbarkeit (consequence + audience + debug) | ⬜ not started | 10 | 0 | 0 | 0 | 0% |
| 3 | Stabilisierung (deferred, gated on Phase 1 + 2) | ⬜ not started | 4 | 0 | 0 | 0 | 0% |

### [road-to-simplicity-and-everywhere.md](roadmaps/road-to-simplicity-and-everywhere.md)

**Road to Simplicity and Everywhere** — 0 / 38 done (0%)

| # | Phase | State | Open | Done | Deferred | Cancelled | % |
|---|---|---|---:|---:|---:|---:|---:|
| 1 | Multi-Tool Installer Selector | ⬜ not started | 6 | 0 | 0 | 0 | 0% |
| 2 | One-Liner Entrypoints (`npx` + `curl|bash`) | ⬜ not started | 5 | 0 | 0 | 0 | 0% |
| 3 | Global User-Level Install | ⬜ not started | 5 | 0 | 0 | 0 | 0% |
| 4 | Claude Desktop + Claude Cowork Path | ⬜ not started | 4 | 0 | 0 | 0 | 0% |
| 5 | Cursor + Windsurf Modern Format | ⬜ not started | 6 | 0 | 0 | 0 | 0% |
| 6 | Per-IDE Setup Documentation | ⬜ not started | 7 | 0 | 0 | 0 | 0% |
| 7 | Marketplace Listings + Discoverability | ⬜ not started | 5 | 0 | 0 | 0 | 0% |


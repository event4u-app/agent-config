...
>
> 33 open roadmaps · [roadmaps/](roadmaps/) · [archive/](roadmaps/archive/) · [skipped/](roadmaps/skipped/) · [later/](roadmaps/later/) · **51** open blockers in the active tree, **23** need you → `agent-config gates`

## Overall

**302 / 598 steps done · 51%**
...
| 23 | [road-to-session-closeout.md](roadmaps/road-to-session-closeout.md) | 8 | 49 | 49 | 0 | 0 | 0 | [3](#blockers-road-to-session-closeout) | ░░░░░░░░░░ 0% |
...
**Road to closing what fifty sessions left open** — 0 / 49 done (0%)

| # | Phase | State | Open | Done | Deferred | Cancelled | % |
|---|---|---|---:|---:|---:|---:|---:|
| 1 | the release line that stops at the tag | ⬜ not started | 5 | 0 | 0 | 0 | 0% |
| 2 | the half of the payload defect that was left unfixed | ⬜ not started | 4 | 0 | 0 | 0 | 0% |
| 3 | double delivery: measured, gated, and still happening | ⬜ not started | 4 | 0 | 0 | 0 | 0% |
| 4 | budgets that document deterioration instead of preventing it | ⬜ not started | 4 | 0 | 0 | 0 | 0% |
| 5 | the gates nobody sees run | ⬜ not started | 3 | 0 | 0 | 0 | 0% |
| 6 | the shared checkout, which is a cause and not a symptom | ⬜ not started | 3 | 0 | 0 | 0 | 0% |
| 7 | rescue, then deletion | ⬜ not started | 9 | 0 | 0 | 0 | 0% |
| 8 | maintenance risk, process economy, and the homeless defects | ⬜ not started | 17 | 0 | 0 | 0 | 0% |

<a id="blockers-road-to-session-closeout"></a>
**Blockers**

- **release-line-disposition** (owner: user) — blocks Phase 1 steps 1.2 and 1.4, and therefore 1.5's verification
  - **Recommendation:** (a). The trunk already declares the version and the CHANGELOG already carries its section, so (b) is not a smaller change — it is the same change plus two reversals, and it spends a second version number to undo the first. (a) becomes the wrong call only if the tagged content is itself unfit to ship, which step 1.1 is what establishes.
  - **If you do nothing:** the registry keeps serving the previous version while the trunk and the CHANGELOG both claim the newer one, no check objects, and the next release run starts from a tree whose version line is already false.
  - **What to do:**
    pick exactly one — (a) carry the currently open release line
  ...
  - **Resolved when:** the maintainer has stated (a) or (b) this turn, and the chosen path has landed as a change rather than as a reply.
- **worktree-safe-set-removal** (owner: user) — blocks Phase 7 steps 7.6 and 7.9
  - **Recommendation:** approve, optionally narrowed by age. The predicate is conservative by construction — trunk ancestry AND a clean tree AND no recent git-dir activity — and `git branch -d` re-checks each merge at execution time, so the failure mode of approving is a refusal rather than a loss. The argument for narrowing is reviewability, not safety: a smaller first sweep makes the residual count readable.
  - **If you do nothing:** the tooling built to make this decidable stays unused a second time, the measured 54 GB stays, and the count keeps moving in the direction it moved 249 → 346 while this same plan was available.
  - **What to do:**
    review the prepared plan from
  ...
  - **Resolved when:** the maintainer has approved, narrowed or declined this turn, and — if approved — the post-removal count is recorded beside the pre-removal one.
- **injection-scan-contract-shape** (owner: maintainer) — blocks Phase 2 steps 2.1 through 2.3
  - **Recommendation:** (a). The pair's other half was fixed by narrowing, the scanner is the member of the pair marked security-relevant, and a fallback that scans more than intended is only safe until the extra content is attacker-influenced — which on this surface is the assumption, not the edge case. (b) is defensible only if a shape exists that (a) would refuse and the dispatcher still emits; step 2.2's fixtures are what would show that.
  - **If you do nothing:** the reader stays correct by accident, no test covers the accident, and the deferral's stated precondition — a contract plus fixtures — remains unmet while the sibling fix makes the pair look closed.
  - **What to do:**
    pick exactly one shape for the scanner's read contract —
  ...
  - **Resolved when:** the maintainer has named (a) or (b) and the contract in 2.1 is written to it.

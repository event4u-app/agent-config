# Roadmap Progress

> Auto-generated by `.augment/scripts/update_roadmap_progress.py`. Do not edit — regenerated on every roadmap-create, -execute, or completion change (last-modified timestamp lives in git history).
>
> 4 open roadmaps · [roadmaps/](roadmaps/) · [archive/](roadmaps/archive/) · [skipped/](roadmaps/skipped/)

## Overall

**63 / 118 steps done · 53%**

```text
█████████████████████░░░░░░░░░░░░░░░░░░░   53%
```

## Open roadmaps

| # | Roadmap | Phases | Steps | Open | Done | Deferred | Cancelled | Progress |
|---|---|---:|---:|---:|---:|---:|---:|---|
| 1 | [road-to-product-ui-track.md](roadmaps/road-to-product-ui-track.md) | 7 | 53 | 42 | 10 | 1 | 0 | ██░░░░░░░░ 19% |
| 2 | [road-to-stable-chat-history.md](roadmaps/road-to-stable-chat-history.md) | 5 | 28 | 6 | 19 | 3 | 0 | ████████░░ 76% |
| 3 | [road-to-universal-distribution.md](roadmaps/road-to-universal-distribution.md) | 7 | 44 | 3 | 34 | 7 | 0 | █████████░ 92% |
| 4 | [road-to-visual-review-loop.md](roadmaps/road-to-visual-review-loop.md) | 1 | 4 | 4 | 0 | 0 | 0 | ░░░░░░░░░░ 0% |

---

## Per-roadmap phase breakdown

### [road-to-product-ui-track.md](roadmaps/road-to-product-ui-track.md)

**Product UI Track** — 10 / 52 done (19%)

| # | Phase | State | Open | Done | Deferred | Cancelled | % |
|---|---|---|---:|---:|---:|---:|---:|
| 1 | Stack detection and UI intent routing | ✅ done | 0 | 5 | 0 | 0 | 100% |
| 2 | Existing-UI-Audit (the hard gate) | ✅ done | 0 | 5 | 1 | 0 | 100% |
| 3 | UI directive set (design → apply → review → polish) | ⬜ not started | 6 | 0 | 0 | 0 | 0% |
| 4 | Mixed orchestration (backend + UI) | ⬜ not started | 5 | 0 | 0 | 0 | 0% |
| 5 | `fe-design` migration and skill repositioning | ⬜ not started | 5 | 0 | 0 | 0 | 0% |
| 6 | Golden Compatibility Tests + UI goldens | ⬜ not started | 4 | 0 | 0 | 0 | 0% |
| 7 | Verification and docs | ⬜ not started | 22 | 0 | 0 | 0 | 0% |

### [road-to-stable-chat-history.md](roadmaps/road-to-stable-chat-history.md)

**Stable Chat History via Platform Hooks** — 19 / 25 done (76%)

| # | Phase | State | Open | Done | Deferred | Cancelled | % |
|---|---|---|---:|---:|---:|---:|---:|
| 1 | Platform analysis (analysis-only, no code) | ✅ done | 0 | 8 | 0 | 0 | 100% |
| 2 | Implement HOOK strategy where available | ✅ done | 0 | 4 | 0 | 0 | 100% |
| 3 | CHECKPOINT fallback for platforms without hooks | ✅ done | 0 | 2 | 0 | 0 | 100% |
| 4 | Rule de-escalation | ✅ done | 0 | 4 | 0 | 0 | 100% |
| 5 | Verification | 🟡 in progress | 6 | 1 | 3 | 0 | 14% |

### [road-to-universal-distribution.md](roadmaps/road-to-universal-distribution.md)

**Universal Distribution (Cloud + Local Fallbacks)** — 34 / 37 done (92%)

| # | Phase | State | Open | Done | Deferred | Cancelled | % |
|---|---|---|---:|---:|---:|---:|---:|
| 0 | Tier audit (DONE) | ⬜ empty | 0 | 0 | 0 | 0 | 0% |
| 1 | Cloud Bundle Builder | ✅ done | 0 | 7 | 1 | 0 | 100% |
| 2 | T3-H Cloud-Aware Variants | ✅ done | 0 | 7 | 0 | 0 | 100% |
| 3 | Linear AI — Rules Digest Distribution | ✅ done | 0 | 5 | 1 | 0 | 100% |
| 4 | Cloud-Aware Documentation Pass | ✅ done | 0 | 4 | 1 | 0 | 100% |
| 5 | Distribution Channels & Install Paths | ✅ done | 0 | 4 | 1 | 0 | 100% |
| 6 | Telemetry & Validation | 🟡 in progress | 3 | 7 | 3 | 0 | 70% |

### [road-to-visual-review-loop.md](roadmaps/road-to-visual-review-loop.md)

**Visual Review Loop + A11y** — 0 / 4 done (0%)

| # | Phase | State | Open | Done | Deferred | Cancelled | % |
|---|---|---|---:|---:|---:|---:|---:|
| 0 | Stub — build trigger and contract | ⬜ not started | 4 | 0 | 0 | 0 | 0% |


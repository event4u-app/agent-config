...
- **Object-oriented where it reduces complexity** — encapsulation, single responsibility, composition over inheritance, behavior-rich domain objects, and design patterns *where they earn their place* — never speculative abstraction. Bounded by [`minimal-safe-diff`](minimal-safe-diff.md) + [`senior-engineering-discipline`](senior-engineering-discipline.md): a reviewer should be able to name the concrete duplication or second axis of change the abstraction removes (rule of thumb: two real repetitions before you extract).

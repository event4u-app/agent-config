 ...
 */
export function trigger_terms(meta: Record<string, unknown>): string[] {
    const triggers = meta['triggers'];
    ...
    for (const t of triggers) {
        if (t === null || typeof t !== 'object' || Array.isArray(t)) continue;
        const obj = t as Record<string, unknown>;
        for (const key of ['keyword', 'phrase'] as const) {
            const v = obj[key];
 ...
 */
export function applies_when(description: string, meta: Record<string, unknown>, cap: number = APPLIES_WHEN_CAP): string {
    if (description.includes(APPLIES_WHEN_PREFIX.trim())) return description;
    ...
    if (kept.length === 0) return description;

...
export interface RtkDetection {
    /** An executable named `rtk` is resolvable on PATH. */
    present: boolean;
    /** Only set when present. */
    identity?: RtkIdentity;
    /** Only set when identity === 'token-killer' (from `rtk --version`). */
    version?: string;
    /** Resolved binary path (diagnostic; only set when present). */
    binPath?: string;
}
...
export function resolveBinaryOnPath(name: string, pathEnv?: string): string | null {
...
export interface ProbeResult {
    /** Process completed (spawned and exited, regardless of exit code). */
    completed: boolean;
    stdout: string;
    stderr: string;
}

type ProbeFn = (binPath: string) => ProbeResult;

function runGainProbe(binPath: string): ProbeResult {
    try {
...
function readVersion(binPath: string): string | undefined {
    try {
...
export function classifyGainOutput(probe: ProbeResult): RtkIdentity {
    if (!probe.completed) return 'unverified';
    const combined = `${probe.stdout}\n${probe.stderr}`;
    if (TOKEN_KILLER_GAIN_SIGNATURE.test(probe.stdout)) return 'token-killer';
    ...
    if (combined.trim().length > 0) return 'unknown-rtk';
    // Completed silently with no output — ambiguous, don't accuse the tool.
    return 'unverified';
}

export interface DetectRtkOptions {
    /** Override PATH (tests / isolated probes). */
    pathEnv?: string;
    /** Override the gain probe (tests). */
    probe?: ProbeFn;
    /** Override the version read (tests). */
    versionReader?: (binPath: string) => string | undefined;
}
 ...
 */
export function detectRtk(options: DetectRtkOptions = {}): RtkDetection {
    const binPath = resolveBinaryOnPath('rtk', options.pathEnv);
    if (binPath === null) return { present: false };
    const probe = options.probe ?? runGainProbe;
    const identity = classifyGainOutput(probe(binPath));
    if (identity !== 'token-killer') return { present: true, identity, binPath };
...
/* ------------------------------------------------------------------ */
/* Cached variant — for high-frequency callers (PreToolUse hook).      */
/* ------------------------------------------------------------------ */
 ...
 */
export function detectRtkCached(options: DetectRtkOptions = {}): RtkDetection {
    const binPath = resolveBinaryOnPath('rtk', options.pathEnv);
    if (binPath === null) return { present: false };
    ...
    if (stat !== null) {
        try {
            const cached = JSON.parse(readFileSync(file, 'utf-8')) as IdentityCacheEntry;
            if (
                cached.binPath === binPath &&
    ...
    if (stat !== null && detection.identity !== undefined && detection.identity !== 'unverified') {
        ...
        if (detection.version !== undefined) entry.version = detection.version;
        try {
...
/* ------------------------------------------------------------------ */
/* Per-OS install commands — verified upstream paths only.             */
/* ------------------------------------------------------------------ */

export interface RtkInstallCommands {
    /** The primary copy-and-run command for this platform. */
    recommended: string;
    /** Human-readable label for the recommended path. */
    recommendedLabel: string;
    /** Secondary/manual path (only where a packaged one may be unavailable). */
    manual?: string;
    manualLabel?: string;
    /** Caveat the UI must surface next to the command. */
    note?: string;
}
 ...
 */
export function rtkInstallCommands(platform: NodeJS.Platform = process.platform): RtkInstallCommands {
    switch (platform) {
        case 'darwin':
            return {
        ...
        case 'win32':
            return {
                ...
                note:
                    'ripgrep is a documented Windows runtime dependency: `winget install BurntSushi.ripgrep.MSVC`. ' +

...
/** What the writer should do with one planned target. */
export type DeployWriteDecision = 'write' | 'preserve';

/** Inputs to {@link decideDeployWrite}. */
export interface DeployWriteInputs {
    /** True when the target already exists on disk. */
    readonly exists: boolean;
    /**
     * The SHA-256 the install manifest recorded for this target.
     *
     * `undefined` — not recorded at all. `null` — recorded without a digest
     * (bridge entries carry `sha256: null`). Both mean this tree cannot say
     * whether the bytes on disk are ours.
     */
    readonly recordedSha256: string | null | undefined;
    /**
     * The SHA-256 of the bytes on disk, computed immediately before the caller
     * mutates the file — not at plan time.
     *
     * The planner's `ConflictEntry` list is advisory state, not a safety
     * capability: minutes can pass between planning and writing, and a file
     * edited in that window would be classified from a digest that no longer
     * describes it. Re-hashing at the write site closes that window.
     *
     * `null` when the file could not be read.
     */
    readonly onDiskSha256: string | null;
    /** True when the run carries `--force`. */
 ...
 */
export function decideDeployWrite(inputs: DeployWriteInputs): DeployWriteDecision {
...
export function sidecarPathFor(target: string): string {
 ...
 */
export function preservedFileMessage(target: string, sidecar: string): string {
    return (
        `Preserved user-modified ${target}. Package content was written to ${sidecar}; ` +
...
export function conflictSummaryMessage(count: number): string {
    const noun = count === 1 ? 'file' : 'files';
    return (
        `Completed with conflicts: ${count} user-modified ${noun} preserved. ` +
 ...
 */
export function foreignSidecarMessage(target: string, sidecar: string): string {
    return (
        `Refusing to replace ${sidecar}: it already exists with different content and this ` +

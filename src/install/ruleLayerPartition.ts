...
export interface PartitionRulesOptions {
    /** Project-relative host directory, e.g. `.cursor/rules`. */
    readonly toolDir: string;
    /** Candidate rule basenames, already scope- and manual-filtered. */
    readonly rules: readonly string[];
    /**
     * Absolute project root.
     *
     * Retained after the 2026-09-07 change removed the only reader that needed
     * it (`partitionActive`, which verified it against `installed.lock`). It
     * stays because every call site already passes it and because a rule
     * partition that cannot name the tree it is partitioning is harder to debug,
     * not because a branch below reads it. Stated rather than quietly dropped.
     */
    readonly projectRoot: string;
    /** Absolute directory the basenames resolve against for classification. */
...
export function partitionRulesForDir(opts: PartitionRulesOptions): string[] {
    ...
    if (!verdict.carries) {
        opts.announce?.(
            `  ⚠️  ${toolDir}: full projection kept — ${toolId}'s global layer ${verdict.reason}` +
...
export interface DedupOptions {
    ...
    readonly rules: readonly string[];
    readonly userHome: string;
    /** Absolute directory the basenames resolve against, for the byte compare. */
    readonly rulesSource: string;
    readonly announce?: (message: string) => void;
}

export function dedupableRules(opts: DedupOptions): Set<string> {
    ...
    for (const rule of rules) {
        ...
        try {
            if (!isFile(twin) || !isFile(source)) continue;

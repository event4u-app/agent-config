...
/**
 * Provenance gate at the write edge — invariant 2 of ADR-130 (`subject`
 * axis). A record whose `origin` resolves into the user-global store, or
 * that carries `subject: user`, is REFUSED entry into the tracked project
 * intake (plain path/field check, no model call). `subject: project` is
 * the additive default. Thrown message names the partition rule.
 */

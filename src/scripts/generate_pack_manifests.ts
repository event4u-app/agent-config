 ...
 *   - `<home>/pack.yaml` — id, label, owner, requires, artefact_count, …
...
import { generatedByComment } from './_lib/generated_by.js';
...
const GENERATED_HEADER =
    `${generatedByComment('generate_pack_manifests')}\n`;
...
export function main(argv: string[] = process.argv.slice(2)): number {
    ...
    for (const [pid, homeDir, mode] of homes) {
        ...
        const meta = _build_pack_yaml(pid, vocab, artefacts);

...
function render(): string {
    ...
    L.push('disclaimered, and still embed a wrong domain assumption. Today **0 of 20**');
    L.push('default-surface domain skills carry a sourced `domain-truth` fixture');
    L.push('(`./scripts-run src/scripts/domain_soundness_status`); all are labeled');
    L.push('`unvalidated` and the validated count is CI-ratcheted. Authoring the');
    L.push('answer keys needs domain competence (a cited method, not the skill\'s own');

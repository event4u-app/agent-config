...
function ensure_cline_bridge(project_root: string, force: boolean): void {
    ...
    for (const { slot, native } of hostBindings('cline')) {
        const target = path.join(hooks_dir, native);
...
function ensure_cline_user_hooks(package_root: string, force: boolean): void {
    ...
    for (const { slot, native } of hostBindings('cline')) {
        const wrapper = path.join(CLINE_USER_DIR, native);
        const body =
            '#!/usr/bin/env bash\n' +
            '# Generated by event4u/agent-config — DO NOT EDIT.\n' +
...
/**
 * Platforms whose bridge the smoke probe exercises, with the slot it probes.
 *
 * The NATIVE event is derived from `host_lowering.yaml`, never written here.
 * The hand-written list this replaced probed `['cursor', 'beforeShellExecution']`
 * — an event no cursor bridge has ever written — so the probe could not fail on
 * a real binding regression, which is the only thing it exists to catch. A
 * platform whose table row does not carry the probed slot is a hard error, not
 * a skip: a silent skip is how the previous list stayed wrong.
 */
const SMOKE_PROBE_SLOTS: ReadonlyArray<readonly [string, string]> = [
...
export function smokeProbeEvents(): ReadonlyArray<readonly [string, string]> {
    return SMOKE_PROBE_SLOTS.map(([platform, slot]) => {
        ...
        return [platform, binding.native] as const;
    });
}
...
const PROG = 'agent-config install';

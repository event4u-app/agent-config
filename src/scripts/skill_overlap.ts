...
// `agents/metrics/` was retired by the agents/ taxonomy consolidation, and
// `agents/reports/skill-overlap.md` is written by the successor gate
// `audit_skill_overlap` — writing there would silently clobber it. Runtime is
// the typed, gitignored home for a report nothing tracks. Reviving a gate whose
// scan root was dead also revives its writes; this one had nowhere legal to land.
const OUT = path.join(REPO, 'agents', 'runtime', 'reports', 'skill-overlap.md');
...
export function render(pairs: Pair[], total: number): string {
    ...
    const lines: string[] = [
        ...
        '',
        // The literal rather than `_lib/generated_by`'s helper, deliberately:
        // `lint_skill_originality`'s harness copies THIS FILE ALONE into a
        // synthetic tree and runs it, so a `./_lib/` import here fails to
        // resolve at runtime. The attribution is a constant, not a
        // computation, and the tree-wide invariant is held by the sweep in
        // tests/scripts/generated_by.test.ts rather than by this import.
        '> Generated by `skill_overlap`. Do not edit. Scans',
        '> `src/skills/*/SKILL.md` frontmatter (description +',
...
export function main(argv?: string[]): number {
    ...
    try {
        // A report is a certification too: writing `Skills scanned: 0` into the
        // baseline is the same lie a green gate tells, in a quieter voice.
        assertScanned({
            gate: 'skill_overlap',
        ...
        if (exc instanceof DeadScopeError) {
            // `no skills under <root>` is the pinned line; the assertion's
            // named-root detail follows rather than replaces it.

...
/**
 * Modules that contribute records to `ALLOWLIST` from OUTSIDE `tools.ts`.
 *
 * `tools.ts` sits past the source-size ceiling, so a tool family whose records
 * would cost ~250 units of excess there is defined in its own module and spread
 * into the allowlist. This generator indexed `tools.ts` alone, so the five
 * `graph_*` tools rendered `_stub-only_` in the published inventory — the doc
 * asserting the exact opposite of the catalogue's own `implemented_on: ["stdio"]`,
 * and `--check` staying green because it compared the doc against its own wrong
 * render. Found by an independent review.
 *
 * A LIST rather than a directory walk: a walk would silently pick up any future
 * file with a matching shape, and the point of this index is to say where a
 * handler actually is. Adding a family here is one line in the same change that
 * adds the family.
 */
export const HANDLER_MODULES: readonly { file: string; label: string; block: string }[] = [
    { file: path.join(ROOT, 'src/scripts/mcp_server/tools.ts'), label: 'tools.ts', block: 'export const ALLOWLIST' },
    {
        file: path.join(ROOT, 'src/scripts/mcp_server/graph_tools.ts'),
        label: 'graph_tools.ts',
        block: 'export const GRAPH_TOOLS',
    },
];
...
function _index_handlers(): Map<string, { label: string; line: number }> {
    const out = new Map<string, { label: string; line: number }>();
    for (const mod of HANDLER_MODULES) {
        let text: string;
        try {
            text = fs.readFileSync(mod.file, 'utf-8');
        } catch {
            continue;
        }
        const lines = _splitlines(text);
        // Only index entries inside the module's own registration block.
        let inBlock = false;
        for (let idx = 0; idx < lines.length; idx += 1) {
            ...
            if (m) {
                out.set(m[1] as string, { label: mod.label, line: i });
            }
        }
    }
    return out;
...
function _render(
    catalog: Catalog,
    handlers: Map<string, { label: string; line: number }>,
    cat_lines: Map<string, number>,
): string {
    ...
    lines.push('');
    // A BARE command name, and deliberately not a link to this file: an
    // independent review flagged the previous `../../audit_mcp_tools` as a
    // path that resolves nowhere and proposed `src/scripts/audit_mcp_tools.ts`.
    // That fix is refused by `generated_by.test.ts` § 1.4, which forbids a
    // generated artifact from hard-coding its generator's script path — the
    // broken link and the literal path are two wrong answers, and the
    // replacement form the rule names is the runnable command.
    lines.push('> Generated by `audit_mcp_tools`. Do not edit.');
    ...
    for (const t of tools) {
        ...
        const h_cite =
            h !== undefined
                ? `[\`${h.label}:${h.line}\`](../../src/scripts/mcp_server/${h.label}#L${h.line})`
                : '_stub-only_';
    ...
    try {
        assertWatchlistResolves({
            ...
            candidates: [
                ...
                path.posix.join('src', 'scripts', 'mcp_server', 'graph_tools.ts'),
            ],
            repoRoot: ROOT,
        });
    } catch (e) {

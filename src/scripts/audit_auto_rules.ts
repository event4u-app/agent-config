 ...
 *
 * Ported from the retired Python `src/scripts/audit_auto_rules.py` (ADR-200 —
...
import { generatedBy } from './_lib/generated_by.js';
...
export function render_markdown(rules: RuleEntry[]): string {
    ...
    const lines: string[] = [
        `<!-- analyzed: ${dateStr} | commit: ${commitShort} | files: ${rules.length} -->`,
        '# Auto-Rule Audit',
        '',
        generatedBy('audit_auto_rules', 'For Phase 5 of'),

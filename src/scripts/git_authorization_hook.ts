...
const PHRASES: ReadonlyArray<{ op: GitOp; re: RegExp }> = [
  ...
  // ── Consequence operations, added 2026-09-06.
  //
  // Every row REQUIRES ITS OBJECT NOUN, and that is the whole reason these
  // patterns are long. A bare verb cannot tell an authorization from a mention,
  // and for this class the mention is the common case: "deploy" appears in a
  // hundred sentences that authorize nothing, while "deploy nach production"
  // appears only when someone means it. The same discipline the `branch` row
  // above documents, applied where the cost of a false positive is a recorded
  // authorization for a production operation.
  //
  // German verb endings are `(e|en)?` because the imperative drops its `-e`,
  // and both word orders are covered — the house style stated above this table.
  {
    op: "prod-deploy",

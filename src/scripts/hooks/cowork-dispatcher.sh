...
let b="";process.stdin.on("data",c=>b+=c);process.stdin.on("end",()=>{let d;try{d=JSON.parse(b)}catch(e){process.exit(0)}const cwd=d&&d.cwd;if(typeof cwd==="string"&&cwd){process.stdout.write(cwd);return}const r=(d&&d.workspace_roots)||[];if(r.length)process.stdout.write(String(r[0]))});
' 2>/dev/null)"
fi

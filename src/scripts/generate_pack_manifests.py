...
def _pack_homes() -> list[tuple[str, Path, bool]]:
    """Every pack home as ``(pack_id, home_dir, is_physical)``.

    ``home_dir`` is the directory that owns the pack's ``pack.yaml`` /
    ``README.md``. ``is_physical`` is ``True`` for a legacy
    ``packages/<dir>/`` tree (artefacts collected by rglob of its
    ``.agent-src.uncondensed/``) and ``False`` for a 6.0.0-D
    ``src/domains/<pack>/`` home (artefacts collected from the flat
    ``src/`` library by frontmatter ``packs:``). A physical pack wins on
    id collision — the move deletes the physical tree, so the two homes
    never coexist for the same id post-move.
    """
    homes: list[tuple[str, Path, bool]] = []
    seen: set[str] = set()
    if PACKAGES.exists():
        for pkg in sorted(PACKAGES.iterdir()):
            ...
            homes.append((pid, pkg, True))
            seen.add(pid)
    if SRC_DOMAINS.is_dir():
        for dom in sorted(SRC_DOMAINS.iterdir()):
            if not dom.is_dir() or dom.name.startswith("_"):
                continue
            pid = dom.name
            if pid in seen:
                continue
            homes.append((pid, dom, False))
            seen.add(pid)
    return homes
...
def _collect_artefacts(pid: str, home_dir: Path, is_physical: bool) -> list[dict[str, Any]]:
    return _collect_physical(home_dir) if is_physical else _collect_flat(pid)
...
def main() -> int:
    ...
    homes = _pack_homes()
    if not homes:
        print("no pack homes (packages/ or src/domains/) — nothing to generate", file=sys.stderr)
        return 0
    drift = 0
    for pid, home_dir, is_physical in homes:
        artefacts = _collect_artefacts(pid, home_dir, is_physical)

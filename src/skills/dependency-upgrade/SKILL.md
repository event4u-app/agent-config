...
1. **Install without running scripts first** — `npm install --ignore-scripts` (npm v12 defaults to this), `composer install --no-scripts`, `pip install --no-build-isolation` — so install-time code (the #1 RCE vector) cannot run before inspection.

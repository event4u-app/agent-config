...
description: "Use when editing a module-shaped path (`Modules/*`, `packages/*`, `apps/*`, `internal/*`) while `modules.enabled` is false — asks once to enable the config; also the project/stack + task-runner detection reference."
domain: process
workspaces:

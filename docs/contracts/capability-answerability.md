...
| **Host subagent-spawn** | **undecided — blocked** | — | Depends on what an empty `host_capabilities: {}` means, which is the open `host-capability-default-flip` decision. Deciding the carry before the semantics would be recording a preference, not a decision. |

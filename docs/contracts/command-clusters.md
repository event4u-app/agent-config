...
| `research` | — | (cluster head only · Phase 2 will add `deep` · `add-items` · `add-fields`) | new — preliminary-research scaffolder ported from `Weizhena/Deep-Research-skills`; emits `outline.yaml` + `fields.yaml` against the `research-schema` contract. Phase 2 of [`road-to-deep-research-adoption.md`](../../agents/roadmaps/road-to-deep-research-adoption.md) ports the deep-research and incremental-edit sub-commands. |

...
| `team-knowledge` | — | `consolidate` | new cluster 2026-07-05 (`road-to-knowledge-system` Phase 5) — consolidation gate over the typed knowledge-observation intake (`convention_detected` / `mistake_made` / `api_shape_learned` / `context_stale`) into committed `agents/knowledge/{sessions,concepts,procedures,decisions}/` pages. Distinct from the pre-existing [`knowledge`](#locked-clusters) cluster (arbitrary local-file ingestion into `agents/memory/knowledge/`) — different namespace, different concern. `:consolidate` reads pending events (`consolidate_knowledge_events.ts`), finds the nearest existing page per topic (mechanical similarity), and presents a NEW/EXTEND/CONFIRM/CONFLICT batch for human approval — never writes a page automatically. `:bootstrap` planned (Phase 6). |

...
| `rules_applied` | string[] | Stable rule ids the run actually carried this phase. Bounded to ≤ 32; remainder dropped silently. **`[]` is an honest absence and the ONLY way absence is spelled here** — unlike `skills_applied` below, this field is mandatory on every line, so there is no absent-key slot to distinguish "not recorded" from "none applied". **Cutover 2026-09-07:** until that date both shipped producers wrote the fixed literal `['delegation-policy']`, and the prose in this row before that read "rule ids whose Iron Law fired this phase" — false for every line either producer had ever written. The claim was deleted rather than softened and replaced by a check (`isProducerConstantField`, `src/scripts/_lib/audit_field_provenance.ts`), because a contract that describes a constant as an observation makes every downstream rate an artifact that looks like a finding. The producers now call `appliedIds` in the same module and the registration row is gone, so the check returns `false` for this field. **A reader spanning the cutover MUST segment on `ts`.** Lines are append-only: everything written before 2026-09-07 still carries the constant, and mining the real stream still surfaces it — `extract_audit_patterns --min-count 2` minted exactly one pattern, `implement:success:delegation-policy`, at count 914 over 935 lines when the defect was found and 1074 over 1104 when it was fixed. The retired reading is preserved in `agents/knowledge/experience-rules-applied-is-a-producer-constant.md`, which is marked retired rather than deleted for exactly this reason. |

...
|---|---|---|---:|
| **selection accuracy** | [`scripts/bench_runner.py`](../../src/scripts/bench_runner.ts) | Keyword-overlap ranker hits the expected skill in top-K | % |
...
| **projection fidelity** | [`scripts/bench_per_tool.py`](../../src/scripts/bench_per_tool.ts) | `accuracy(tool) / accuracy(augment)` for skill-projecting tools | ratio |
...
[`scripts/bench_drift_check.py`](../../src/scripts/bench_drift_check.ts)
...
[`scripts/bench_baseline_ready.py`](../../src/scripts/bench_baseline_ready.ts)

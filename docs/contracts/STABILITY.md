...
The link checker (`src/scripts/check_public_links.ts`, P0.1b) reads this
frontmatter and:
...
and is generated by `src/scripts/check_public_links.ts --list`.
...
1. **Commit-annotation layer (`release.ts`).** `infer_bump` reads the

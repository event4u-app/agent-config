...
<!-- BEGIN generated: host-standing-cost -->

### Standing rule cost, per host

Generated by `./scripts-run src/scripts/generate_host_cost_table` from `agents/evidence/analysis/standing-payload-by-host-2026-09.md`, pinned to `beccf605c0000ba7b2839039c9fa5af1356df999`. Do not hand-edit between the markers.

The unit is the projection **source** minus the ADR-004 `type: manual` rules no per-tool tree receives — the upper bound a host loads on an unscoped, un-deduplicated install. Every scope narrowing (user-scope dedup, workspace and pack scope) moves a host DOWN from these figures, never up. **This measures delivery, not consumption:** whether a host reads what it is handed is the axis `docs/enforcement-by-host.md` owns.

| Host | Surface | Shape | Bytes | chars/4 tok |
|---|---|---|---:|---:|
| `augment` | `.augment/rules` | per-rule tree | 485474 | 121369 |
| `claude-code` | `.claude/rules` | per-rule tree | 485474 | 121369 |
| `cline` | `.clinerules` | per-rule tree | 485474 | 121369 |
| `copilot` | `.github/copilot-instructions.md` | single file | 7491 | 1873 |
| `cowork` | — | none | — | — |
| `cursor` | `.cursor/rules` | per-rule tree | 485474 | 121369 |
| `gemini` | `GEMINI.md` | single file | 2982 | 746 |
| `windsurf` | `.windsurfrules` | single file | 255783 | 63946 |
| `codex` | `.codex/agent-config.md` | single file | absent | absent |

To measure what **one concrete install** loads rather than the upper bound, run `./scripts-run src/scripts/check_preamble_payload_budget --host <id>`. That reading is informational and never gated, and it prints a `PARTIAL TREE` warning when the host tree holds fewer rule files than the source — which a maintainer checkout always does.

<!-- END generated: host-standing-cost -->

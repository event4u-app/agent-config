...
5. **TypeScript CLI shell** — the `agent-config` binary is now a compiled TS entry point (`dist/cli/agent-config.js`) that handles native commands (`versions`, `doctor-shell`, `ui:serve`) and forwards every other subcommand to the legacy Bash dispatcher at `scripts/_dispatch.bash`. The shell also embeds a Fastify-based local server bound to `127.0.0.1` for the in-browser UI track. Decision: [`docs/decisions/ADR-012-typescript-cli-shell.md`](decisions/ADR-012-typescript-cli-shell.md). Wire shape + security stance: [`local-server-api.md`](contracts/local-server-api.md) (beta).
...
| Layer | Count | Purpose |
|---|---|---|
| **Skills** | 292 | On-demand expertise — stack analysis (Laravel · Symfony · Zend / Laminas · Next.js · React · Node), testing, Docker, API design, security, observability, … |
| **Rules** | 119 | Always-active constraints — coding standards, scope control, verification, language-and-tone, agent-authority |
| **Commands** | 201 | Slash-command workflows — `/commit`, `/create-pr`, `/fix ci`, `/optimize skills`, `/feature plan`, `/work`, `/implement-ticket`, `/condense`, … |
| **Guidelines** | 109 | Reference material cited by skills — PHP patterns, Eloquent, Playwright, agent-infra, … |

...
the transcripts are regenerated by `node node_modules/.bin/tsx tests/golden/capture.ts` and
...
```bash
node node_modules/.bin/tsx tests/golden/capture.ts --scenarios GT-1 GT-P1 GT-U2 GT-2
```

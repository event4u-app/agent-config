# CHANGELOG — 6.0.0 release overview (archived)

> Drained from `CHANGELOG.md` `## [Unreleased]` on 2026-07-21. This was the
> hand-authored "6.0.0 at a glance" release overview, staged under
> `[Unreleased]` during 6.0.0 readiness and never removed after the release
> shipped. It is preserved here verbatim for history; `[Unreleased]` now
> starts clean. The dated per-version 6.0.0-era entries live under the
> `# Era: pre-7.0.0 — archived` pointer in `CHANGELOG.md`.

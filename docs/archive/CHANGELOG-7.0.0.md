# Changelog Archive — 7.0.0

> Frozen full entry for the `7.0.0` release of `event4u/agent-config`,
> split out of the main [`CHANGELOG.md`](../../CHANGELOG.md) to keep the
> active `7.0.x` era under the 250-line drift cap enforced by
> `tests/lib/changelog_eras.test.ts`. `7.0.0` was a large catch-up
> release; its full per-commit detail lives here.
>
> **Read-only.** Git tag `7.0.0` remains the canonical record of what
> shipped. Entry shape follows

...
| 279 skills. | quant | `src/scripts/check_artefact_count_messaging.ts#Artefact-count messaging gate` | ✅ |
...
as good as its measurement. Today **42 of 279** skills carry a behavioural
`evals.json`; the highest-traffic / highest-cost tiers (default-surface +
`rich` + routers) are **fully covered (35 of 35)**, the long tail
(7 of 244) is not. We publish that gap rather than imply
"279 evaluated skills": coverage is measured per tier

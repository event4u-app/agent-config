...
| 280 skills. | quant | `src/scripts/check_artefact_count_messaging.ts#Artefact-count messaging gate` | ✅ |
...
**26 backed claim(s)** — all evidence pointers resolve in CI.
...
as good as its measurement. Today **42 of 280** skills carry a behavioural
`evals.json`; the highest-traffic / highest-cost tiers (default-surface +
`rich` + routers) are **fully covered (35 of 35)**, the long tail
(7 of 245) is not. We publish that gap rather than imply
"280 evaluated skills": coverage is measured per tier

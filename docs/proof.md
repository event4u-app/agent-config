...
| 290 skills. | quant | `exec:check_artefact_count_messaging -> 0` | ✅ |
...
We also publish our **debt**: 12 claim(s) are logged as
...
as good as its measurement. Today **42 of 290** skills carry a behavioural
`evals.json`; the highest-traffic / highest-cost tiers (default-surface +
`rich` + routers) are **fully covered (35 of 35)**, the long tail
(7 of 255) is not. We publish that gap rather than imply
"290 evaluated skills": coverage is measured per tier
...
**Axis 2 — evidence form per public claim.** 64 ledger entries · 47 backed · 12 unbacked inventory · 5 resolved-null.

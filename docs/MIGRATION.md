...
| `code_graph` native code-graph engine (`code_graph.enabled`, the `code-intelligence` skill's native arm, `code_graph_nudge_hook`; runtime paths `src/scripts/code_graph/`, `src/scripts/hooks/code_graph_nudge_hook.ts`) | 2026-07-28 | next major after 9.x | the major after that | ⚠️ **OVERDUE BY ONE MAJOR, and recorded here rather than only reported.** The removal resolves to **11.0**; `package.json` shipped **12.0.0**, so the commitment was missed — by exactly the decay this table exists to prevent, since nothing checked it until `lint_scheduled_deprecations` did. **Removal owner: the maintainer** (`### blocker: code-graph-removal-authorisation`); it is a public-surface change gated by `downstream-changes` and `scope-control` and lands at a major cut, so recording the slip here is not authorisation to act on it. A consumer case the graph answers and disciplined grep cannot. Measured null: recall 0.365 vs grep 0.797 (`docs/CLAIMS.md` `code-graph-retrieval-null`). Until then `enabled: false` is permanent. **Its parser pair (`web-tree-sitter@0.24.7` / `tree-sitter-wasms@0.1.13`, ~51 MB unpacked) moved from `dependencies` to `devDependencies` ahead of the schedule, so consumers no longer install it — the runtime paths (CLI leaf, nudge hook, skill, rule) stay registered until the removal date above, but re-enabling now also means installing that ABI-locked pair yourself.** |

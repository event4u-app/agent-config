...
| Hook-dispatch latency | `npm run build:hooks && ./scripts-run src/scripts/bench_hook_latency --gate` | p50 76–83 ms · p95 81–89 ms per event on the gating CI runner (budget: pre_tool_use p95 ≤ 150 ms, any event ≤ 250 ms; a darwin dev machine measures ~10 ms faster) | 50 invocations/event of `node dist/hooks/dispatch.js` with a synthetic payload, temp workspace, replay mode; recorded in [`hook-latency.json`](hook-latency.json) FROM the CI environment (the regression window compares like-for-like); budget + >20% regression gated in CI. Was ~1.6 s p50 via the retired CLI→bash→tsx→per-concern-tsx chain. |

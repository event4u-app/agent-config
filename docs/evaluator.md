...
| Hook-dispatch latency | `npm run build:hooks && ./scripts-run src/scripts/bench_hook_latency --gate` | p50 67–80 ms · p95 69–83 ms per event (budget: pre_tool_use p95 ≤ 150 ms, any event ≤ 250 ms) | 50 invocations/event of `node dist/hooks/dispatch.js` with a synthetic payload, temp workspace, replay mode; recorded in [`hook-latency.json`](hook-latency.json); budget + >20% regression gated in CI. Was ~1.6 s p50 via the retired CLI→bash→tsx→per-concern-tsx chain. |

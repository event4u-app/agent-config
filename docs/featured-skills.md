...
A curated subset of the 299 skills. Three tiers, one per primary audience. Full catalog lives in [`dist/agent-src/skills/`](../dist/agent-src/skills/); see also [`docs/catalog.md`](catalog.md) for the complete index and [`docs/featured-commands.md`](featured-commands.md) for command-level highlights.
...
CI runs this in `taskfiles/ci-fast.yml`. Stale entries (renamed / removed skill or command) fail the build. See [`scripts/lint_featured_skills.py`](../src/scripts/lint_featured_skills.ts) for the matcher.

→ Browse all 299 skills: [`dist/agent-src/skills/`](../dist/agent-src/skills/) · all 203 commands: [`dist/agent-src/commands/`](../dist/agent-src/commands/)

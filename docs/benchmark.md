...
> ⚠️ **SUPERSEDED (v1, interim).** The numbers below are the **v1 binary-capability**
> result — they came back a flat ~100% in every arm because a single pass/fail
> oracle on a capable host measures *capability* (shared by both arms), not the
> package's *discipline*. A council scrapped that frame; **v2 rebuilds around a
> discipline axis** — see `agents/roadmaps/road-to-discipline-axis-benchmark.md`.
> This file is regenerated with the v2 two-table format once the v2 pilot runs.

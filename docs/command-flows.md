...
**183 commands** across **10 surfaces** (4 user-work flows + 6 platform/domain surfaces).
...
- `team` — Team orchestrator — cross-model depth review (one strong model builds, a second reviews the real diff); rou…
...
- `team-status` — Thin wrapper — plugin job status via /codex:status plus a ledger line with today's cli_call_budget openai c…

...
- `analyze-repo` — Analyze an external reference repository (competitor, inspiration, peer) and produce a structured compariso…
...
- `review` — Review orchestrator — routes to changes (seven-judge self-review of the local diff) and routing (compute re…
- `review-changes` — Self-review local changes before creating a PR — dispatches to seven specialized judges (bug, security, tes…
...
- `git-pr-merge` — Prepare one open PR to mergeable, or the whole open-PR queue with `all`, then merge what the invocation nam…

...
Loaded by [`roadmap-process-loop § 3`](roadmap-process-loop.md) when
resolved roadmap declares `execution.mode` in frontmatter
(`autonomous` | `phase-checkpoints`; absent / `interactive` = legacy
behavior, this context stays unloaded). Defines the run-start
pre-scan, the contract summary user confirms ONCE, the grants that
...
at authoring time. Grants nothing. Authorization happens exactly once
per run: user invokes `/roadmap:process-*` (a this-turn action), loop
derives the contract below from roadmap's CURRENT content, and user's
single confirmation converts declared mode into this-turn grants
**cached for this run only** (same mechanism as commit pre-scan it
extends). Stale frontmatter is harmless: no confirmation, no grants.
...
2. **Git-shape needs** — steps requiring a branch / push / PR
   ...
   run's own delivery needs when roadmap's definition of done includes
   shipping (autonomous mode default, § 4).
3. **Artifact-authoring steps** — steps that create or materially
   ...
   `src/{skills,rules}/`, command/guideline trees). Feed the batched
...
4. **Open questions / ambiguity** — explicit `?` markers, "decide
   whether", options left unresolved, or steps matching
...
Accept = ONE authorization of the run. Cache all grants; never re-ask
inside the run. Adjust → apply, re-show once. Contract re-derived on
every invocation — always reflects roadmap's and repo's current state.
...
Grants satisfy [`scope-control`](../../rules/scope-control.md)'s
...
If roadmap contains **no** delivery need and no commit steps, git
grants are omitted from the contract (nothing to authorize —

...
Run the [`brand-to-tokens`](../../skills/brand-to-tokens/SKILL.md) skill — brand
decisions → a DTCG `.tokens.json` source of truth → `design-tokens` emits CSS
vars + Tailwind (no Node). The same `.tokens.json` is the export pack-ai-image's
brand-asset generation and the greenfield scaffold seed consume; stays on
`$value`/`$type` so Tokens Studio / Style Dictionary round-trip. Args:
`"<identity ref>"`.

...
projection, routing or lint behaviour is testable** — the clause a reader tries
to escape with *the artefact is markdown, so there is nothing to test*. Markdown
is not the discriminator. A rule whose triggers decide what loads, a projection
whose output a gate reads and a lint whose verdict flips are executable contracts
in a prose extension, each with a test that fails before the change, passes after.

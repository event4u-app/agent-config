...
A COMMENT EXPLAINING WHERE THE CODE CAME FROM — A ROADMAP PATH, A PHASE AND
STEP, A PROTOTYPE OR REFERENCE ARTEFACT — IS THAT COPY, IN THE WRONG PLACE.
```

The provenance of a value is not a WHY, and a roadmap cannot license a comment
this rule forbids. Measured case, both enforcers, and why the keyword triggers
miss the write: [`code-clarity § Comment discipline`](../docs/guidelines/code-clarity.md#comment-discipline--state-a-constraint-not-a-narration).

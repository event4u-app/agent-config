...
**Blockers follow same cadence as checkboxes.** Clearing a `## Blockers` entry (per [`templates/roadmaps.md` rule 20](../templates/roadmaps.md)) flips its `Status: resolved`, regenerates dashboard in same reply — Iron Law 1's "same response" obligation applies to blocker resolution exactly as it applies to a checkbox flip.

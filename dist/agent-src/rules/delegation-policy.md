...
5. **Respect the failure-type stop and the ordering gate** — two consecutive
   ...
   session (an application of the N=3 budget, no new mechanism); an ordered
   slice never dispatches before its declared parent's return is verified.
   Both per [`subagent-steering`](../contexts/execution/subagent-steering.md).
6. **Record** telemetry after each dispatch — run `orchestration_record`
   ...
   § Emit. Agent-behavioral capture the savings report reads; skipping it leaves
   the value-of-orchestration question unmeasured.

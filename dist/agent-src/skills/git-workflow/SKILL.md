...
- **"Based on main" / "current"** → prove it with
  `./scripts-run src/scripts/check_branch_freshness`; exit `1` ⇒ the branch is
  behind and is **not** current — merge the base in, regenerate the derived
  files, then open the PR (see [`/create-pr`](../../commands/pr/create.md) § 1b).

...
**Design-system export (current-repo source):** to seed `DESIGN.md` from *this*
repo, emit the inventory as a `design-system.json` artifact (the import contract
in [`design-system-capture`](../design-system-capture/reference/design-system-json.md))
and hand it to `design-system-capture` — same shape an external tool produces,
so the import path is uniform.

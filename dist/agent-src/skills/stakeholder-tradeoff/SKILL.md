...
- The trade-off is purely technical (perf vs storage, sync vs async)
  — route to [`decision-record`](../decision-record/SKILL.md); one costly
  choice on ≥ 3 options with user-weighted criteria → its § Weighted-matrix
  mode (this skill answers WHO pays; that mode answers WHICH option wins).

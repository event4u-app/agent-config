...
Toolchains differ per language and project, often unknown to the agent —
...
2. **Concrete CI failure** — remote pipeline reports a failing check;
   ...
   the canonical flow).
3. **New-gate carve-out** — change itself introduces a NEW CI gate,
   smoke test, or test file; must run once locally to be proven.

`quality.local_auto_run: true` restores legacy autonomous behaviour
(run pipeline when work is ready for verification). When runs are
suppressed, completion message says *"quality gates delegated to
...
- `verify-before-complete` rule — no pass claims without fresh output; suppressed runs surfaced as "delegated to remote CI", never claimed

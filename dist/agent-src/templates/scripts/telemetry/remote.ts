...
/** `rule_loading_tier` values from the settings template. */
export const ALLOWED_RULE_TIERS = ['minimal', 'balanced', 'full', 'custom'] as const;
...
/**
 * Pseudonymous per-session id. Domain-separated from the user hash by the
 * literal prefix, so the same salt cannot make the two digests collide or
 * let one be derived from the other.
 */
export function derive_session_hash(salt: string, session_id: string): string {
...
export function build_class_a_record(input: BuildClassAInput): ClassARecord {
    ...
    check_id_redaction('host', input.host);
    check_id_redaction('org_id', input.org_id);

    const rule_tier = input.rule_tier ?? null;
    if (rule_tier !== null && !(ALLOWED_RULE_TIERS as readonly string[]).includes(rule_tier)) {
        throw new ClassARecordError(
            `rule_tier must be one of ${ALLOWED_RULE_TIERS.join(', ')} or null`,
        );
    }

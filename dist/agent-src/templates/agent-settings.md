...
| `knowledge.global_sharing.enabled` | `true`, `false` | `false` | **User-global** (keep in `~/.event4u/agent-config/agent-settings.yml`). Master switch for the file-first global knowledge-card store (ADR-100; default-off per ADR-101 / Evidence v2 Phase 0 until cross-project reuse is measured). `false` fully no-ops the layer; v1 project-local cards unaffected. |

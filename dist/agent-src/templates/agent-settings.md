...
| `worktrees.mode` | `off`, `on`, `ask` | `on` | Controls autonomous `git worktree` usage. `on` = standing permission (skill skips the per-creation ask; ignore-check and clean-baseline gates still apply). `ask` = `scope-control` permission gate runs every time. `off` = skill refuses unless the user explicitly asks for a worktree that turn (then it runs); `subagent-orchestration` mode 6 falls back to mode 3. |

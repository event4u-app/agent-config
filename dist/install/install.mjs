...
function read_lockfile(path17) {
  const target = path17 ?? lockfile_path();
  let text;
  try {
...
function read_manifest(path17) {
  let text;
  try {
    text = require_read_text(path17);
  } catch {
    return null;
  }
...
function require_read_text(path17) {
  return fs8.readFileSync(path17, { encoding: "utf-8" });
}
...
function write_manifest(path17, version, tools, options = {}) {
  const rendered = _render2(version, tools, { deploy_roots: options.deploy_roots ?? null });
  return write_atomic(path17, rendered);
}
...
function _walk_skill_files(skill_dir) {
  const out = [];
  const resolved = fs10.realpathSync(skill_dir);
  const walk = (root, relRoot) => {
    const dirNames = [];
    const fileNames = [];
    for (const dirent of fs10.readdirSync(root, { withFileTypes: true })) {
      const full = path9.join(root, dirent.name);
    ...
    for (const fname of fileNames) {
      ...
      out.push([path9.join(root, fname), rel_parts]);
    }
    for (const dname of dirNames) {
      if (_is_excluded([dname])) {
        continue;
      }
      walk(path9.join(root, dname), [...relRoot, dname]);
    }
  };
  walk(resolved, []);
...
function _manifest_digest(files) {
  ...
  for (const [abs_path, rel_parts] of files) {
    ...
    h.update(crypto2.createHash("sha256").update(fs10.readFileSync(abs_path)).digest());
    h.update(Buffer.from([0]));
  }
  return h.digest("hex");
}
function _atomic_write_zip(zip_path, files) {
  fs10.mkdirSync(path9.dirname(zip_path), { recursive: true });
  const stem = path9.basename(zip_path, path9.extname(zip_path));
  const tmp_path = path9.join(
    path9.dirname(zip_path),
    `.${stem}.${crypto2.randomBytes(6).toString("hex")}.zip.tmp`
  );
  try {
    const entries = files.map(([abs_path, rel_parts]) => ({
      name: rel_parts.join("/"),
      data: fs10.readFileSync(abs_path)
    }));
    fs10.writeFileSync(tmp_path, zip_write_sync(entries));
    fs10.renameSync(tmp_path, zip_path);
  } finally {
    if (fs10.existsSync(tmp_path)) {
      fs10.unlinkSync(tmp_path);
    }
  }
}
function _write_if_changed(dest_dir, slug, files, force, written) {
  const digest = _manifest_digest(files);
  const zip_path = path9.join(dest_dir, `${slug}.zip`);
  const digest_path = path9.join(dest_dir, `${slug}.sha256`);
  const recorded = fs10.existsSync(digest_path) ? fs10.readFileSync(digest_path, "utf-8").trim() : "";
  if (!force && recorded === digest && fs10.existsSync(zip_path)) {
    return;
  }
  _atomic_write_zip(zip_path, files);
  fs10.writeFileSync(digest_path, digest + "\n", "utf-8");
  written.push(zip_path);
}
function build_skill_bundles(package_root, dest_dir, force = false, curation = null) {
  const skills_root = path9.join(package_root, "dist/agent-src", "skills");
  if (!_is_dir(skills_root)) {
    return [];
  }
  fs10.mkdirSync(dest_dir, { recursive: true });
  const written = [];
  for (const name of fs10.readdirSync(skills_root).sort()) {
    const entry = path9.join(skills_root, name);
    const isSymlink2 = fs10.lstatSync(entry).isSymbolicLink();
    ...
    const skill_md = path9.join(entry, "SKILL.md");
    if (!fs10.existsSync(skill_md)) {
      continue;
    }
...
function _command_slug(source_file, commands_root) {
  const rel = path9.relative(commands_root, source_file);
  const noExt = rel.slice(0, rel.length - path9.extname(rel).length);
  return noExt.split(path9.sep).join("-");
}
function _iter_command_files(commands_root) {
  const found = [];
  const walk = (dir) => {
    for (const dirent of fs10.readdirSync(dir, { withFileTypes: true })) {
      const full = path9.join(dir, dirent.name);
  ...
  return found.filter((source_file) => path9.basename(source_file) !== "AGENTS.md");
}
function build_command_bundles(package_root, dest_dir, force = false, curation = null) {
  const commands_root = path9.join(package_root, "dist/agent-src", "commands");
  ...
  if (_is_dir(skills_root)) {
    skill_names = new Set(
      fs10.readdirSync(skills_root).filter((name) => _is_dir(path9.join(skills_root, name)))
    );
  }
  fs10.mkdirSync(dest_dir, { recursive: true });
  const written = [];
  for (const source_file of _iter_command_files(commands_root)) {
    ...
    const files = [[fs10.realpathSync(source_file), ["SKILL.md"]]];
...
function atomicWriteFile(target, data, options = {}) {
  ...
  const tmp = join10(parent, `.tmp.${process.pid}.${randSuffix()}`);
  let fd = null;
  try {
...
function build_claude_hook_matrix(manifest_path2) {
  const raw = fs11.readFileSync(manifest_path2, "utf8");
...
function _read_settings(settings_path) {
  ...
  const raw = fs11.readFileSync(settings_path, "utf8");
  if (raw.trim() === "") return {};
  let parsed;
  try {
...
function _with_lock(settings_path, fn) {
  ...
  try {
    ...
    fs11.closeSync(fd);
    fs11.rmSync(lockPath, { force: true });
  }
}
...
function _local_settings_path(project_root) {
  return path11.join(project_root, ...LOCAL_PROJECT_SUBDIR, LOCAL_PROJECT_FILE);
}
function _canonical_settings_path(project_root) {
  return path11.join(project_root, ...LOCAL_PROJECT_SUBDIR, DEFAULT_PROJECT_FILE);
}
...
function _resolve(p) {
  const absolute = path11.resolve(p);
  try {
...
function _boundary_anchor_at(dir) {
  const agents_dir = path11.join(dir, "agents");
  if (_is_dir2(agents_dir)) {
    for (const marker of _AGENTS_DIR_MARKERS) {
      if (_exists(path11.join(agents_dir, marker))) {
        return ANCHOR_AGENTS_DIR;
      }
    }
  }
  if (_exists(path11.join(dir, ".git"))) {
    return ANCHOR_GIT;
  }
  return null;
}
function find_project_root_with_anchor(start) {
  ...
  if (legacy) {
    for (const candidate of chain) {
      if (_exists(path11.join(candidate, ".git"))) {
  ...
  let outermost = null;
  for (const candidate of chain) {
    if (_exists(path11.join(candidate, DEFAULT_PROJECT_FILE))) {
      outermost = candidate;
    }
  }
  if (outermost !== null) {
...
function _resolve_cascade_paths(cwd, project_path) {
  if (cwd === null) {
    ...
    const parent = path11.dirname(legacy);
  ...
  chain.reverse();
  return [
    ...chain.map((d) => path11.join(d, DEFAULT_PROJECT_FILE)),
...
function _read_yaml(p) {
  ...
  let data;
  try {
    const text = fs12.readFileSync(p, "utf-8");
...
function rule_in_scope(source_path, scope, pack_scope = null, role_scope = null) {
  ...
  const [meta] = parseFrontmatter(fs13.readFileSync(source_path, "utf-8"));
  ...
  hooks: external_exports.object({
    ...
    turn_end_gate: external_exports.object({
      ...
      promissory: external_exports.boolean().default(true).describe(
        "Detector A of the turn-end gate: refuse a turn whose closing paragraph promises work not yet performed while asking the user nothing (20 measured occurrences). Fires on 9.3% of turns over a 30-session corpus (24 of 259). Inert while hooks.turn_end_gate.enabled is false."
      ),
      language: external_exports.boolean().default(true).describe(
        "Detector B of the turn-end gate: refuse a turn whose user-visible prose is not in the pinned language, with fenced code, inline code, quoted tool output, tables, URLs, paths and identifiers excluded first (19 measured occurrences). Fires on 15.1% of turns over a 30-session corpus (39 of 259) \u2014 measured per turn, over the last assistant text, which is the population the gate actually inspects. That rate is why the master switch is off by default. Inert while hooks.turn_end_gate.enabled is false."
      )
    }).default({})
  }),
...
function _list_module_subdirs(root) {
  ...
  const out = [];
  for (const name of entries) {
    if (!_isDir(path13.join(root, name))) {
      continue;
    }
    if (name.startsWith(".")) {
      continue;
...
function _score_confidence(stack, root, subdirs) {
  ...
  if (stack === "node-monorepo") {
    const withPkgJson = subdirs.filter((name) => _isFile(path13.join(root, name, "package.json")));
    return withPkgJson.length > 0 ? "high" : "medium";
  }
  if (stack === "python-src") {
    const withInit = subdirs.filter((name) => _isFile(path13.join(root, name, "__init__.py")));
...
function read_model_tier(skill_md) {
  if (!fs15.existsSync(skill_md)) return null;
  const text = fs15.readFileSync(skill_md).toString("utf-8");
  if (!text.startsWith("---\n")) return null;
  const end = text.indexOf("\n---\n", 4);
...
function resolvePackageRoot(fromUrlOrPath, legacyHops = 3) {
  ...
  for (let i = 0; i < MAX_ASCENT; i++) {
    ...
    if (fs16.existsSync(manifest)) {
      try {
        const parsed = JSON.parse(fs16.readFileSync(manifest, "utf8"));
  ...
  return path14.resolve(startDir, ...Array.from({ length: legacyHops }, () => ".."));
}
...
function _readText(p) {
  return fs17.readFileSync(p, { encoding: "utf-8" });
}
function _writeText(p, text) {
  fs17.writeFileSync(p, text, { encoding: "utf-8" });
}
...
function _detect_legacy_state(project) {
  return _isFile2(path15.join(project, LEGACY_STATE_FILENAME));
}
function _detect_legacy_settings(project) {
  const found = [];
  for (const name of LEGACY_SETTINGS_FILES) {
    const flat = path15.join(project, name);
    if (_isFile2(flat)) {
      found.push(flat);
    }
    const typed = path15.join(project, "settings", name);
    if (_isFile2(typed)) {
      found.push(typed);
    }
  }
...
function _detect_empty_shell(project) {
  const shell = path15.join(project, LEGACY_AGENT_CONFIG_SHELL);
  if (!_isDir2(shell) || _isSymlink(shell)) {
    return false;
  }
  try {
    return fs17.readdirSync(shell).length === 0;
  } catch {
    return false;
  }
}
function _detect_already_migrated(project) {
  if (_detect_npm(path15.join(project, "package.json"))) {
    return false;
  }
  if (_detect_composer(path15.join(project, "composer.json"))) {
    return false;
  }
  for (const name of MANAGED_SYMLINKS) {
    if (_classify_symlink(path15.join(project, name)) === "legacy") {
      return false;
    }
  }
...
function _purge_legacy_symlinks(project) {
  ...
  const preserved = [];
  for (const name of MANAGED_SYMLINKS) {
    const link = path15.join(project, name);
    const kind = _classify_symlink(link);
    if (kind === "legacy") {
      try {
        fs17.unlinkSync(link);
        removed.push(name);
      } catch {
        preserved.push(name);
      }
...
function _migrate_state_file(project) {
  const source = path15.join(project, LEGACY_STATE_FILENAME);
  if (!_isFile2(source)) {
    return null;
  }
  const target = path15.join(project, LEGACY_STATE_V1_FILENAME);
  if (_exists2(target)) {
    try {
      fs17.unlinkSync(source);
...
function _load_state_migrator() {
  ...
  const driver = [path15.join(pkg_root, "dist", rel), path15.join(pkg_root, "src", rel)].find(
    (p) => fs17.existsSync(p)
  ) ?? null;
  if (driver === null) {
    return null;
  ...
  for (let dir = pkg_root; ; ) {
    const cand = path15.join(dir, "node_modules", ".bin", binName);
    if (fs17.existsSync(cand)) {
      tsxBin = cand;
      break;
    }
...
function _delete_legacy_settings(project) {
  ...
  for (const p of _detect_legacy_settings(project)) {
    try {
      fs17.unlinkSync(p);
      removed.push(path15.relative(project, p));
    } catch {
      continue;
    }
  }
  const settings_dir = path15.join(project, "settings");
  if (_isDir2(settings_dir) && !_isSymlink(settings_dir)) {
    try {
      if (fs17.readdirSync(settings_dir).length === 0) {
        fs17.rmdirSync(settings_dir);
...
function _update_gitignore(project) {
  const gitignore = path15.join(project, ".gitignore");
...
function _build_plan(project) {
  return {
    npm: _detect_npm(path15.join(project, "package.json")),
    composer: _detect_composer(path15.join(project, "composer.json")),
    symlinks_legacy: MANAGED_SYMLINKS.filter(
      (name) => _classify_symlink(path15.join(project, name)) === "legacy"
    ),
    symlinks_user: MANAGED_SYMLINKS.filter(
      (name) => _classify_symlink(path15.join(project, name)) === "user"
    ),
    state_file: _isFile2(path15.join(project, LEGACY_STATE_FILENAME)),
    settings_files: _detect_legacy_settings(project).map((p) => path15.relative(project, p)),
...
function _apply(project, out, err) {
  const summary = [];
  if (_strip_npm_entry(path15.join(project, "package.json"))) {
    summary.push(`removed ${PACKAGE_NAME_NPM} from package.json`);
  }
  if (_strip_composer_entry(path15.join(project, "composer.json"))) {
...
function expanduser6(p) {
  ...
  if (p.startsWith("~/") || p.startsWith("~\\")) {
    return path16.join(os7.homedir(), p.slice(2));
  }
  return p;
}
function resolvePath(p) {
  try {
    return fs18.realpathSync(path16.resolve(p));
  } catch {
...
function isSymlink(p) {
  try {
    return fs18.lstatSync(p).isSymbolicLink();
  } catch {
    return false;
  }
}
function readText(p) {
  return fs18.readFileSync(p, "utf-8");
}
function writeText(p, content) {
  fs18.writeFileSync(p, content, "utf-8");
}
function mkdirp(p) {
  fs18.mkdirSync(p, { recursive: true });
}
function sortedGlobStems(directory, suffix) {
  let entries;
  try {
    entries = fs18.readdirSync(directory);
  } catch {
    return [];
  }
  const stems = [];
...
function countZips(directory) {
  ...
  let n = 0;
  for (const name of fs18.readdirSync(directory)) {
...
function atomicWrite0644(target, body, prefix) {
  ...
  const tmpName = path16.join(
    dir,
    `${prefix}${process3.pid}.${crypto3.randomBytes(6).toString("hex")}.yml.tmp`
  );
  let fd = null;
  try {
    fd = fs18.openSync(tmpName, "wx", 420);
    fs18.writeFileSync(fd, body, "utf-8");
    fs18.closeSync(fd);
    fd = null;
    fs18.chmodSync(tmpName, 420);
    fs18.renameSync(tmpName, target);
  } catch (err) {
    if (fd !== null) {
      try {
        fs18.closeSync(fd);
...
function _canonical_settings_target(project_root) {
  return path16.join(project_root, ...SETTINGS_SUBDIR, SETTINGS_FILE);
}
function _resolve_settings_read(project_root) {
  ...
  const legacy = path16.join(project_root, SETTINGS_FILE);
...
function detect_package_root(project_root) {
  const npm_path = path16.join(project_root, "node_modules", "@event4u", "agent-config");
  if (isDir(npm_path)) return resolvePath(npm_path);
  if (pathExists(path16.join(project_root, "src", "config", "profiles", "minimal.ini"))) {
    return project_root;
  }
  fail(
...
function detect_package_type(package_root) {
  if (package_root.split(path16.sep).includes("node_modules")) return "npm";
  return "local";
}
function detect_package_type_for_project(project_root, package_root) {
  const npm_path = resolvePath(
    path16.join(project_root, "node_modules", "@event4u", "agent-config")
  );
...
function write_file(p, content) {
  ensure_directory(path16.dirname(p));
  writeText(p, content);
}
...
function _migrate_legacy_if_present(project_root, template_body) {
  const legacy_target = path16.join(project_root, LEGACY_SETTINGS_FILE);
  ...
  const backup_target = path16.join(project_root, LEGACY_BACKUP_FILE);
  writeText(backup_target, legacy_text);
  fs18.unlinkSync(legacy_target);
...
function _load_valid_user_types(package_root) {
  const directory = path16.join(package_root, USER_TYPES_DIR);
...
function ensure_agent_settings(project_root, package_root, profile, force, user_type = "", packs = null) {
  ...
  const profile_source = path16.join(package_root, "src", "config", "profiles", `${profile}.ini`);
  const template_source = path16.join(package_root, "src", "config", "agent-settings.template.yml");
  ...
  if (profile_values["rule_loading_tier"] !== profile) {
    ...
    fail(
      `Profile preset ${path16.basename(profile_source)} has rule_loading_tier=${got} but --profile=${profile}`
    );
  }
  ...
  const legacy_root = path16.join(project_root, SETTINGS_FILE);
  if (isFile(legacy_root) && !pathExists(target)) {
    mkdirp(path16.dirname(target));
    writeText(target, readText(legacy_root));
    fs18.unlinkSync(legacy_root);
...
function ensure_augment_bridge(project_root, force) {
  ...
  return merge_json_file(
    path16.join(project_root, ".augment", "settings.json"),
    bridge,
    force,
    ".augment/settings.json"
  );
}
var AUGMENT_USER_DIR = path16.join(os7.homedir(), ".augment");
var AUGMENT_USER_HOOKS_DIR = path16.join(AUGMENT_USER_DIR, "hooks");
...
function _deploy_augment_trampoline(package_root, name, force) {
  const src = path16.join(package_root, "scripts", "hooks", name);
  if (!pathExists(src)) {
    skip(`augment trampoline missing in package: ${src}`);
    return null;
  }
  mkdirp(AUGMENT_USER_HOOKS_DIR);
  const dst = path16.join(AUGMENT_USER_HOOKS_DIR, name);
...
function _remove_legacy_augment_trampolines() {
  for (const name of AUGMENT_LEGACY_TRAMPOLINES) {
    const legacy = path16.join(AUGMENT_USER_HOOKS_DIR, name);
    try {
      if (isFile(legacy)) {
...
function ensure_augment_user_hooks(package_root, force) {
  ...
  return merge_json_file(
    path16.join(AUGMENT_USER_DIR, "settings.json"),
    settings_patch,
    force,
    "~/.augment/settings.json"
  );
...
function ensure_claude_bridge(project_root, force) {
  const target = path16.join(project_root, ".claude", "settings.json");
...
function ensure_cursor_bridge(project_root, force) {
  ...
  return merge_json_file(
    path16.join(project_root, ".cursor", "hooks.json"),
    bridge,
    force,
    ".cursor/hooks.json"
  );
}
var CURSOR_USER_DIR = path16.join(os7.homedir(), ".cursor");
var CURSOR_USER_HOOKS_DIR = path16.join(CURSOR_USER_DIR, "hooks");
var CURSOR_DISPATCHER_TRAMPOLINE = "cursor-dispatcher.sh";
function ensure_cursor_user_hooks(package_root, force) {
  const src = path16.join(package_root, "scripts", "hooks", CURSOR_DISPATCHER_TRAMPOLINE);
  if (!pathExists(src)) {
    skip(`cursor trampoline missing in package: ${src}`);
    return [];
  }
  mkdirp(CURSOR_USER_HOOKS_DIR);
  const dst = path16.join(CURSOR_USER_HOOKS_DIR, CURSOR_DISPATCHER_TRAMPOLINE);
  const src_text = readText(src);
  if (pathExists(dst) && readText(dst) === src_text && !force) {
  ...
  return merge_json_file(
    path16.join(CURSOR_USER_DIR, "hooks.json"),
    settings_patch,
    force,
    "~/.cursor/hooks.json"
  );
...
var CLINE_USER_DIR = path16.join(os7.homedir(), "Documents", "Cline", "Hooks");
var CLINE_DISPATCHER_TRAMPOLINE = "cline-dispatcher.sh";
function ensure_cline_user_hooks(package_root, force) {
  const src = path16.join(package_root, "scripts", "hooks", CLINE_DISPATCHER_TRAMPOLINE);
  if (!pathExists(src)) {
    skip(`cline trampoline missing in package: ${src}`);
    return;
  }
  mkdirp(CLINE_USER_DIR);
  const trampoline = path16.join(CLINE_USER_DIR, CLINE_DISPATCHER_TRAMPOLINE);
  const src_text = readText(src);
...
function ensure_windsurf_bridge(project_root, force) {
  ...
  const bridge = { hooks };
  return merge_json_file(
    path16.join(project_root, ".windsurf", "hooks.json"),
    bridge,
    force,
    ".windsurf/hooks.json"
  );
}
var WINDSURF_USER_DIR = path16.join(os7.homedir(), ".codeium", "windsurf");
var WINDSURF_USER_HOOKS_DIR = path16.join(WINDSURF_USER_DIR, "hooks");
var WINDSURF_DISPATCHER_TRAMPOLINE = "windsurf-dispatcher.sh";
function ensure_windsurf_user_hooks(package_root, force) {
  const src = path16.join(package_root, "scripts", "hooks", WINDSURF_DISPATCHER_TRAMPOLINE);
  if (!pathExists(src)) {
    skip(`windsurf trampoline missing in package: ${src}`);
    return [];
  }
  mkdirp(WINDSURF_USER_HOOKS_DIR);
  const dst = path16.join(WINDSURF_USER_HOOKS_DIR, WINDSURF_DISPATCHER_TRAMPOLINE);
  const src_text = readText(src);
  if (pathExists(dst) && readText(dst) === src_text && !force) {
  ...
  return merge_json_file(
    path16.join(WINDSURF_USER_DIR, "hooks.json"),
    settings_patch,
    force,
...
function ensure_gemini_bridge(project_root, force) {
  ...
  return merge_json_file(
    path16.join(project_root, ".gemini", "settings.json"),
    bridge,
    force,
    ".gemini/settings.json"
  );
}
var GEMINI_USER_DIR = path16.join(os7.homedir(), ".gemini");
var GEMINI_USER_HOOKS_DIR = path16.join(GEMINI_USER_DIR, "hooks");
var GEMINI_DISPATCHER_TRAMPOLINE = "gemini-dispatcher.sh";
function ensure_gemini_user_hooks(package_root, force) {
  const src = path16.join(package_root, "scripts", "hooks", GEMINI_DISPATCHER_TRAMPOLINE);
  if (!pathExists(src)) {
    skip(`gemini trampoline missing in package: ${src}`);
    return [];
  }
  mkdirp(GEMINI_USER_HOOKS_DIR);
  const dst = path16.join(GEMINI_USER_HOOKS_DIR, GEMINI_DISPATCHER_TRAMPOLINE);
  const src_text = readText(src);
  if (pathExists(dst) && readText(dst) === src_text && !force) {
  ...
  return merge_json_file(
    path16.join(GEMINI_USER_DIR, "settings.json"),
    settings_patch,
    force,
    "~/.gemini/settings.json"
  );
...
function ensure_copilot_bridge(project_root, force) {
  const target = path16.join(project_root, ".github", "plugin", "marketplace.json");
  const bridge = {
    marketplace: {
...
function ensure_roocode_bridge(project_root, force) {
  const target = path16.join(project_root, ".roo", "rules", "agent-config.md");
  if (pathExists(target) && !force) {
...
function ensure_claude_desktop_bridge(project_root, force) {
  const target = path16.join(project_root, ".claude-desktop", "agent-config.md");
  if (pathExists(target) && !force) {
...
function ensure_aider_bridge(project_root, force) {
  const target = path16.join(project_root, ".aider", "agent-config.md");
  if (pathExists(target) && !force) {
...
function ensure_codex_bridge(project_root, force) {
  const target = path16.join(project_root, ".codex", "agent-config.md");
  if (pathExists(target) && !force) {
...
function ensure_continue_bridge(project_root, force) {
  const target = path16.join(project_root, ".continue", "rules", "agent-config.md");
  if (pathExists(target) && !force) {
...
function ensure_kilocode_bridge(project_root, force) {
  const target = path16.join(project_root, ".kilocode", "rules", "agent-config.md");
  if (pathExists(target) && !force) {
...
function ensure_zed_bridge(project_root, force) {
  const target = path16.join(project_root, ".zed", "agent-config.md");
  if (pathExists(target) && !force) {
...
function ensure_jetbrains_bridge(project_root, force) {
  const target = path16.join(project_root, ".jetbrains", "agent-config.md");
  if (pathExists(target) && !force) {
...
function ensure_kiro_bridge(project_root, force) {
  const target = path16.join(project_root, ".kiro", "steering", "agent-config.md");
  if (pathExists(target) && !force) {
...
function dirHasEntries(p) {
  try {
    return fs18.readdirSync(p).length > 0;
  } catch {
    return false;
  }
}
function _resolve_tsx_invocation(scriptPath, scriptArgs) {
  ...
  let dir = path16.dirname(scriptPath);
  for (; ; ) {
    const candidate = path16.join(dir, "node_modules", ".bin", binName);
    if (isFile(candidate)) {
...
function _smoke_test_hooks(project_root, package_root) {
  const dispatcher = path16.join(package_root, "scripts", "hooks", "dispatch_hook.ts");
  const manifest = path16.join(package_root, "scripts", "hook_manifest.yaml");
  ...
  for (const [platform, native] of SMOKE_PROBE_EVENTS) {
    ...
    const bridge_path = rel_bridge ? path16.join(project_root, rel_bridge) : null;
...
var GLOBAL_ROOT = path16.join(os7.homedir(), ".event4u", "agent-config");
var GLOBAL_USER_SETTINGS_PATH = path16.join(GLOBAL_ROOT, ".agent-user.yml");
var GLOBAL_AGENT_SETTINGS_PATH = path16.join(GLOBAL_ROOT, ".agent-settings.yml");
...
function _suppress_rule_layer(project_root, suppress_dir) {
  const target = path16.join(project_root, ".claude", "settings.local.json");
  const existing = pathExists(target) ? read_json_file(target) : {};
  let real_dir = suppress_dir;
  try {
    real_dir = fs18.realpathSync(suppress_dir);
  } catch {
...
function _gate_rule_layer_overlap(project_root, layer, dry_run) {
  const global_dir = path16.join(os7.homedir(), ".claude", "rules");
  const project_dir = path16.join(project_root, ".claude", "rules");
...
function detect_scope(cwd) {
  ...
  const has_manifest = SCOPE_DETECT_MANIFESTS.find((m) => pathExists(path16.join(cwd, m))) ?? null;
  const has_ai_dir = SCOPE_DETECT_AI_DIRS.find((d) => isDir(path16.join(cwd, d))) ?? null;
  const has_ai_file = SCOPE_DETECT_AI_FILES.find((f) => pathExists(path16.join(cwd, f))) ?? null;
...
function readLineSyncRaw(promptText) {
  ...
  for (; ; ) {
    let n;
    try {
      n = fs18.readSync(0, buf, 0, 1, null);
    } catch (err) {
      const code = err.code;
      if (code === "EAGAIN") {
        continue;
...
function _files_by_tool_from_bridges(tools, project_root, scope) {
  ...
  for (const tool_id of [...tools].sort()) {
    ...
    if (!path16.isAbsolute(marker_path)) {
      marker_path = path16.join(project_root, marker_path);
    }
...
function _update_installed_tools_manifest(project_root, tools, scope, force, files_by_tool = null, merged_keys_by_tool = null) {
  ...
  if (!state.QUIET) {
    const rel = isRelativeTo(target, project_root) ? path16.relative(project_root, target) : target;
...
function _resolve_package_root_for_global() {
  ...
  const candidate = path16.dirname(path16.dirname(path16.dirname(here)));
  if (!pathExists(path16.join(candidate, "src", "config", "profiles", "minimal.ini"))) {
    fail(
...
var CONSUMER_BRIDGE_MARKER_RELPATH = path16.join("agents", ".event4u-bridge.yml");
...
function _is_agent_config_source_repo(project_root) {
  ...
  const pkg_json = path16.join(project_root, "package.json");
  if (isFile(pkg_json)) {
    let data = {};
    try {
  ...
  if (isDir(path16.join(project_root, ".agent-src.uncondensed"))) {
    return [true, ".agent-src.uncondensed/"];
  }
  const packages_dir = path16.join(project_root, "packages");
  if (isDir(packages_dir)) {
    for (const child of fs18.readdirSync(packages_dir)) {
      if (isDir(path16.join(packages_dir, child, ".agent-src.uncondensed"))) {
        return [true, `packages/${child}/.agent-src.uncondensed/`];
      }
    }
  }
  const installer_self = path16.join(project_root, "scripts", "install.py");
  try {
...
function _detect_legacy_for_migration(project_root) {
  ...
  if (isFile(path16.join(project_root, INSTALL_MODE_MARKER_REL))) return [];
  const found = [];
  for (const name of MIGRATE_LEGACY_YAML_FILES) {
    if (isFile(path16.join(project_root, name))) {
      found.push(name);
    } else if (isFile(path16.join(project_root, "settings", name))) {
      found.push(`settings/${name}`);
    }
  }
  for (const name of MIGRATE_LEGACY_TOOL_DIRS) {
    const p = path16.join(project_root, name);
...
function _prompt_migrate_to_global(project_root, artefacts) {
  if (!state.QUIET) {
    ...
    for (const rel of artefacts) {
      info(`  ${path16.join(project_root, rel)}`);
    }
...
function _format_global_root_for_marker(global_root) {
  ...
  const rel = path16.relative(home, resolved);
...
function _remove_legacy_consumer_bridge_marker(project_root, env = null) {
  ...
  if (isDir(path16.join(project_root, ".agent-src.uncondensed"))) return null;
  const target = path16.join(project_root, CONSUMER_BRIDGE_MARKER_RELPATH);
...
function _write_per_tool_project_anchors(project_root, tools, env = null, now = null) {
  ...
  if (isDir(path16.join(project_root, ".agent-src.uncondensed"))) return [];
  ...
  for (const tool_id of Object.keys(PROJECT_ANCHOR_TOOLS).sort()) {
    ...
    const target = path16.join(project_root, rel_path);
    mkdirp(path16.dirname(target));
...
function _inject_package_tag(target, source, package_root) {
  if (path16.extname(target) !== ".md") return;
  let text;
  try {
  ...
  if (source !== null) {
    ...
    if (package_root !== null) {
      const rel = path16.relative(resolvePath(package_root), resolved_src);
      if (rel !== "" && !rel.startsWith("..") && !path16.isAbsolute(rel)) {
        source_value = rel;
      } else {
...
function _escapes_package_root(resolved, package_root) {
  ...
  let root;
  try {
    root = fs18.realpathSync(package_root);
  } catch {
    return false;
  }
...
function _copy_dir_dereferencing_symlinks(src, dest, force, package_root = null, file_filter = null) {
  ...
  if (!isDir(src)) {
    ...
    try {
      resolved_src = fs18.realpathSync(src);
    } catch {
      resolved_src = src;
    }
  ...
  const walk = (node) => {
    const acc = [];
    const names = fs18.readdirSync(node).sort((a, b) => a < b ? -1 : a > b ? 1 : 0);
    for (const name of names) {
      const entry = path16.join(node, name);
      acc.push(entry);
      const lst = fs18.lstatSync(entry);
      if (lst.isDirectory() && !lst.isSymbolicLink()) {
        acc.push(...walk(entry));
      }
    }
  ...
  for (const entry of walk(src)) {
    const rel = path16.relative(src, entry);
    const target = path16.join(dest, rel);
    const lst = fs18.lstatSync(entry);
    if (lst.isDirectory() && !lst.isSymbolicLink()) {
      mkdirp(target);
      continue;
    ...
    let resolved = entry;
    try {
      resolved = fs18.realpathSync(entry);
      resolvedIsDir = fs18.statSync(entry).isDirectory();
    } catch {
      resolvedIsDir = false;
    }
    ...
    mkdirp(path16.dirname(target));
    fs18.copyFileSync(resolved, target);
...
function _write_claude_desktop_marker(_force, lockfile_path2, bundles_dir, bundle_count) {
  ...
  const target = path16.join(anchor, "agent-config.md");
  mkdirp(anchor);
...
function _apply_claude_flat_command_wrappers(anchor, package_root, current_files) {
  ...
  const collisions = [];
  const reserved = [];
  const commands_dir = path16.join(anchor, "commands");
  let flat_entries = [];
  try {
    flat_entries = fs18.readdirSync(commands_dir).filter((f) => f.endsWith(".md"));
  } catch {
  }
  for (const fname of flat_entries.sort()) {
    const slug = fname.slice(0, -".md".length);
    if (!is_claude_builtin_name(slug)) continue;
    fs18.rmSync(path16.join(commands_dir, fname), { force: true });
  ...
  try {
    const manifest = JSON.parse(
      fs18.readFileSync(path16.join(package_root, "dist", "discovery", "discovery-manifest.json"), "utf8")
    );
  ...
  for (const slug of [...eligible].sort()) {
    ...
    const flat_abs = path16.join(anchor, "commands", `${slug}.md`);
    if (!fs18.existsSync(flat_abs)) continue;
    const skill_dir = path16.join(anchor, "skills", slug);
    if (fs18.existsSync(skill_dir)) {
      collisions.push(slug);
      continue;
    }
    let body = fs18.readFileSync(flat_abs, "utf8");
    if (body.startsWith("---\n") && !/^name:/m.test(body.split("\n---")[0] ?? "")) {
      body = body.replace("---\n", `---
name: ${slug}
`);
    }
    fs18.mkdirSync(skill_dir, { recursive: true });
    fs18.writeFileSync(path16.join(skill_dir, "SKILL.md"), body, "utf8");
    fs18.rmSync(flat_abs, { force: true });
...
function _deploy_global_content(tools, force, package_root, lockfile_path2) {
  ...
  for (const tool_id of [...tools].sort()) {
    ...
    for (const [src_rel, dest_sub] of plan) {
      const src = path16.join(package_root, src_rel);
      const dest = dest_sub ? path16.join(anchor, dest_sub) : anchor;
    ...
    if (tool_id === "claude-code") {
      try {
        ...
        const res = ensure_managed_hooks(
          path16.join(anchor, "settings.json"),
          matrix
        );
        if (!state.QUIET) {
          if (res.changed) {
...
function _verify_deploy_targets(anchor, plan) {
  ...
  for (const [, dest_sub] of plan) {
    const target = dest_sub ? path16.join(anchor, dest_sub) : anchor;
    ...
    try {
      const entries = fs18.readdirSync(target);
...
function _prune_modules_by(deploy_results, is_pruned) {
  ...
  for (const tool_id of Object.keys(deploy_results)) {
    ...
    for (const p of paths) {
      const parts = p.split(path16.sep);
      if (parts.includes("skills")) {
        const i = parts.indexOf("skills");
        if (i + 1 < parts.length) {
          const skill_root = parts.slice(0, i + 2).join(path16.sep);
          if (!pruned_skill_dirs.has(skill_root)) {
            const skillmd = path16.join(skill_root, "SKILL.md");
    ...
    for (const p of paths) {
      ...
      if (parts.includes("skills")) {
        const i = parts.indexOf("skills");
        if (i + 1 < parts.length && pruned_skill_dirs.has(parts.slice(0, i + 2).join(path16.sep))) {
          is_target = true;
        }
      } else if (parts.includes("commands") && path16.extname(p) === ".md" && is_pruned(p)) {
        is_target = true;
      }
      (is_target ? delete_files : keep).push(p);
    }
    for (const d of pruned_skill_dirs) {
      fs18.rmSync(d, { recursive: true, force: true });
    }
    for (const p of delete_files) {
      if (p.split(path16.sep).includes("commands") && pathExists(p)) {
        try {
          fs18.unlinkSync(p);
        } catch {
        }
...
function _resolve_global_settings_path() {
  ...
  const canonical = path16.join(root, "settings", SETTINGS_FILE);
  if (pathExists(canonical)) return canonical;
  const legacy = path16.join(root, SETTINGS_FILE);
  if (pathExists(legacy)) return legacy;
  return null;
}
...
`);
  ...
  if (project_root !== null && pathExists(_resolve_settings_read(project_root)) && !isDir(path16.join(project_root, ".agent-src.uncondensed"))) {
...
`
  ...
  if (project_root !== null && pathExists(_resolve_settings_read(project_root)) && !isDir(path16.join(project_root, ".agent-src.uncondensed"))) {
    ...
    if (removed_marker !== null && !state.QUIET) {
      const rel = isRelativeTo(removed_marker, project_root) ? path16.relative(project_root, removed_marker) : removed_marker;
    ...
    if (anchor_paths.length > 0 && !state.QUIET) {
      for (const p of anchor_paths) {
        const rel = isRelativeTo(p, project_root) ? path16.relative(project_root, p) : p;
...
var SETTINGS_SURFACE_REL = path16.join("state", "settings-surface.json");
var SETTINGS_DELTA_REL = path16.join("state", "settings-delta.json");
...
function _write_settings_surface_snapshot(installed_version) {
  const root = event4u_root();
  const surface_path = path16.join(root, SETTINGS_SURFACE_REL);
  const delta_path = path16.join(root, SETTINGS_DELTA_REL);
  const next = _current_settings_surface(installed_version);
  let previous = null;
  try {
    const parsed = JSON.parse(fs18.readFileSync(surface_path, "utf8"));
  ...
  if (previous !== null && previous.version !== next.version) {
    ...
    if (delta.changes.length > 0) {
      fs18.mkdirSync(path16.dirname(delta_path), { recursive: true, mode: 448 });
      fs18.writeFileSync(delta_path, `${JSON.stringify(delta, null, 2)}
`, { mode: 384 });
      if (!state.QUIET) {
        const counts = {};
  ...
  fs18.mkdirSync(path16.dirname(surface_path), { recursive: true, mode: 448 });
  fs18.writeFileSync(surface_path, `${JSON.stringify(next, null, 2)}
`, { mode: 384 });
}
...
function _minimal_templates_root() {
  ...
  for (const ancestor of chain) {
    const candidate = path16.join(ancestor, "src", "templates", "minimal");
...
function _write_install_mode_marker(project_root, mode) {
  ...
  const marker = path16.join(project_root, INSTALL_MODE_MARKER_REL);
  try {
    mkdirp(path16.dirname(marker));
...
function install_minimal(target_root_in, force, user_type = "") {
  ...
  mkdirp(target_root);
  const parent = path16.dirname(target_root);
  ...
  const settings_src = path16.join(templates, SETTINGS_FILE);
  const overrides_gitkeep_src = path16.join(templates, "overrides-gitkeep");
  const overrides_readme_src = path16.join(templates, "agents-overrides-readme.md");
  ...
  const overrides_root = path16.join(target_root, "agents", "overrides");
  mkdirp(overrides_root);
  const gitkeep_body = readText(overrides_gitkeep_src);
  for (const sub of ["rules", "skills", "commands"]) {
    const sub_dir = path16.join(overrides_root, sub);
    mkdirp(sub_dir);
    const gitkeep_dst = path16.join(sub_dir, ".gitkeep");
  ...
  const readme_dst = path16.join(overrides_root, "README.md");
...
personal:
  user_type: ${user_type}
`;
      mkdirp(path16.dirname(settings_dst));
  ...
  if (removed_marker !== null) {
    const rel = isRelativeTo(removed_marker, target_root) ? path16.relative(target_root, removed_marker) : removed_marker;
...
function run_interactive_init(project_root, force) {
  ...
  const target = path16.join(project_root, _LOCAL_CONFIG_FILE);
  if (pathExists(target) && !force) {
    warn(
  ...
  success(`Wrote ${path16.relative(project_root, target)} (${user_type} / ${stack} / ${verbosity})`);
  return 0;
}
...
function _wizard_cli_dist(_project_root) {
  const package_root = path16.dirname(path16.dirname(path16.dirname(resolvePath(_HERE2))));
  const cli = path16.join(package_root, "dist", "cli", "agent-config.js");
  return pathExists(cli) ? cli : null;
}
function _server_info_path() {
  return path16.join(os7.homedir(), ".event4u", "agent-config", "local-server.json");
}
...
function _wizard_run_sync(cmd, env, cli) {
  ...
  let log_fd = null;
  try {
    log_fd = fs18.openSync(log_path, "w");
...
function main2(argv) {
  ...
  if (opts.minimal) {
    ...
    const minimal_package_root = path16.dirname(
      path16.dirname(path16.dirname(_minimal_templates_root()))
    );
  ...
  const is_first_run = !pathExists(path16.join(project_root, SETTINGS_FILE));
...
function finalize_claude_model_tiers(project_root) {
  const claude_skills = path16.join(project_root, ".claude", "skills");
  const augment_skills = path16.join(project_root, ".augment", "skills");
  if (!isDir(claude_skills) || !isDir(augment_skills)) return 0;
  if (_read_consumer_auto_switch(project_root) !== "auto") return 0;
  let rendered = 0;
  const entries = fs18.readdirSync(claude_skills).sort((a, b) => a < b ? -1 : a > b ? 1 : 0);
  for (const name of entries) {
    const entry = path16.join(claude_skills, name);
    const src_dir = path16.join(augment_skills, name);
    const src_md = path16.join(src_dir, "SKILL.md");
    let tier;
    try {
    ...
    if (isSymlink(entry) || isFile(entry)) {
      fs18.unlinkSync(entry);
    } else if (isDir(entry)) {
      fs18.rmSync(entry, { recursive: true, force: true });
    }
    mkdirp(entry);
    const srcFiles = fs18.readdirSync(src_dir).sort((a, b) => a < b ? -1 : a > b ? 1 : 0);
    for (const fname of srcFiles) {
      if (fname === "SKILL.md") {
        writeText(
          path16.join(entry, "SKILL.md"),
          render_native_model_md(readText(src_md), tier)
        );
      } else {
        fs18.symlinkSync(
          path16.join("../../../.augment/skills", name, fname),
          path16.join(entry, fname)
        );
      }
    }
    rendered += 1;
...
function _main_project_install(opts, project_root, parsed_tools, is_first_run) {
  ...
  if (opts.package) {
    ...
    if (!pathExists(path16.join(package_root, "src", "config", "profiles", "minimal.ini"))) {
...
function _resolvedArgv1() {
  ...
  try {
    return fs18.realpathSync(path16.resolve(process3.argv[1]));
  } catch {
    return path16.resolve(process3.argv[1]);
  }
}

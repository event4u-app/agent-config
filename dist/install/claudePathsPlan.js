 ...
 */
export function derive_trigger_globs(meta) {
 ...
 */
export function _is_unresolved_placeholder(pattern) {
 ...
 */
export function _escape_claude_bracket(pattern) {
 ...
 */
export function _expanded_pattern_count(pattern) {
    let n = 1;
    for (const m of pattern.matchAll(/\{([^}]*)\}/g)) {
        n *= m[1].split(',').length;
    }
 ...
 */
export function _has_non_path_trigger(meta) {
    const triggers = meta['triggers'];
    if (!Array.isArray(triggers))
        return false;
    for (const t of triggers) {
        if (t === null || typeof t !== 'object' || Array.isArray(t))
            continue;
        const obj = t;
        if (typeof obj['keyword'] === 'string' && obj['keyword'])
            return true;
        if (typeof obj['phrase'] === 'string' && obj['phrase'])
            return true;
    }
    return false;
}
 ...
 * path-scoped at all.
 */
export function _claude_paths_plan(meta) {
...
//# sourceMappingURL=claudePathsPlan.js.map
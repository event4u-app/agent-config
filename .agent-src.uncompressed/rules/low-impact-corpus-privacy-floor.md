...
type: "auto"
tier: "1"
description: "Use when writing, editing, or upstreaming entries in `agents/low-impact-decisions.md` — non-bypassable privacy floor for the low-impact learning corpus."
source: package
triggers:
...
2. **Upstream gate** — every `/learn-low-impact` PR draft, before
   the diff leaves the repo (Phase 12 § Step 5).
...
- `.agent-src.uncompressed/data/low-impact-decisions-seed.md` —
  upstream seed in the agent-config package.

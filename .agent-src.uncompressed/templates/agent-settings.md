...
| `hooks.chat_history.enabled` | `true`, `false` | `true` | Register the chat-history hooks (`session_start` auto-adopt, `append`, `halt_append`). Gated by **both** this flag AND `chat_history.enabled`; either off → no chat-history hook registers. |

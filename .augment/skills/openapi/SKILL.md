...
1. **Gather context** — read `agents/docs/controller.md` for OpenAPI patterns, `agents/contexts/api-versioning.md` for versioning, check 2-3 existing controllers for annotation style.
2. **Detect tooling** — check `composer.json` for `l5-swagger`/`laravel-openapi`, look for `@OA\` vs `#[OA\` syntax, find config file.
3. **Write annotations** — add `#[OA\...]` attributes to controller method. Include path, summary, tags, all parameters, all response codes (200, 401, 403, 404, 422).
4. **Define schemas** — create or reuse `#[OA\Schema]` for request/response types. Use `$ref` for shared types.
5. **Validate** — run spec validation (`npx @redocly/cli lint` or `php artisan l5-swagger:generate`). Fix errors.
6. **Verify accuracy** — compare documented request/response with actual controller + FormRequest + Resource.

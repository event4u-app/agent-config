...
`lint_command_tiers.py` (ADR-092 — every command declares a valid `tier:` +
...
tier/visibility would silently pass CI — the exact gap ADR-092 closed.
"""
...
def test_tier_visibility_lints_wired_in_skill_lint_ci() -> None:
    ...
    assert not missing, (
        ...
        f"workflow: {missing}. They must run in CI on every PR (ADR-092) so a "

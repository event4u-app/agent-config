...
def seed_state(workspace: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "ui_review": {
            "findings": [],
            "review_clean": True,
            # Render-capable stack (react-shadcn): seed render evidence so the
            # greenfield-scaffold Phase-1 preview gate stays a no-op and this
            # golden keeps pinning the apply dispatch, not the render gate.

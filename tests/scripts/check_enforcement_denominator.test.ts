...
describe('check_enforcement_denominator — what must NOT be a finding', () => {
    ...
    it("the resolver's own generated projection, when it carries the split", () => {
        const body =
...
describe('check_enforcement_denominator — the projection owes the split', () => {
    // WHY THIS BLOCK EXISTS. The generated-file exemption settles WHERE the
    // number may be published. It said nothing about whether the published
    ...
    // restored, it exits 0 and the file is byte-identical (sha256 compared).

    it('reds on a projection that states `undeclared` bare', () => {
        const found = scanGenerated(
            'docs/proof.md',
            '120 rules · 15 blocking · 86 undeclared (no enforced_by yet).\n',
        );
        expect(found).toHaveLength(1);
        expect(found[0]?.kind).toBe('missing-split');
    });

    it('greens on the same line once the split is printed', () => {
        expect(
            scanGenerated(
                'docs/proof.md',
                '120 rules · 15 blocking · 86 undeclared — of which 9 kernel-denied.\n',
    ...
    it('reds on the prose restatement shape too — `Undeclared rules (N)`', () => {
        expect(scanGenerated('docs/proof.md', 'Undeclared rules (86) carry no row.\n')).toHaveLength(
            1,
        );
        expect(
            scanGenerated('docs/proof.md', 'Undeclared rules (86, of which 9 kernel-denied) carry no row.\n'),
        ).toEqual([]);
    });

    it('does not demand the split beside a figure that is not `undeclared`', () => {
        // Deliberately narrow: `blocking` and `observer` have no split to print,
        // and a wider net would turn every generated enforcement line into a finding.
        expect(
            scanGenerated('docs/proof.md', '120 rules · 16 blocking (13.3%) · 10 observer.\n'),
        ).toEqual([]);
    });

    it('never checks the split VALUE — presence only, same reason as the count', () => {
        // A value comparison here would be a second place the number lives, which
        // is the plurality this gate exists to prevent.
        expect(
            scanGenerated('docs/proof.md', '86 undeclared — of which 9999 kernel-denied.\n'),
        ).toEqual([]);
    });

    it('the live projection carries the split', () => {
        const proof = fs.readFileSync(join(REPO_ROOT, 'docs', 'proof.md'), 'utf-8');
        expect(isGenerated(proof)).toBe(true);
        expect(scanGenerated('docs/proof.md', proof)).toEqual([]);
        expect(/\bundeclared\b/i.test(proof)).toBe(true);
    });
});

...
function runPy(args: string[]): Run {
    const r = runPyScript('council_cli', args, { cwd: REPO_ROOT });
...
// clients. argv: <question-file> [rounds] [debate]
...
qfile = sys.argv[1]
rounds_env = os.environ.get("CC_ROUNDS")
debate_env = os.environ.get("CC_DEBATE")
ns_args = ["estimate", qfile]
if debate_env == "1":
    ns_args.append("--debate")
if rounds_env:
    ns_args += ["--rounds", rounds_env]
...
const rounds = process.env.CC_ROUNDS ? Number(process.env.CC_ROUNDS) : null;
const debate = process.env.CC_DEBATE === '1';
const args = {
    cmd: 'estimate',
    question: qfile,
    input_mode: 'prompt',
...
function runHarnessPy(qfile: string, env: NodeJS.ProcessEnv): Run {
    const r = spawnSync('python3', [HARNESS_PY, qfile], {
        cwd: REPO_ROOT,
        encoding: 'utf8',
        env: { ...pyEnv(), ...env },
    });
...
function runHarnessTs(qfile: string, env: NodeJS.ProcessEnv): Run {
    const r = spawnSync(TSX_BIN, [HARNESS_TS, qfile], {
        cwd: REPO_ROOT,
        encoding: 'utf8',
        env: { ...process.env, ...env },
    });
...
beforeAll(() => {
    ...
    HARNESS_PY = path.join(TMP, 'est_harness.py');
    fs.writeFileSync(HARNESS_PY, PY_HARNESS);
...
describe.runIf(py3)('council_cli replay — byte-parity', () => {
    ...
    it('replay --low-impact-stats with no resolutions log alongside', () => {
        const py = runPy(['replay', RESP_PLAIN, '--low-impact-stats']);
...
describe.runIf(py3)('council_cli output-path validation — byte-parity', () => {
    it('render --output outside agents/runtime/council/sessions → exit 2', () => {
        ...
        const py = runPy(['render', RESP_PLAIN, '--output', badOut]);
...
describe.runIf(py3)('council_cli render --output (canonical dir) — byte-parity', () => {
    ...
    it('render --output writes byte-identical files + identical stdout', () => {
        ...
        try {
            ...
            created.push(path.join(REPO_ROOT, pyRel), path.join(REPO_ROOT, tsRel));
            expect(py.status).toBe(0);
            expect(ts.status).toBe(0);
            ...
            // Written file bodies must be byte-identical.
            const pyBody = fs.readFileSync(path.join(REPO_ROOT, pyRel), 'utf8');
            const tsBody = fs.readFileSync(path.join(REPO_ROOT, tsRel), 'utf8');
            expect(tsBody, 'written file byte-parity').toBe(pyBody);
        } finally {
            // Restore: delete only the files we created; if the dir did not
...
describe.runIf(py3)('council_cli estimate cost-output — byte-parity (injected members)', () => {
    ...
    it('estimate --debate --rounds 2 · round-by-round projection', () => {
        const env = { CC_DEBATE: '1', CC_ROUNDS: '2' };
        const py = runHarnessPy(QUESTION, env);
        const ts = runHarnessTs(QUESTION, env);
        expect(ts.status, 'exit code').toBe(py.status);
        expect(ts.stdout, 'stdout byte-parity').toBe(py.stdout);
        expect(ts.stderr, 'stderr byte-parity').toBe(py.stderr);

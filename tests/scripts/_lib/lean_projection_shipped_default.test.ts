 ...
 * the shipped-state guard for ADR-267.
 *
 ...
 *     `reopen_policy: owner` — `docs/decisions/ADR-267-delivery-default-for-claude-code.md:2-8`;
 ...
 * red here and is sent to ADR-267 first — which is the same job this file has
 * always had.
 */
...
describe('lean_projection.mode — shipped default is delivery for claude-code only', () => {
    ...
    it('the template scopes the flip to claude-code and to no other host', () => {
        ...
        // ADR-267's whole claim is per-host. A widening edit here is the one
    ...
    it('the in-code default has NOT been moved — an absent key still means eager-all', () => {
        // ADR-267 § Decision point 4 flips the shipped TEMPLATE and deliberately
    ...
    it('every generated host tree still carries full rule bodies, not pointers', () => {
        ...
        // a host ADR-267 does not cover — both are things to look at, not to
        // adjust away.

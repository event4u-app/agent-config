...
describe('guard_release_branch_push — refuses before any remote state, and only there', () => {
  const MARKED = [
    ...
    '',
    // The measured mix line, in the shape the writer renders it (ADR-261: one
    // line, no written answer). This fixture stands for a section, not a head.
...
describe('guard_release_curation — asks before the release commit exists', () => {
  const MARKED_HEAD = [
    ...
    '- **Honest nulls:** _none_',
    '',
    // The tests footer below is the section-level publication obligation
    // (`section_publication_blockers`), so a bare curated head is not a
    // publishable SECTION. The mix line is a measurement and blocks nothing
    // since ADR-261; it stays for realism.
    '> **Governance mix:** governance-only 2 vs consumer-only 1 (taxonomy 1.0.0).',
    '',
    'Tests: 1000 (+0 since 0.0.0)',
    '',

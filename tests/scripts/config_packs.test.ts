...
// Runs against the REAL repo (deterministic resolver over the committed vocab
// + the gitignored discovery manifest if present). Both sides read the same
// inputs, so the JSON is byte-comparable. legacy-all is fully deterministic.
const py = hasPython3();
describe.skipIf(!py)('packs — golden parity (python3 vs tsx CLI)', () => {
    it('legacy-all --json matches', () => {
        const pyOut = runPy([...PY_MODULE_ARGS, '--legacy-all', '--json']);
    ...
    it('scoped --packs laravel,finance-basic --json matches', () => {
        const pyOut = runPy([...PY_MODULE_ARGS, '--packs', 'laravel,finance-basic', '--json']);
        const tsOut = runTsx(TS_SCRIPT, ['--packs', 'laravel,finance-basic', '--json']);
        expect(tsOut.stdout).toBe(pyOut.stdout);
    });

    it('always-on only --json matches', () => {
        const pyOut = runPy([...PY_MODULE_ARGS, '--json']);

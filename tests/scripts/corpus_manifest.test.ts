...
describe('captureUserScope', () => {
    it('marks a byte-identical twin as causing a skip, replaying the generator comparison', () => {
        const root = fixture({ 'a.md': 'projected' }, { 'a.md': 'source bytes' });
        const h = home({ 'a.md': 'source bytes' });
        const twins = captureUserScope(root, h);
        expect(twins).toHaveLength(1);
        expect(twins[0]?.causes_skip).toBe(true);
    });

    it('records a differing twin without marking it as a skip', () => {
        const root = fixture({ 'a.md': 'projected' }, { 'a.md': 'source bytes' });
        const h = home({ 'a.md': 'stale bytes' });
        expect(captureUserScope(root, h)[0]?.causes_skip).toBe(false);
    });

    it('leaks no account name anywhere in a captured manifest, twin or global layer', () => {
        const root = fixture({ 'a.md': 'p' }, { 'a.md': 's' });
        const h = home({ 'a.md': 's' });
        const captured = captureUserScope(root, h);
...
describe('captureManifest and diffManifests', () => {
    ...
    it('surfaces a user-scope skip change, which a recorded commit cannot explain', () => {
        const root = fixture({ 'a.md': 'a', 'b.md': 'b' }, { 'a.md': 'source' });
        const before = captureManifest(opts(root, home({})));
        const after = captureManifest(opts(root, home({ 'a.md': 'source' })));
        expect(diffManifests(before, after).map((d) => d.field)).toContain('user_scope.skipping');
    });
});

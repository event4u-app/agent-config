#!/usr/bin/env bash
# Contract tests for scripts/install_anthropic_key.sh.
#
# Mirror of tests/test_install_openai_key.sh. Two test surfaces:
...
test_target_path_contract() {
    ...
    assert_grep "TARGET_FILE is anthropic.key" 'TARGET_FILE=.*anthropic\.key'
}
...
test_format_check() {
    assert_grep "rejects non-sk-ant- prefix" 'sk-ant-\*'
    assert_grep "rejects empty input" '\-z "\$\{API_KEY\}"'
}

test_atomic_write() {
    assert_grep "uses mktemp for tmpfile" 'mktemp .*anthropic\.key'
...
test_no_bypass_paths() {
    ...
    assert_no_grep "no ANTHROPIC_API_KEY env-var fallback" 'ANTHROPIC_API_KEY'
...
test_post_install_hint_present() {
    ...
    assert_grep "hint mentions ai_council.members.anthropic.enabled" 'ai_council.*members.*anthropic.*enabled'
...
echo "🧪  Running install_anthropic_key.sh contract tests..."
echo ""

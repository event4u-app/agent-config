...
SPEC = importlib.util.spec_from_file_location(
    ...
    ROOT / "src" / "scripts" / "check_test_coverage_diff.py",
)
...
def test_naming_variance_still_matches():
    ...
    changed = [("A", "src/scripts/check_foo.py"), ("A", "tests/sub/foo_test.py")]
...
def test_new_gate_without_test_warns():
    changed = [("A", "src/scripts/lint_bar.py")]
    warnings, suppressed = mod.evaluate(changed, NO_PRAGMA)
    assert warnings == ["src/scripts/lint_bar.py"]
    assert suppressed == []
...
def test_pragma_suppresses_warning():
    changed = [("A", "src/scripts/check_baz.py")]
    reason_for = lambda p: "no behaviour — thin re-export" if p == "src/scripts/check_baz.py" else None  # noqa: E731
    warnings, suppressed = mod.evaluate(changed, reason_for)
    assert warnings == []
    assert suppressed == [("src/scripts/check_baz.py", "no behaviour — thin re-export")]
...
def test_modified_existing_gate_does_not_trigger():
    ...
    changed = [("M", "src/scripts/check_foo.py")]
    warnings, suppressed = mod.evaluate(changed, NO_PRAGMA)
...
def test_main_is_warn_only_exit_zero(monkeypatch, capsys):
    monkeypatch.setattr(mod, "_git_name_status", lambda base: [("A", "src/scripts/check_untested.py")])

...
- **Kernel + Router** (beta) — 9 always-loaded Iron-Law rules, tier-1 / tier-2 routing, cost profiles, per-rule char caps: [`kernel-membership`](docs/contracts/kernel-membership.md) + [`rule-router`](docs/contracts/rule-router.md).
- **Trust & Safety** (beta) — closed trust-level enum, HRR banner, per-pack safety floors, installer confirm + lockfile escalation, coherence lint: [`trust-and-safety`](docs/contracts/trust-and-safety.md) ([`ADR-018`](docs/decisions/ADR-018-trust-and-safety-layer.md)).
...
- **Personas** — 11 review-lens cast (6 core · 5 specialist), `personas:` vs `/mode` axes: [`docs/personas.md`](docs/personas.md) (beta).

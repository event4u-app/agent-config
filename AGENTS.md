...
- **Discovery** (workspaces / packs / `dist/discovery/discovery-manifest.json`) — contract [`ADR-013`](docs/decisions/ADR-013-discovery-frontmatter-contract.md), per-artefact worked examples (beta) [`frontmatter-contract`](docs/contracts/frontmatter-contract.md), how-to [`customization § Workspaces & packs`](docs/customization.md#workspaces--packs-discovery). Linted by `task lint-artefact-frontmatter`; opt-in via `./agent-config hooks:install`.

...
- **Multi-tool projection** — Augment, Claude Code, Cursor, Cline, Windsurf, Gemini CLI, Claude.ai bundle pipeline that ships from `.agent-src/` to consumer surfaces: [`docs/architecture.md`](docs/architecture.md#cloud-bundle-pipeline). Built-in MCP server (`scripts/mcp_server/`) exposes the same content over JSON-RPC to MCP-aware clients: [`docs/mcp-server.md`](docs/mcp-server.md).

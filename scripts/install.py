...
def ensure_claude_bridge(project_root: Path, force: bool) -> list[dict[str, Any]]:
    ...
    them here would re-introduce the shared-array collision with a neighbour
    tool (e.g. ruflo) or a developer's `settings.local.json` — Claude Code
    merges plugin-scope and settings-scope hooks at runtime and dedups by
    command string, so plugin delivery needs no `hooks` block here. See
...
# --- Ruflo coexistence detection (road-to-ruflo-bridge Phase 2) ---

# Signals that ruvnet/ruflo is installed in the project. Kept as data so
# the detection reason is auditable and the lists are easy to extend.
RUFLO_PLUGIN_IDS = ("ruflo-core@ruflo",)
RUFLO_MCP_KEYS = ("claude-flow", "ruflo")
RUFLO_HOOK_MARKERS = ("hook-handler", "auto-memory-hook")
RUFLO_HELPER_FILES = ("hook-handler.cjs", "auto-memory-hook.mjs")


def detect_ruflo(project_root: Path) -> tuple[bool, str]:
    """Detect whether ruvnet/ruflo is installed in this project.

    Returns ``(found, reason)``; ``reason`` names the first matched signal so
    the install flow can surface *why* coexistence engaged (and stays empty
    when nothing matched). Checks, in order: the `claude-flow.config.json`
    marker file, ruflo's plugin id / MCP server / hook-handler command in
    `.claude/settings.json`, then ruflo's helper scripts under
    `.claude/helpers/`. See docs/contracts/ruflo-coexistence.md.
    """
    if (project_root / "claude-flow.config.json").exists():
        return True, "claude-flow.config.json present"
    ...
    if settings.exists():
        ...
        if isinstance(data, dict):
            ...
            for pid in RUFLO_PLUGIN_IDS:
                if pid in plugins:
                    return True, f"enabledPlugins[{pid!r}]"
            mcp = data.get("mcpServers", {}) or {}
            for key in RUFLO_MCP_KEYS:
                if key in mcp:
                    return True, f"mcpServers[{key!r}]"
            hook_blob = json.dumps(data.get("hooks", {}) or {})
            for marker in RUFLO_HOOK_MARKERS:
                if marker in hook_blob:
                    return True, f"hook command references {marker!r}"

    helpers = project_root / ".claude" / "helpers"
    for name in RUFLO_HELPER_FILES:
        if (helpers / name).exists():
            return True, f".claude/helpers/{name}"

    return False, ""


RUFLO_MODES = ("coexist", "skip")


def _read_ruflo_mode(project_root: Path) -> str:
    """Return the persisted ``integrations.ruflo.mode``, or ``""`` if unset."""
    settings = project_root / SETTINGS_FILE
    if not settings.exists():
        return ""
    try:
        import yaml  # noqa: PLC0415
        data = yaml.safe_load(settings.read_text(encoding="utf-8")) or {}
    except Exception:  # noqa: BLE001 — best-effort read, never block install
        return ""
    mode = ((data.get("integrations") or {}).get("ruflo") or {}).get("mode")
    return mode if mode in RUFLO_MODES else ""


def _persist_ruflo_mode(project_root: Path, mode: str) -> None:
    """Write ``integrations.ruflo.mode`` into .agent-settings.yml, preserving rest."""
    settings = project_root / SETTINGS_FILE
    text = settings.read_text(encoding="utf-8") if settings.exists() else ""
    if "integrations:" in text and "ruflo:" in text and "    mode:" in text:
        updated = _replace_template_value(text, "integrations.ruflo.mode", mode)
        if updated != text:
            settings.write_text(updated, encoding="utf-8")
            return
    block = (
        "\n# Written by the ruflo coexistence detector "
        "(docs/contracts/ruflo-coexistence.md).\n"
        f"integrations:\n  ruflo:\n    mode: {mode}\n"
    )
...
def prompt_ruflo_mode(reason: str) -> str:
    """One-time interactive choice when ruflo is detected. Returns the mode."""
    print()
    info(f"ruflo detected ({reason}).")
    info("agent-config ships its Claude hooks via plugin scope, so both tools coexist.")
    print("  1) Coexist — run agent-config's hooks alongside ruflo (recommended)")
    print("  2) Skip    — opt out of agent-config's hooks in this project")
    attempts = 0
    while attempts < 3:
        try:
            reply = _read_line("Choose [1/2]: ")
        except EOFError:
            return "coexist"
        if reply in ("1", "coexist", "c", ""):
            return "coexist"
        if reply in ("2", "skip", "s"):
            return "skip"
        attempts += 1
        warn(f"Invalid choice '{reply}'. Enter 1 or 2.")
    return "coexist"


def resolve_ruflo_mode(
    project_root: Path, *, interactive: bool, force: bool = False,
) -> str:
    """Detect ruflo and resolve the one-time coexistence mode.

    Returns the active mode (``coexist``/``skip``) or ``""`` when ruflo is
    absent. A previously persisted choice is honored without re-asking
    (decline = silence, per scope-control). Non-interactive runs default to
    ``coexist`` (the no-block default) and persist it so the hook dispatcher
    can read it. See docs/contracts/ruflo-coexistence.md and
    agents/roadmaps/road-to-ruflo-bridge.md (Phase 3).
    """
    found, reason = detect_ruflo(project_root)
    if not found:
        return ""
    existing = _read_ruflo_mode(project_root)
    if existing and not force:
        return existing
    mode = prompt_ruflo_mode(reason) if interactive else "coexist"
    _persist_ruflo_mode(project_root, mode)
    success(f"ruflo coexistence mode: {mode} ({reason})")
    return mode

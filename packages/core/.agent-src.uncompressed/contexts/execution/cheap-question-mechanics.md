...
Triggers — files under `agents/runtime/`, council `questions/` / `responses/` / `sessions/`, cache directories, build artefacts, `dist/` regenerated by `task sync`, anything `.gitignore`'d that the system already prunes on a schedule. The system owns the lifecycle; surfacing it as a choice fabricates a decision point.
